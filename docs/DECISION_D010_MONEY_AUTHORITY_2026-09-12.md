...
A verified apartment/hotel payment is not immediately withdrawable Partner money. `settle_verified_property_partner_payment` records the commercial split/commission and adds the Partner portion to `wallets.pending_balance`. `property_partner_earning_releases` records the lifecycle release gate. For Hotel and Short Let stays, authorized check-in starts the booking's snapshotted two-to-four-hour arrival-issue window; release cannot occur before that deadline or while a linked case/protection dispute remains open. Checkout is not the normal payout signal. `release_property_partner_earning` moves only eligible value from pending to available. A reversal/refund before release reverses pending value rather than fabricating a payout.
...
- Hotel/Short Let check-in does not release early; a timely formal arrival issue
  freezes the affected earning and suppresses queued release.
