...
- The paid Worker plan (default public name: WeHouse Works) is optional monthly
  or yearly work software. Its gold `PRO` mark means only that the server has verified an active
  subscription. It is not identity, licensing, safety, ranking, endorsement or
  job access.
- Web paid-plan billing uses Paystack. iOS and Android use their respective
...
| Paid Worker web subscription | **Fail-closed** | Final monthly/yearly prices and feature schedule; renewal, paid-through cancellation, failed-payment, refund and support terms; consumer-law review; tax treatment; Paystack monthly/annual plans; signed webhook; cancellation route; support capacity | `worker_pro_sales_enabled=false` until legal approval, versioned terms and at least one synchronized Paystack plan are present; provider event creates entitlement |
| Paid Worker iOS subscription | **Blocked** | App Store Connect monthly/yearly auto-renewable products in one subscription group, store prices, purchase/restore/manage flows, App Store Server Notifications and server transaction verification | `worker_pro_ios_sales_enabled=false`; client cannot grant entitlement; enabling requires a current legal launch approval |
| Paid Worker Android subscription | **Blocked** | Play Console subscription/base plans, store prices, Billing Library purchase/acknowledgement, Real-time Developer Notifications and server verification | `worker_pro_android_sales_enabled=false`; client cannot grant entitlement; enabling requires a current legal launch approval |
| Featured Workers / Sponsored placement | **Blocked** | Written counsel classification of paid Worker placement under the exact marketplace model; non-deceptive Sponsored disclosure; eligibility and category-credential matrix; fair-rotation rules; substantiated impression/open/booking metrics; advertising and consumer-law review | `worker_featured_sales_enabled=false`; a separate `worker_featured_placement` approval and the approved Worker marketplace gate are both required; Featured is a separate labelled section and never changes organic results or trust |
...
| Reviewed / Trusted / public claims | **Partly implemented** | Published definitions and evidence; substantiation logs; correction/appeal and expiry rules; marketing review that avoids safety, licence or endorsement guarantees | Reviewed and Trusted are non-purchasable; paid membership and its PRO mark cannot influence them |
