...
# 1) Existing client helper must use the authoritative property/reservation conversation APIs.
...
# 2) Atomic first Send must choose the same authoritative thread instead of generic Support.
...
# 3) Hotel booking Message WeHouse is the canonical booking-linked WeHouse thread.
...
# 4) Restore pre-booking Hotel -> Message WeHouse in the current modern footer.
...
if old not in source: raise SystemExit('current hotel sticky action block not found')
source=source.replace(old,new,1)
path.write_text(source)
