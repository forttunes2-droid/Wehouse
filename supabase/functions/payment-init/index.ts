...
serve(async(req)=>{
 ...
 try{
  ...
  let initialized:any=null;try{initialized=await response.json()}catch{initialized=null}if(!response.ok||!initialized?.status||!initialized?.data?.authorization_url||!initialized?.data?.access_code)return json({success:false,error:initialized?.message||'Paystack could not initialize this payment'},response.status>=500?502:400);
 ...
 }catch(error){return json({success:false,error:error instanceof Error?error.message:'Payment initialization failed'},500)}
});
