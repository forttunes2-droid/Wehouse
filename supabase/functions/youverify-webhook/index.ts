...
function toBase64(bytes: Uint8Array) { let binary=''; for (const b of bytes) binary += String.fromCharCode(b); return btoa(binary); }
...
async function validSignature(rawBody:string,signatureHeader:string,secret:string){ const encoder=new TextEncoder(); const key=await crypto.subtle.importKey('raw',encoder.encode(secret),{name:'HMAC',hash:'SHA-256'},false,['sign']); const signature=new Uint8Array(await crypto.subtle.sign('HMAC',key,encoder.encode(rawBody))); const supplied=signatureHeader.trim().replace(/^sha256=/i,''); return constantTimeEqual(supplied.toLowerCase(),toHex(signature))||constantTimeEqual(supplied,toBase64(signature)); }
function firstString(...values:unknown[]){ for(const value of values){ if(typeof value==='string'&&value.trim())return value.trim(); if(typeof value==='number'&&Number.isFinite(value))return String(value); } return ''; }
...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  let identityStatus:'verified'|'failed'|'pending_external'|'needs_retry'; let failureReason:string|null=null;
  ...
  const {data:workerId,error}=await db.rpc('record_external_worker_identity_result_by_reference',{p_provider:'youverify',p_reference:reference,p_status:identityStatus,p_failure_reason:failureReason});
