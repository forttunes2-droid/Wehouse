...
async function markRequest(admin:any,requestId:string,userId:string,patch:Record<string,unknown>){
  return admin.from('payout_account_change_requests').update({...patch,updated_at:new Date().toISOString()}).eq('request_id',requestId).eq('user_id',userId);
}
...
async function reconcileChange(admin:any,paystackSecret:string,row:any){
  if(row.status==='succeeded'){
    if(!row.bank_account_id)return{pending:false,status:'failed',error:'Saved request is missing its payout account reference'};
    const{data:account}=await admin.from('bank_accounts').select('id,bank_name,bank_code,account_number,account_name,verified_at,is_default,is_active').eq('id',row.bank_account_id).eq('user_id',row.user_id).maybeSingle();
    return{pending:false,status:'succeeded',account};
  }
  ...
  if(!recipientCode&&ageMs>=90_000){
    ...
    if(!recipientCode){
      await markRequest(admin,row.request_id,row.user_id,{status:'failed',error_code:'PROVIDER_RESULT_NOT_FOUND',error_message:'The payout-account change was not completed. You can safely try again.'});
      return{pending:false,status:'failed',error:'The payout-account change was not completed. You can safely try again.'};
    }
  }
