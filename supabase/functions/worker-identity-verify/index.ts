...
serve(async (req) => {
  ...
  try {
    ...
    if (!providerResponse.ok || !provider?.success) {
      ...
      await admin.rpc('record_external_worker_identity_result', { p_worker_id: profile.user_id, p_provider: 'youverify', p_reference: provider?.data?.id || provider?.data?.verificationId || null, p_status: transient ? 'needs_retry' : 'failed', p_failure_reason: failure });
    ...
    if (providerStatus === 'pending') {
      if (!providerReference) {
        await admin.rpc('record_external_worker_identity_result', { p_worker_id: profile.user_id, p_provider: 'youverify', p_reference: null, p_status: 'needs_retry', p_failure_reason: 'Identity provider returned pending without a verification reference' });
        return response({ success: false, verified: false, reason: 'Identity provider returned an incomplete pending response' }, 503);
      }
      const { error: pendingError } = await admin.rpc('record_external_worker_identity_result', { p_worker_id: profile.user_id, p_provider: 'youverify', p_reference: providerReference, p_status: 'pending_external', p_failure_reason: null });
    ...
    const { error: recordError } = await admin.rpc('record_external_worker_identity_result', { p_worker_id: profile.user_id, p_provider: 'youverify', p_reference: providerReference || null, p_status: verified ? 'verified' : 'failed', p_failure_reason: reason });
