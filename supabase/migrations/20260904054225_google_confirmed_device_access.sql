...
begin
  ...
  if coalesce((v_result ->> 'new_device')::boolean, false) and v_session_id is not null then
    ...
    set message = concat_ws(' · ', p_device, p_os, p_browser, v_location),
        destination_params = coalesce(destination_params, '{}'::jsonb) || jsonb_build_object('location', v_location)
...
begin
  ...
  if not v_google_verified then raise exception 'Confirm with Google is required'; end if;
  ...
  update public.notifications
  set title = 'New device confirmed', read = true, read_at = coalesce(read_at, now()),
      destination_params = coalesce(destination_params, '{}'::jsonb) || jsonb_build_object('decision', 'confirmed_with_google')
