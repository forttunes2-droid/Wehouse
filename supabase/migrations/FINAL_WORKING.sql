-- ═══════════════════════════════════════════════════════════════════
-- WEHOUSE FINAL WORKING SQL
-- Functions do NOT reference profiles.user_id — frontend passes all data
-- ═══════════════════════════════════════════════════════════════════

-- Step 1: pgcrypto
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- Step 2: Add columns to profiles (no constraints, no references)
DO $$ BEGIN
...
EXCEPTION WHEN OTHERS THEN RAISE NOTICE 'profiles columns: %', SQLERRM;
END $$;
...
CREATE POLICY IF NOT EXISTS "sr" ON platform_settings FOR SELECT USING (TRUE);
CREATE POLICY IF NOT EXISTS "sw" ON platform_settings FOR ALL TO authenticated USING (TRUE) WITH CHECK (TRUE);
...
-- Step 5: Clean wrong settings, insert correct ones
...
EXCEPTION WHEN OTHERS THEN RAISE NOTICE 'Worker status constraint: %', SQLERRM;
END $$;

-- Step 7: Wallet balances (NO REFERENCES to any other table)
...
CREATE POLICY IF NOT EXISTS "wa" ON wallet_balances FOR ALL USING (TRUE) WITH CHECK (TRUE);
...
CREATE POLICY IF NOT EXISTS "wtx" ON wallet_transactions FOR ALL USING (TRUE) WITH CHECK (TRUE);
...
CREATE POLICY IF NOT EXISTS "ea" ON escrow_transactions FOR ALL USING (TRUE) WITH CHECK (TRUE);
...
CREATE POLICY IF NOT EXISTS "wda" ON withdrawal_requests FOR ALL USING (TRUE) WITH CHECK (TRUE);
...
CREATE POLICY IF NOT EXISTS "aa" ON financial_audit_log FOR ALL USING (TRUE) WITH CHECK (TRUE);

-- Step 12: Commission rate function
...
DECLARE v_rate TEXT; BEGIN
  ...
  RETURN COALESCE(NULLIF(trim(v_rate),'')::NUMERIC, 10);
END; $$;

-- Step 13: Withdrawal function — NO profiles table lookup, all data passed from frontend
...
BEGIN
  ...
  IF v_balance IS NULL THEN
    -- Auto-create wallet
    INSERT INTO wallet_balances (user_id) VALUES (p_user_id);
    v_balance := 0;
  END IF;
  ...
  IF p_amount < v_min THEN
    RETURN jsonb_build_object('success', FALSE, 'error', format('Minimum withdrawal is N%s', v_min));
  END IF;
  ...
  IF COALESCE(p_bank_code, '') = '' OR COALESCE(p_account_number, '') = '' THEN
    RETURN jsonb_build_object('success', FALSE, 'error', 'Bank details required. Add bank account in profile settings.');
  END IF;
  ...
  -- Create withdrawal request — NO fee, full amount
  INSERT INTO withdrawal_requests (
    user_id, user_role, amount_requested,
    withdrawal_fee, amount_paid,
    bank_name, bank_code, bank_account_number,
    status, metadata
  ) VALUES (
    p_user_id, 'worker', p_amount,
    0, p_amount,
    COALESCE(p_bank_name, ''), COALESCE(p_bank_code, ''), COALESCE(p_account_number, ''),
    'pending', jsonb_build_object('net_amount', p_amount, 'fee', 0)
  ...
  VALUES (p_user_id, 'withdrawal_request', -p_amount, v_balance - p_amount,
    v_req_id::text, 'withdrawal', format('Withdrawal request: N%s', p_amount));
...
-- Step 14: Credit wallet function
...
SELECT 'COMPLETE: All tables and functions created successfully!' AS status;
