-- WEHOUSE: COMPLETE FIX SQL (July 5, 2026)
-- Run this ENTIRE file in your Supabase SQL Editor

-- ================================================================
-- PART 0: STORAGE BUCKETS
-- ================================================================
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES ('listing-videos', 'listing-videos', true, 52428800, ARRAY['video/mp4','video/quicktime','video/webm'])
ON CONFLICT (id) DO NOTHING;

INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES ('worker-files', 'worker-files', true, 104857600, ARRAY['video/mp4','video/quicktime','video/webm','image/jpeg','image/png','image/webp','application/pdf'])
ON CONFLICT (id) DO NOTHING;

INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES ('chat-files', 'chat-files', true, 26214400, ARRAY['image/jpeg','image/png','image/webp','image/gif','video/mp4','audio/mpeg','audio/wav','application/pdf'])
ON CONFLICT (id) DO NOTHING;

DROP POLICY IF EXISTS "listing-videos-public" ON storage.objects;
CREATE POLICY "listing-videos-public" ON storage.objects
...
CREATE POLICY "worker-files-public" ON storage.objects
...
CREATE POLICY "chat-files-public" ON storage.objects
...
-- ================================================================
-- PART 5: WORKER BOOKING FUNCTIONS (with scheduled_date)
-- ================================================================
...
BEGIN
  ...
  VALUES (v_code, p_user_id, p_worker_id, p_service_type, p_description, p_address, NULL, 0, 300, 0, 0, 'booking_requested', p_customer_message, NOW(), NOW())
...
CREATE OR REPLACE FUNCTION public.worker_accept_booking(
  p_booking_id UUID, p_worker_id TEXT, p_negotiated_amount DECIMAL, p_scheduled_date TEXT DEFAULT NULL
)
RETURNS BOOLEAN LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  UPDATE worker_bookings SET status = 'waiting_payment', negotiated_amount = p_negotiated_amount, agreed_amount = p_negotiated_amount, worker_commission = p_negotiated_amount * 0.125, worker_receives = p_negotiated_amount * 0.875, worker_approved = TRUE, scheduled_date = COALESCE(p_scheduled_date::DATE, scheduled_date), updated_at = NOW()
...
-- ================================================================
-- PART 6: PARTNER SUPPORT FUNCTIONS
-- ================================================================
