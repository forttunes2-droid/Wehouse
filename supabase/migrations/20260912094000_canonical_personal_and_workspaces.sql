...
-- Normalize legacy identity categories without deleting any work authority.
...
-- Backfill the compatibility role into an additive workspace grant.
...
select
  ...
  case when p.role='creator' then 'global'
       when p.role in ('admin','staff') and nullif(btrim(coalesce(p.admin_state,p.state,'')),'') is not null then 'branch'
       else 'global' end,
  case when p.role in ('admin','staff') then coalesce(nullif(btrim(p.admin_state),''),nullif(btrim(p.state),'')) else null end,
  case when p.role in ('admin','staff') then coalesce(nullif(btrim(p.admin_lga),''),nullif(btrim(p.city),'')) else null end,
  'active',
  null,
  now(),now(),now()
...
as $$
  select jsonb_build_object(
    ...
    'privileged_workspaces',coalesce((
      select jsonb_agg(x order by x->>'role')
      from (
        select jsonb_build_object(
          'role',a.workspace_role,
        ...
        where exists(
          ...
          where tm.member_user_id=p.user_id and tm.status='active'
        )
      ) roles
    ),'[]'::jsonb)
  )
...
-- Public signup creates only a Personal identity. p_role is retained in the
-- signature so an older client cannot break creation, but it never shapes identity.
...
begin
  ...
  update public.profiles
  set role='worker',account_kind='consumer',worker_status=coalesce(worker_status,'pending'),updated_at=now()
...
begin
  ...
  update public.profiles
  set role='property_partner',account_kind='consumer',updated_at=now()
  where user_id=v_profile.user_id;
