...
select
  legacy.staff_id,'worker_operations',legacy.granted_by,
...
where legacy.is_active
  ...
  and lower(btrim(legacy.permission)) in(
    'verification','worker_verification','worker_review','worker_operations'
  )
...
where lower(btrim(permission)) in(
  'verification','worker_verification','worker_review'
) and is_active;
...
begin
  ...
  insert into public.admin_audit_log(
    ...
    v_actor.user_id,'staff_operation_changed','profiles',p_staff_id,
    jsonb_build_object(
      'operation',v_domain,'enabled',p_enabled,
      'actor_role',v_actor.role,'state',v_target.assigned_state,
      'lga',v_target.assigned_lga,'authority_model','one_staff_one_operation'
    )::text,now()
  );
end
$$;
...
begin
  ...
  if pending_role='worker' then
    return public.current_staff_has_permission('worker_operations');
  end if;
