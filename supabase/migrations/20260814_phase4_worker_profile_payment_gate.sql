...
BEGIN
  ...
  WHERE k NOT IN (
    ...
    'profile_complete','worker_occupation','worker_skills','worker_price','worker_bio','worker_experience'
  );
  ...
  IF v_profile.role<>'worker' AND (
    ...
    p_updates ? 'worker_price' OR p_updates ? 'worker_bio' OR p_updates ? 'worker_experience'
  ) THEN
...
BEGIN
  ...
  IF NOT public.worker_professional_profile_ready(v_caller) THEN
    RETURN jsonb_build_object('success',false,'error','Complete your professional profile and service coverage before payment');
  END IF;
  ...
  WHERE user_id=v_caller AND purpose='worker_verification' AND status='pending' AND created_at<now()-interval '30 minutes';
  ...
  IF v_existing IS NOT NULL THEN
    IF v_existing.amount_total=v_amount THEN
      RETURN jsonb_build_object('success',true,'reference',v_existing.paystack_reference,'amount',v_amount,'existing',true);
    END IF;
