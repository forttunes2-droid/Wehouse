-- ═══════════════════════════════════════════════════════════════
-- WEHOUSE: EXACT VERIFIED FIX — July 5, 2026
-- Verified against live database schema before writing
-- Run this ENTIRE file in Supabase SQL Editor
-- ═══════════════════════════════════════════════════════════════

-- ================================================================
-- PART 0: TABLES (verified — none of these exist in live DB)
-- ================================================================

-- 0a. worker_bookings — stores all worker job bookings
CREATE TABLE IF NOT EXISTS worker_bookings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  booking_code TEXT,
  user_id TEXT NOT NULL,          -- customer who booked
  worker_id TEXT NOT NULL,        -- worker who was booked
  service_type TEXT,
  description TEXT,
  address TEXT,
  scheduled_date DATE,
...
-- 0b. booking_conversations — private chat per booking
...
-- 0c. booking_messages — messages inside booking conversations
...
-- 0d. partner_support_conversations — property partner support threads
...
-- 0e. partner_support_messages — messages inside support conversations
...
CREATE POLICY wb_all ON worker_bookings
  FOR ALL TO authenticated
  USING (user_id = (SELECT user_id FROM profiles WHERE auth_id = auth.uid()::text LIMIT 1)
...
CREATE POLICY bc_all ON booking_conversations
  FOR ALL TO authenticated
  USING (user_id = (SELECT user_id FROM profiles WHERE auth_id = auth.uid()::text LIMIT 1)
         OR worker_id = (SELECT user_id FROM profiles WHERE auth_id = auth.uid()::text LIMIT 1)
         OR EXISTS (SELECT 1 FROM profiles WHERE auth_id = auth.uid()::text AND role IN ('staff','admin','creator')));
...
CREATE POLICY bm_all ON booking_messages
  FOR ALL TO authenticated
...
CREATE POLICY psc_all ON partner_support_conversations
  FOR ALL TO authenticated
  USING (partner_id = (SELECT user_id FROM profiles WHERE auth_id = auth.uid()::text LIMIT 1)
...
CREATE POLICY psm_all ON partner_support_messages
  FOR ALL TO authenticated
  USING (conversation_id IN (
...
-- ================================================================
-- PART 2: WORKER BOOKING RPC FUNCTIONS
-- ================================================================
...
-- ================================================================
-- PART 3: FIELD OFFICER RPC FUNCTIONS
-- ================================================================

CREATE OR REPLACE FUNCTION public.get_my_inspections(p_field_officer_id TEXT)
RETURNS TABLE(id UUID, inspection_code TEXT, property_address TEXT, property_city TEXT, property_state TEXT, property_type TEXT, status TEXT, owner_id TEXT, owner_name TEXT, owner_email TEXT, owner_phone TEXT, notes TEXT, field_officer_id TEXT, partner_id TEXT, scheduled_date TIMESTAMPTZ, completed_at TIMESTAMPTZ, created_at TIMESTAMPTZ, photo_urls TEXT[], document_urls TEXT[], _source TEXT)
LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT ir.id, ir.request_code, ir.property_address, ir.property_city, ir.property_state, ir.property_type, ir.status, ir.owner_id, ir.owner_name, ir.owner_email, ir.owner_phone, ir.notes, ir.field_officer_id, ir.partner_id, ir.scheduled_date, ir.completed_at, ir.created_at, ir.photo_urls, ir.document_urls, 'inspection'::TEXT
  FROM inspection_requests ir
  WHERE ir.field_officer_id = p_field_officer_id
  ORDER BY ir.created_at DESC;
$$;

CREATE OR REPLACE FUNCTION public.update_inspection_status(p_inspection_id UUID, p_new_status TEXT, p_source TEXT DEFAULT 'user', p_report TEXT DEFAULT NULL, p_condition TEXT DEFAULT NULL)
RETURNS BOOLEAN LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
...
-- ================================================================
-- PART 4: PARTNER SUPPORT RPC FUNCTIONS
-- ================================================================
...
-- ================================================================
-- DONE: 5 tables + 23 RPC functions created
-- ================================================================
