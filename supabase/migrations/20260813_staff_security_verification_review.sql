...
BEGIN
  ...
  IF v_actor.role='staff' AND NOT (
    v_actor.assigned_state IS NOT NULL AND lower(v_worker.state)=lower(v_actor.assigned_state)
  ...
  UPDATE public.worker_verifications SET status=p_status,reviewed_by=v_actor.user_id,review_notes=COALESCE(NULLIF(BTRIM(p_notes),''),NULLIF(BTRIM(p_reason),'')),reviewed_at=now(),updated_at=now() WHERE id=(SELECT id FROM public.worker_verifications WHERE worker_id=p_worker_id ORDER BY created_at DESC LIMIT 1);
