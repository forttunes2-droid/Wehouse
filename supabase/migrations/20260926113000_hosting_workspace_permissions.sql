-- Delegated Hosting workspace permissions.
-- Operations co-hosts can run assigned stays. Full-hosting co-hosts additionally
-- control future price/availability. Ownership, payout and management mode remain owner-only.
...
as $$
  select case
    when assignment.assignment_role='owner' then 'owner'
    else assignment.access_level
  end
...
create or replace function public.current_actor_can_control_property_commercials(p_listing_id uuid)
returns boolean
language sql
stable
security definer
  ...
  select coalesce(jsonb_agg(
    (to_jsonb(l)-'access_code'-'private_video_url'-'private_video_path')
      || jsonb_build_object(
        ...
        '_access_level',case when a.assignment_role='owner' then 'owner' else a.access_level end,
        '_can_manage',a.status='active',
        '_can_control_commercials',
          a.assignment_role='owner' or a.access_level='full_hosting',
    ...
    where (p_listing_id is null or l.id::text=p_listing_id or l.listing_id=p_listing_id)
      and (
        (
          ...
          and (
            (a.assignment_role='owner')
            or (a.assignment_role='manager' and r.responsible_host_user_id=v_actor)
          )
          and (r.reservation_fee_status='paid' or r.manual_payment_status in ('paid','completed'))
        )
        or (
          a.assignment_role='owner'
          and r.management_mode_snapshot='wehouse'
          and (
            (
...
begin
  ...
  return jsonb_build_object(
    ...
    'my_access_level',public.current_actor_property_host_access_level(v_listing.id),
    'assignments',coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'status',a.status,
        'access_level',case when a.assignment_role='owner' then 'owner' else a.access_level end
...
declare
  ...
  v_before numeric;
begin
  if not public.current_actor_can_control_property_commercials(p_listing_id) then
    raise exception 'Full hosting access is required to change pricing';
  end if;
  ...
  if v_listing.management_mode<>'host' then raise exception 'This property is not Host managed'; end if;
...
begin
  if not public.current_actor_can_control_property_commercials(p_listing_id) then
    raise exception 'Full hosting access is required to close dates';
  end if;
  select * into v_listing
  from public.listings
  where id=p_listing_id and deleted_at is null
...
begin
  ...
  if not public.current_actor_can_control_property_commercials(v_block.listing_id) then
...
revoke all on function public.current_actor_can_control_property_commercials(uuid) from public,anon;
...
grant execute on function public.current_actor_can_control_property_commercials(uuid) to authenticated,service_role;
grant execute on function public.get_my_property_host_invites() to authenticated,service_role;
