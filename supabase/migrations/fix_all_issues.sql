-- ═══════════════════════════════════════════════════════════════
-- FIX ALL ISSUES — Run this entire file in Supabase SQL Editor
-- ═══════════════════════════════════════════════════════════════

-- ═══════════════════════════════════════════════════════════════
-- 1. FIX WORKER STATUS CHECK CONSTRAINT
-- ═══════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════
-- 2. FIX CREATOR AUTH — Add password column and fix functions
-- ═══════════════════════════════════════════════════════════════

-- Add password column to profiles (bcrypt hashed)
...
-- Set creator password (hashes with bcrypt)
...
-- Verify creator password (bcrypt compare)
...
-- Check if creator has auth set up
CREATE OR REPLACE FUNCTION public.has_creator_auth(p_user_id TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  RETURN EXISTS(
    ...
    WHERE user_id = p_user_id 
    AND creator_auth_enabled = true 
    AND creator_auth_password IS NOT NULL
  );
END;
$$;
...
CREATE TABLE IF NOT EXISTS announcements (
  ...
  target_roles TEXT[] DEFAULT ARRAY['user', 'worker', 'property_partner', 'staff', 'admin'],
...
CREATE POLICY IF NOT EXISTS announcements_select ON announcements FOR SELECT USING (is_active = true);

-- Only creator/admin can manage
CREATE POLICY IF NOT EXISTS announcements_manage ON announcements FOR ALL
  USING (EXISTS (
    SELECT 1 FROM profiles 
...
-- RPC: Get announcements for a role
CREATE OR REPLACE FUNCTION get_announcements_for_role(p_role TEXT DEFAULT 'user')
RETURNS TABLE (id UUID, title TEXT, content TEXT, created_by TEXT, created_at TIMESTAMPTZ)
...
BEGIN
  ...
  AND (p_role = ANY(a.target_roles) OR a.target_roles @> ARRAY['all'])
  ORDER BY a.created_at DESC;
END;
$$;

-- RPC: Create announcement
CREATE OR REPLACE FUNCTION create_announcement(
  p_title TEXT, p_content TEXT, p_target_roles TEXT[], p_created_by TEXT, p_created_by_email TEXT
)
RETURNS UUID
LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE v_user_role TEXT; v_id UUID;
BEGIN
  ...
  IF v_user_role NOT IN ('creator', 'admin') THEN
    RAISE EXCEPTION 'Only creator or admin can create announcements';
  END IF;
  INSERT INTO announcements (title, content, target_roles, created_by, created_by_email)
  VALUES (p_title, p_content, p_target_roles, p_created_by, p_created_by_email)
  RETURNING id INTO v_id;
  RETURN v_id;
END;
$$;

-- RPC: Delete announcement
...
BEGIN
  SELECT role INTO v_user_role FROM profiles WHERE auth_id = auth.uid()::text AND deleted_at IS NULL;
  IF v_user_role NOT IN ('creator', 'admin') THEN
    RAISE EXCEPTION 'Only creator or admin can delete announcements';
  END IF;
  UPDATE announcements SET is_active = false WHERE id = p_id;
  RETURN FOUND;
END;
$$;

-- Grant execute
GRANT EXECUTE ON FUNCTION get_announcements_for_role TO authenticated;
GRANT EXECUTE ON FUNCTION get_announcements_for_role TO anon;
...
GRANT EXECUTE ON FUNCTION has_creator_auth TO authenticated;

-- ═══════════════════════════════════════════════════════════════
-- 4. PLATFORM SETTINGS (if not already created)
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'platform_settings') THEN
    ...
    CREATE POLICY platform_settings_modify ON platform_settings FOR ALL
      USING (EXISTS (SELECT 1 FROM profiles WHERE auth_id = auth.uid()::text AND role IN ('creator', 'admin') AND deleted_at IS NULL));
  END IF;
END $$;

-- Insert settings if table is empty
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM platform_settings LIMIT 1) THEN
    ...
    -- COMMISSIONS (what WeHouse charges)
    ...
    ('worker_video_intro_required', 'true', 'worker', 'Video Intro Required', 'Require video introduction', 'boolean'),
    ...
    ('worker_status_flow', 'pending,approved_for_verification,verified', 'worker', 'Status Flow', 'Comma-separated worker status progression', 'string'),
    -- BOOKING
...
BEGIN SELECT ps.value INTO v_value FROM platform_settings ps WHERE ps.key = p_key; RETURN v_value; END;
$$;
...
BEGIN
  ...
  END IF;
  UPDATE platform_settings SET value = p_value, updated_at = NOW() WHERE key = p_key AND editable = true;
  RETURN FOUND;
END;
$$;
