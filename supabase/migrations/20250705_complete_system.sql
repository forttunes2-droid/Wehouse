-- ═══════════════════════════════════════════════════════════════
-- WEHOUSE: COMPLETE SYSTEM SQL (Fixes + Three-System Refactor)
-- Run this ENTIRE file in your Supabase SQL Editor
-- ═══════════════════════════════════════════════════════════════

-- ================================================================
-- SECTION 0: FIX EXISTING RPC FUNCTIONS (Creator can't see workers)
-- ================================================================

-- Drop and recreate admin_get_all_workers (the main fix for creator seeing workers)
...
-- Ensure worker status can be read by checking worker_status column exists
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'profiles' AND column_name = 'worker_status') THEN
    ALTER TABLE profiles ADD COLUMN worker_status TEXT DEFAULT 'pending';
  END IF;
END $$;

-- Update existing workers that have no status to be 'pending'
UPDATE profiles SET worker_status = 'pending' 
WHERE role = 'worker' AND (worker_status IS NULL OR worker_status = '');

-- ================================================================
-- SECTION 1: WORKER BOOKING NEGOTIATION WORKFLOW
-- ================================================================

-- 1a. Drop existing status constraint and add new columns
...
ALTER TABLE worker_bookings ADD CONSTRAINT worker_bookings_status_check 
  CHECK (status IN (
    ...
    'completed',             -- Step 7: Worker marked complete
    'completed_pending_approval', -- Step 7: Waiting customer to confirm
    'approved_released',     -- Payment released to worker
...
-- 1b. Create booking_conversations table
...
-- Enable RLS on booking_conversations
...
-- 1c. Create booking_messages table
...
-- ================================================================
-- SECTION 2: RPC FUNCTIONS FOR WORKER BOOKING NEGOTIATION
-- ================================================================

-- 2a. Create booking request (Step 1)
DROP FUNCTION IF EXISTS public.create_booking_request(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT);
...
BEGIN
  ...
  INSERT INTO worker_bookings (
    ...
    p_address, p_scheduled_date::DATE, 0, 300, 0, 0,
...
-- ================================================================
-- SECTION 3: FIELD OFFICER AUTO-LINK PROPERTY PARTNER
-- ================================================================

-- 3a. Ensure inspection_requests has the right columns
...
BEGIN
  ...
  IF (p_data->>'inspection_id') IS NOT NULL THEN
    ...
    IF FOUND AND v_inspection.owner_id IS NOT NULL THEN
      v_partner_id := v_inspection.owner_id;
    END IF;
  END IF;

  -- Fallback to explicit partner_id in data
  IF v_partner_id IS NULL AND (p_data->>'partner_id') IS NOT NULL THEN
    v_partner_id := p_data->>'partner_id';
  END IF;
  ...
  IF (p_data->>'inspection_id') IS NOT NULL THEN
    UPDATE inspection_requests SET listing_created = TRUE, listing_id = v_listing_id 
    WHERE id = (p_data->>'inspection_id')::UUID;
  END IF;
...
-- ================================================================
-- SECTION 4: PROPERTY PARTNER SUPPORT CONVERSATION SYSTEM
-- ================================================================
...
-- ================================================================
-- SECTION 5: RPC FUNCTIONS FOR PARTNER SUPPORT CONVERSATION
-- ================================================================

-- 5a. Create partner support conversation
...
-- 5c. Get staff support conversations
...
AS $$
  SELECT 
    psm.id,
    psm.sender_id,
    COALESCE(p.full_name, 'WeHouse') as sender_name,
    psm.sender_role,
    psm.content,
    psm.attachments,
...
-- 5f. Add system action to conversation
...
-- ================================================================
-- DONE! All functions and tables created.
-- ================================================================
