-- ═══════════════════════════════════════════════════════════════
-- WEHOUSE: RUN THIS ENTIRE FILE IN SUPABASE SQL EDITOR
-- Copy everything below, paste in Supabase → SQL Editor → New Query → Run
-- ═══════════════════════════════════════════════════════════════

-- ============================================================
-- PART 1: STORAGE BUCKETS (for photo upload)
-- ============================================================
...
-- Storage policies
DROP POLICY IF EXISTS "listing-videos-upload" ON storage.objects;
CREATE POLICY "listing-videos-upload" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'listing-videos');
DROP POLICY IF EXISTS "listing-videos-read" ON storage.objects;
CREATE POLICY "listing-videos-read" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'listing-videos');

DROP POLICY IF EXISTS "worker-files-upload" ON storage.objects;
CREATE POLICY "worker-files-upload" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'worker-files');
DROP POLICY IF EXISTS "worker-files-read" ON storage.objects;
CREATE POLICY "worker-files-read" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'worker-files');
...
-- ============================================================
-- PART 2: TABLES (booking + support conversations)
-- ============================================================

-- Worker bookings columns
...
-- ============================================================
-- PART 3: ALL ADMIN/CREATOR RPC FUNCTIONS
-- ============================================================

-- admin_get_all_users: Returns ALL users (bypasses RLS)
...
-- ============================================================
-- PART 4: ALL WORKER BOOKING RPC FUNCTIONS
-- ============================================================
...
-- create_partner_support_conversation
...
-- get_staff_support_conversations
...
-- ============================================================
-- PART 6: REMAINING HELPER FUNCTIONS
-- ============================================================
...
CREATE OR REPLACE FUNCTION public.set_creator_auth(p_user_id TEXT)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  -- Mark user as having completed creator auth setup
  UPDATE profiles SET creator_auth_enabled = true WHERE user_id = p_user_id;
END;
$$;

-- verify_creator_auth
DROP FUNCTION IF EXISTS public.verify_creator_auth(TEXT);
CREATE OR REPLACE FUNCTION public.verify_creator_auth(p_user_id TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  RETURN EXISTS(SELECT 1 FROM profiles WHERE user_id = p_user_id AND creator_auth_enabled = true);
END;
$$;

-- ============================================================
-- PART 7: EXPAND WORKER STATUS OPTIONS
-- ============================================================
...
-- ============================================================
-- DONE! All 49 RPC functions created.
-- ============================================================
