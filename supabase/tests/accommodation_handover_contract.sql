...
insert into public.booking_payments(
  ...
  status,purpose,paystack_reference,verified_amount,verified_at,
  verification_source,paid_at,webhook_processed,metadata
) values
  ('30000000-0000-0000-0000-000000000001','TEST-DIRECT','test-user','test-user',100,100,'NGN','paid','apartment_rent','TEST-DIRECT',100,now(),'webhook',now(),true,'{"reservation_id":"test-short"}'::jsonb),
  ('30000000-0000-0000-0000-000000000002','TEST-SHARED-1','test-user','test-user',50,50,'NGN','paid','shared_housing_share','TEST-SHARED-1',50,now(),'webhook',now(),true,'{"canonical_shared_payment_group_id":"40000000-0000-0000-0000-000000000001","canonical_shared_payment_member_id":"50000000-0000-0000-0000-000000000001"}'::jsonb),
  ('30000000-0000-0000-0000-000000000003','TEST-SHARED-2','test-peer','test-peer',50,50,'NGN','paid','shared_housing_share','TEST-SHARED-2',50,now(),'webhook',now(),true,'{"canonical_shared_payment_group_id":"40000000-0000-0000-0000-000000000001","canonical_shared_payment_member_id":"50000000-0000-0000-0000-000000000002"}'::jsonb);
...
begin
  decision:=public.accommodation_access_authorization(
    'test-short','test-user','short_let','ready_for_move_in','paid',now(),
  ...
  decision:=public.accommodation_access_authorization(
    'test-shared','test-user','short_let','reserved','paid',now(),
...
insert into accommodation_guard_probe(
  id,user_id,stay_type,status,requested_move_in_at,
  ...
  'test-short','test-user','short_let','ready_for_move_in',now(),
...
begin
  begin
    insert into accommodation_guard_probe(
      id,user_id,stay_type,status,canonical_state,checked_in_at,
      manual_payment_status,paid_at,rent_payment_status,rent_paid_at,
      rent_payment_reference,stay_rent_total
    ) values(
      'missing-protection','test-user','short_let','reserved','checked_in',now(),
  ...
  rejected:=false;
  begin
    insert into accommodation_guard_probe(id,user_id,stay_type,status,canonical_state)
    values('terminal-bypass','test-user','short_let','completed','completed');
  exception when others then
