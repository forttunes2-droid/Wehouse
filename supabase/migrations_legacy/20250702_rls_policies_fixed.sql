-- ============================================================
-- RLS POLICIES FOR CORE TABLES — FIXED VERSION
-- ============================================================
-- Fixes: auth.uid() returns UUID, profiles.user_id is TEXT.
-- Solution: Cast auth.uid() to text using ::text
--
  ...
  USING (
    EXISTS (
      ...
      WHERE conversations.id = messages.conversation_id
        AND (conversations.participant_a = auth.uid()::text OR conversations.participant_b = auth.uid()::text)
    )
    OR EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.user_id = auth.uid()::text
