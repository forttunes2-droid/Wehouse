...
-- The old Worker confirmation RPC still reads platform_settings. Keep that
-- compatibility value as a derived read model of the Creator registry so it
-- cannot become a competing commercial authority.
...
as $$
begin
  if new.policy_key='commission_worker' and new.scope_type='global'
    and new.scope_key='*' and new.status='active' then
    if (new.value->>'percent')::numeric<0
      or (new.value->>'percent')::numeric>50 then
      raise exception 'Invalid Creator Worker commission';
    end if;
    insert into public.platform_settings(
      key,value,category,label,description,data_type,editable,is_active,
      created_at,updated_at
    ) values(
      'worker_commission_rate',new.value->>'percent','payments',
      'Worker commission rate',
      'Compatibility read model. Creator policy registry is authoritative.',
...
drop trigger if exists sync_creator_commission_compatibility
  on public.creator_policy_versions;
create trigger sync_creator_commission_compatibility
after insert or update of status,value on public.creator_policy_versions
for each row execute function public.sync_creator_commission_compatibility();
...
select
  'worker_commission_rate',p.value->>'percent','payments',
  'Worker commission rate',
  'Compatibility read model. Creator policy registry is authoritative.',
  'number',false,true,now(),now()
...
revoke all on function public.sync_creator_commission_compatibility()
from public,anon,authenticated;
grant execute on function public.sync_creator_commission_compatibility()
to service_role;
...
set search_path to 'pg_catalog','public'
as $$
declare v_policy jsonb; v_amount numeric:=coalesce(new.security_deposit_amount,0);
begin
  ...
  select p.value into v_policy from public.creator_policy_versions p
  where p.policy_key='short_let_caution_cap'
    and p.scope_type='global' and p.scope_key='*' and p.status='active'
    and p.effective_from<=now()
    and (p.effective_until is null or p.effective_until>now())
  order by p.effective_from desc limit 1;
  if not coalesce((v_policy->>'enabled')::boolean,false)
    or (v_policy->>'maximum_amount') is null
    or v_amount>(v_policy->>'maximum_amount')::numeric then
    raise exception 'Short Let Caution exceeds the active Creator policy';
  end if;
  return new;
end
$$;

drop trigger if exists enforce_listing_caution_policy on public.listings;
create trigger enforce_listing_caution_policy
before insert or update of sub_type,security_deposit_amount on public.listings
for each row execute function public.enforce_listing_caution_policy();

revoke all on function public.enforce_listing_caution_policy()
from public,anon,authenticated;
grant execute on function public.enforce_listing_caution_policy()
to service_role;
...
where namespace.nspname='public'
  and procedure.proname in(
    ...
    'sync_creator_commission_compatibility',
    'enforce_listing_caution_policy',
