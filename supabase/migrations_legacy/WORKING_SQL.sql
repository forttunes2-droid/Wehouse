-- ═══════════════════════════════════════════════════════════════════
-- WEHOUSE WORKING SQL — No UNIQUE/REFERENCES constraints
-- ═══════════════════════════════════════════════════════════════════

-- Step 1: pgcrypto
CREATE EXTENSION IF NOT EXISTS pgcrypto;
SELECT crypt('test', gen_salt('bf')) AS test;

-- Step 2: Add columns to profiles (if missing)
...
-- Step 5: Delete wrong settings, insert correct ones
...
-- Step 7: Wallet balances (NO REFERENCES — simple TEXT key)
...
-- Step 12: Functions (no withdrawal fee)
...
CREATE OR REPLACE FUNCTION public.create_withdrawal_request(p_user_id TEXT, p_amount NUMERIC) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$ DECLARE v_balance NUMERIC; v_frozen BOOLEAN; v_min NUMERIC; v_bank_name TEXT; v_bank_code TEXT; v_acct TEXT; v_req_id UUID; BEGIN SELECT available_balance, frozen INTO v_balance, v_frozen FROM wallet_balances WHERE user_id = p_user_id; IF v_balance IS NULL THEN RETURN jsonb_build_object('success',FALSE,'error','No wallet found'); END IF; IF v_frozen THEN RETURN jsonb_build_object('success',FALSE,'error','Wallet is frozen'); END IF; SELECT COALESCE(NULLIF(value,''),'1000')::NUMERIC INTO v_min FROM platform_settings WHERE key = 'wallet_minimum_withdrawal'; IF p_amount < v_min THEN RETURN jsonb_build_object('success',FALSE,'error',format('Minimum withdrawal is N%s',v_min)); END IF; IF p_amount > v_balance THEN RETURN jsonb_build_object('success',FALSE,'error',format('Insufficient balance. Available: N%s',v_balance)); END IF; SELECT bank_name, bank_code, bank_account_number INTO v_bank_name, v_bank_code, v_acct FROM profiles WHERE user_id = p_user_id; IF COALESCE(v_bank_code,'') = '' OR COALESCE(v_acct,'') = '' THEN RETURN jsonb_build_object('success',FALSE,'error','Bank details not set. Add in profile settings.'); END IF; UPDATE wallet_balances SET available_balance = available_balance - p_amount, pending_balance = pending_balance + p_amount, updated_at = NOW() WHERE user_id = p_user_id; INSERT INTO withdrawal_requests (user_id, user_role, amount_requested, withdrawal_fee, amount_paid, bank_name, bank_code, bank_account_number, status, metadata) SELECT p_user_id, role, p_amount, 0, p_amount, v_bank_name, v_bank_code, v_acct, 'pending', jsonb_build_object('net_amount',p_amount,'fee',0) FROM profiles WHERE user_id = p_user_id RETURNING id INTO v_req_id; INSERT INTO wallet_transactions (user_id, transaction_type, amount, balance_after, reference_id, reference_type, description) VALUES (p_user_id, 'withdrawal_request', -p_amount, v_balance - p_amount, v_req_id::text, 'withdrawal', format('Withdrawal request: N%s',p_amount)); RETURN jsonb_build_object('success',TRUE,'request_id',v_req_id,'amount',p_amount,'fee',0,'net_amount',p_amount); END; $$;
