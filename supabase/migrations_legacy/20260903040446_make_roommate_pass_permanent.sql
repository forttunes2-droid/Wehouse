-- A pass is a final private discovery decision. It must not be recycled after refresh.
...
begin
  ...
  update public.roommate_search_results
  set status=case when p_status='viewed' then 'declined' else p_status end,updated_at=now()
  where id=p_match_id;
