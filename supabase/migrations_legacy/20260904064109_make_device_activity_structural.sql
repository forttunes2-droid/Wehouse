...
create or replace function public.sync_device_activity_status()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  update public.notifications
  set title = case new.trust_status
        when 'pending' then 'New device needs verification'
        when 'trusted' then 'New device verified'
        else 'Device sign-in blocked'
      end,
      destination_route = 'security',
      destination_params = coalesce(destination_params, '{}'::jsonb) || jsonb_build_object(
        'decision', case new.trust_status when 'pending' then 'pending' when 'trusted' then 'approved' else 'blocked' end
      ),
      read = case when new.trust_status = 'pending' then read else true end,
      read_at = case when new.trust_status = 'pending' then read_at else coalesce(read_at, now()) end
...
revoke all on function public.sync_device_activity_status() from public;
revoke all on function public.sync_device_activity_status() from anon;

drop trigger if exists sync_device_activity_status_after_update on public.user_sessions;
create trigger sync_device_activity_status_after_update
after update of trust_status on public.user_sessions
for each row
when (old.trust_status is distinct from new.trust_status)
execute function public.sync_device_activity_status();

update public.notifications as notification
set title = case session.trust_status
      when 'pending' then 'New device needs verification'
      when 'trusted' then 'New device verified'
      else 'Device sign-in blocked'
    end,
    destination_route = 'security',
    destination_params = coalesce(notification.destination_params, '{}'::jsonb) || jsonb_build_object(
      'decision', case session.trust_status when 'pending' then 'pending' when 'trusted' then 'approved' else 'blocked' end
    ),
    read = case when session.trust_status = 'pending' then notification.read else true end,
    read_at = case when session.trust_status = 'pending' then notification.read_at else coalesce(notification.read_at, now()) end
from public.user_sessions as session
where notification.type = 'new_device_login'
  and notification.source_id = session.id::text;
