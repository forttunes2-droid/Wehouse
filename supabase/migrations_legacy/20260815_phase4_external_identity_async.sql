...
BEGIN
  ...
  SET
    ...
    identity_reference = COALESCE(NULLIF(BTRIM(p_reference),''), identity_reference),
...
REVOKE ALL ON FUNCTION public.record_external_worker_identity_result_by_reference(text,text,text,text) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.record_external_worker_identity_result(text,text,text,text,text) TO service_role;
GRANT EXECUTE ON FUNCTION public.record_external_worker_identity_result_by_reference(text,text,text,text) TO service_role;
