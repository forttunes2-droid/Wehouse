...
alter table public.canonical_threads
  add column if not exists thread_key text;
update public.canonical_threads
set thread_key=thread_type||':'||subject_type||':'||subject_id
where thread_key is null;
alter table public.canonical_threads alter column thread_key set not null;
alter table public.canonical_threads
  drop constraint if exists canonical_threads_thread_type_subject_type_subject_id_key;
create unique index if not exists canonical_threads_type_key_unique
  on public.canonical_threads(thread_type,thread_key);
...
begin
  ...
  insert into public.canonical_threads(
    ...
    'case','operational_case:'||v_case_id,'operational_case',v_case_id::text,
    'open',now(),now()
  )
