...
const USER_PAGES=new Set<NavPage>(['home','explore','search','saved','roommate','activity','messages','chat','detail','worker_discovery','worker_categories','hotels','hotel_detail','hotel_booking','hotel_reservation','my_bookings','my_reservations']);
function isRestorable(p:string):p is NavPage{return RESTORABLE_PAGES.includes(p as NavPage)}
function roleRootFor(role:string):NavPage{return role==='creator'?'creator':role==='admin'?'admin':role==='staff'?'staff_dashboard':role==='worker'?'worker_dashboard':role==='property_partner'?'property_partner':'home'}
function normalizePageForRole(role:string,page:NavPage):NavPage{
 if(page==='privacy_policy'||page==='terms_of_service')return page;
 ...
 if(role==='user')return USER_PAGES.has(page)?page:'home';
 return 'home';
}
...
export default function App(){
 ...
 useEffect(()=>{if(!profile?.user_id||!isUserRole){setUnreadCount(0);return}const uid=profile.user_id;async function count(){const[{data},{count:official}]=await Promise.all([supabase.from('conversations').select('participant_a,unread_a,unread_b').or(`participant_a.eq.${uid},participant_b.eq.${uid}`),getUnreadAnnouncementCount(uid)]);let roommate=0;(data||[]).forEach((c:any)=>roommate+=Number(c.participant_a===uid?c.unread_a:c.unread_b)||0);setUnreadCount(roommate+Number(official||0))}void count();const chatChannel=supabase.channel(`app-unread-chat:${uid}`).on('postgres_changes',{event:'*',schema:'public',table:'conversations'},()=>void count()).subscribe();const officialChannel=supabase.channel(`app-unread-official:${uid}`).on('postgres_changes',{event:'*',schema:'public',table:'announcement_recipients',filter:`user_id=eq.${uid}`},()=>void count()).subscribe();return()=>{supabase.removeChannel(chatChannel);supabase.removeChannel(officialChannel)}},[profile?.user_id,isUserRole]);
