...
const GROUPS: Group[] = [
  ...
  },
  {
    ...
    description: 'Rules for the private WeHouse Worker verification journey.',
    note: 'Payment is only a verification fee. It never buys approval, public visibility or marketplace trust.',
    settings: [
      ...
      { key: 'worker_identity_retention_days', label: 'Private identity evidence retention (days)', description: 'How long approved/rejected private identity media should be retained before cleanup.', kind: 'number', defaultValue: '90', min: 1, max: 3650, step: 1, category: 'worker' },
    ],
  },
  {
    id: 'worker_trust',
    label: 'WeHouse Trusted',
    description: 'Marketplace trust is earned from real WeHouse performance after professional approval.',
    note: 'A Worker is first WeHouse Reviewed. WeHouse Trusted is earned later from completed jobs, rating, cancellation behaviour and dispute history.',
    settings: [
  ...
  },
  {
    ...
    description: 'High-level access switches.',
    settings: [
  ...
  },
  {
    ...
    description: 'Current Privacy Policy and Terms & Conditions shown to users.',
    settings: [
...
export default function CreatorSettingsTabV2({ profile }: { profile?: Profile }) {
  ...
  function stored(def: Def) {
    return rows.find((row) => row.key === def.key && row.is_active !== false)?.value ?? def.defaultValue;
  }
  function current(def: Def) {
    return drafts[def.key] !== undefined ? drafts[def.key] : stored(def);
  }
  ...
  function normalize(def: Def, raw: string) {
    ...
    if ((def.step ?? 1) >= 1 && !Number.isInteger(value)) return null;
    return String(value);
  }
  ...
  async function save(def: Def, raw: string) {
    ...
    if (value === null) {
      const range = def.min !== undefined || def.max !== undefined ? ` (${def.min ?? 0}–${def.max ?? '∞'})` : '';
      toast.error(`${def.label} has an invalid value${range}`);
      return false;
    }
    ...
    if (verifyError || !verified || String(verified.value) !== String(value)) {
  ...
  async function saveAll() {
    ...
    for (const def of changed) await save(def, drafts[def.key]);
  }
  ...
  async function saveAssistant() {
    ...
    toast.success('WeHouse Assistant connection saved');
  }
  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div><h2 className="text-base font-bold">Platform settings</h2><p className="mt-1 max-w-2xl text-[10px] leading-relaxed text-[#686C7E]">Product policy and global configuration. Operational queues stay out of Settings.</p></div>
        {changed > 0 && <button onClick={() => void saveAll()} disabled={Object.values(saving).some(Boolean)} className="min-h-10 rounded-xl bg-violet-500 px-4 text-[10px] font-semibold disabled:opacity-40">Save changes ({changed})</button>}
      </div>

      <section className="rounded-2xl border border-blue-500/15 bg-blue-500/[.035] p-4 sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div><div className="flex items-center gap-2"><h3 className="text-sm font-semibold">WeHouse Assistant</h3><Status good={assistantConnected}>{assistantConnected ? 'CONNECTED' : 'NOT CONNECTED'}</Status></div><p className="mt-1 text-[10px] text-[#687082]">Private server-side Assistant connection. Human Support remains separate.</p></div>
          {assistantConnected && <button onClick={() => void testAssistant()} disabled={assistantTesting} className="h-10 rounded-xl border border-blue-500/20 px-4 text-[10px] font-semibold text-blue-300 disabled:opacity-40">{assistantTesting ? 'Testing…' : 'Test connection'}</button>}
        </div>
        <div className="mt-3 flex flex-col gap-2 sm:flex-row"><input type="password" autoComplete="off" value={assistantKey} onChange={(event) => setAssistantKey(event.target.value)} placeholder={assistantConnected ? 'Paste a new key only to replace the current one' : 'sk-…'} className="h-11 min-w-0 flex-1 rounded-xl border border-white/[.08] bg-[#171A23] px-3 text-xs outline-none focus:border-blue-500/40" /><button onClick={() => void saveAssistant()} disabled={assistantBusy || !assistantKey.trim()} className="h-11 rounded-xl bg-blue-500 px-4 text-[10px] font-semibold disabled:opacity-40">{assistantBusy ? 'Saving…' : assistantConnected ? 'Replace key' : 'Connect Assistant'}</button></div>
      </section>

      <div className="grid gap-4 xl:grid-cols-2">
        {GROUPS.map((group) => <section key={group.id} className={`${group.id === 'legal' ? 'xl:col-span-2' : ''} rounded-2xl border border-white/[.06] bg-[#10131B] p-4 sm:p-5`}><div className="mb-4"><h3 className="text-sm font-semibold">{group.label}</h3><p className="mt-1 text-[10px] leading-relaxed text-[#666A7C]">{group.description}</p>{group.note && <p className="mt-3 rounded-xl border border-violet-500/10 bg-violet-500/[.035] p-3 text-[9px] leading-relaxed text-violet-100/70">{group.note}</p>}</div><div className="space-y-3">{group.settings.map((def) => <Setting key={def.key} def={def} value={current(def)} dirty={drafts[def.key] !== undefined} busy={saving[def.key]} setValue={(value) => setDraft(def.key, value)} save={() => void save(def, current(def))} />)}</div></section>)}
      </div>
    </section>
  );
}
...
function Status({ good, children }: { good: boolean; children: React.ReactNode }) {
  return <span className={`rounded-full px-2 py-1 text-[8px] font-semibold ${good ? 'bg-emerald-500/10 text-emerald-300' : 'bg-amber-500/10 text-amber-300'}`}>{children}</span>;
}
