...
export default function PasswordRecoveryStable(){
 ...
 return <div className="flex min-h-screen items-center justify-center bg-transparent px-5 text-white"><div className="w-full max-w-[390px]">
  ...
  <div className="rounded-3xl border border-white/[.07] bg-[#10131A] p-5 shadow-2xl shadow-black/20">
   <p className="text-[9px] font-bold uppercase tracking-[.18em] text-violet-400">ACCOUNT RECOVERY</p><h1 className="mt-1 text-xl font-bold">Choose a new password</h1><p className="mt-1 text-[10px] leading-5 text-[#747A8C]">WeHouse verifies the secure email link before allowing a password change.</p>

   {stage==='preparing'&&<div className="mt-5 rounded-2xl border border-violet-500/15 bg-violet-500/[.06] p-4"><p className="text-xs font-semibold text-violet-200">Preparing secure reset…</p><p className="mt-1 text-[9px] leading-4 text-[#838A9B]">This should finish automatically. It will not stay on this screen indefinitely.</p></div>}

   {(stage==='ready'||stage==='done')&&<form onSubmit={updatePassword} className="mt-5 space-y-4">{error&&<Notice tone="error">{error}</Notice>}{info&&<Notice>{info}</Notice>}<PasswordField label="New password" value={password} set={setPassword} visible={show} toggle={()=>setShow(v=>!v)}/><PasswordField label="Confirm new password" value={confirm} set={setConfirm} visible={show} toggle={()=>setShow(v=>!v)}/><button type="submit" disabled={busy||stage==='done'} className="h-12 w-full rounded-xl bg-violet-500 text-sm font-semibold disabled:opacity-40">{busy?'Updating…':stage==='done'?'Password updated':'Update password'}</button></form>}

   {(stage==='failed'||stage==='sent')&&<form onSubmit={sendNewLink} className="mt-5 space-y-4">{error&&<Notice tone="error">{error}</Notice>}{info&&<Notice>{info}</Notice>}<label className="block"><span className="mb-1.5 block text-[11px] font-medium text-[#8B90A0]">Account email</span><Input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" required className="h-12 rounded-xl border-white/[.08] bg-[#171A23] text-white"/></label><button type="submit" disabled={busy} className="h-12 w-full rounded-xl bg-violet-500 text-sm font-semibold disabled:opacity-40">{busy?'Sending…':'Send a new reset link'}</button></form>}
...
function Brand(){return <div className="mb-7 text-center"><div className="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-2xl bg-gradient-to-br from-violet-500 to-violet-600 shadow-lg shadow-violet-500/15"><svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><path d="M9 22V12h6v10"/></svg></div><h2 className="text-2xl font-bold">WeHouse</h2><p className="mt-1 text-xs text-[#5C6172]">We make living easy</p></div>}
...
function Notice({children,tone='info'}:{children:React.ReactNode;tone?:'info'|'error'}){const cls=tone==='error'?'border-red-500/20 bg-red-500/10 text-red-300':'border-violet-500/20 bg-violet-500/10 text-violet-300';return <div className={`rounded-xl border p-3 text-xs leading-relaxed ${cls}`}>{children}</div>}
