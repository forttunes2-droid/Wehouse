...
export default function WorkerSetupProfessional({ profile, onComplete }: Props) {
  ...
  async function save(event: React.FormEvent) {
    ...
    // update_my_profile is the canonical server boundary: for Worker accounts it
    // also creates/updates worker_service_coverage from this State/LGA/area.
    ...
    <div className="min-h-[100dvh] bg-[#090B11] text-white">
      ...
      <main className="mx-auto max-w-2xl space-y-5 px-4 py-6">
        <div>
          ...
          <p className="mt-2 text-[11px] leading-relaxed text-[#747A8B]">This is your professional identity and service coverage. Payment, the Worker test, professional evidence and external identity happen afterward in Verification.</p>
        </div>

        <section className="rounded-2xl border border-white/[.07] bg-[#11151D] p-4">
          <div className="flex items-center gap-4">
            <button type="button" onClick={() => photoRef.current?.click()} className="grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-2xl bg-cyan-500 text-xl font-bold">
              {avatar ? <img src={avatar} alt="" className="h-full w-full object-cover" /> : (name || 'W')[0]}
            </button>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            ...
            <Field label="Starting price" value={price} set={(value) => setPrice(value.replace(/[^0-9]/g, ''))} />
          </div>
        </section>

        <section className="rounded-2xl border border-white/[.07] bg-[#11151D] p-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <Select label="Service category" value={category} set={(value) => { setCategory(value); setSpecialty(''); }} options={categories.map((item) => [item.id, item.name])} />
            <Select label="Specialty" value={specialty} set={setSpecialty} options={subs.map((item) => [item.name, item.name])} />
          </div>
          <label className="mt-3 block">
            <span className="mb-1 block text-[9px] uppercase text-[#666D7E]">Professional experience</span>
            <textarea value={experience} onChange={(event) => setExperience(event.target.value)} rows={3} placeholder="Example: 4 years installing and repairing household electrical systems" className="w-full rounded-xl border border-white/[.08] bg-[#171B24] p-3 text-xs outline-none" />
          </label>
        ...
        <section className="rounded-2xl border border-white/[.07] bg-[#11151D] p-4">
          <h2 className="text-sm font-semibold">Service coverage</h2>
          <p className="mt-1 text-[10px] leading-relaxed text-[#747A8B]">Your State and LGA decide where customers can discover you. The server saves this as your Worker service coverage.</p>
          <div className="mt-3"><LocationSelector value={location} onChange={setLocation} /></div>
        </section>

        {profile.worker_status === 'verified' && (
          <p className="rounded-xl border border-amber-500/15 bg-amber-500/[.05] p-3 text-[10px] text-amber-200">Major professional changes to a live Worker may require a new review before they are reflected publicly.</p>
        )}

        <button onClick={save as any} disabled={busy} className="h-12 w-full rounded-xl bg-cyan-500 text-sm font-semibold text-[#051018] disabled:opacity-40">
