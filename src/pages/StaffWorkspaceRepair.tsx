...
function Workspace({module,profile,onLogout,onNavigate}:{module:Module;profile:Profile;onLogout:()=>void;onNavigate?:((page:string)=>void)}){const first=module==='verification'||module==='finance'?'overview':'work';const[tab,setTab]=useState(first);const scope={state:profile.assigned_state||profile.state||'',lga:profile.assigned_lga||profile.local_government||profile.city||''};const config=module==='operations'?{title:'Property Operations',items:[{id:'work',label:'Properties'}]}:module==='support'?{title:'Support',items:[{id:'work',label:'Inbox'}]}:module==='field_officer'?{title:'Field Operations',items:[{id:'work',label:'Inspections'}]}:module==='finance'?{title:'Finance',items:[{id:'overview',label:'Overview'},{id:'payments',label:'Payments'},{id:'payouts',label:'Payouts'},{id:'ledger',label:'Ledger & Audit'}]}:{title:'Security & Verification',items:[{id:'overview',label:'Overview'},{id:'reviews',label:'Reviews'}]};let content:React.ReactNode;if(module==='operations')content=<PropertyPipelineWorkspace profile={profile}/>;else if(module==='support')content=<CommunicationsWorkspace profile={profile} scope={scope} forcedView="inbox" hideViewTabs/>;else if(module==='field_officer')content=<StaffInspectionWorkspaceV2 profile={profile}/>;else if(module==='finance')content=tab==='overview'?<StaffFinanceSummary open={setTab as any}/>:<StaffFinanceRecords view={tab as 'payments'|'payouts'|'ledger'}/>;else content=tab==='reviews'?<StaffVerificationQueueV2/>:<StaffSecurityOverviewV2 onOpenReviews={()=>setTab('reviews')} onOpenCases={()=>setTab('reviews')}/>;return <><Toaster position="top-center" richColors/><WorkspaceFrameV2 label={`WEHOUSE STAFF · ${scope.lga||'BRANCH'}`} title={config.title} items={config.items} active={tab} setActive={setTab} onAccount={onNavigate?()=>onNavigate('profile'):undefined} onLogout={onLogout}>{content}</WorkspaceFrameV2></>}
