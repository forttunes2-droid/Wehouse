...
function Workspace({module,profile,onLogout,onNavigate}:{module:Exclude<Module,'finance'>;profile:Profile;onLogout:()=>void;onNavigate?:((page:string)=>void)}){const[tab,setTab]=useState('work');const scope={state:profile.assigned_state||profile.state||'',lga:profile.assigned_lga||profile.local_government||profile.city||''};const config=module==='operations'?{title:'Property Operations',items:[{id:'work',label:'Properties'}]}:module==='support'?{title:'Support',items:[{id:'work',label:'Inbox'}]}:module==='field_officer'?{title:'Field Operations',items:[{id:'work',label:'Inspections'}]}:{title:'Security & Verification',items:[{id:'overview',label:'Overview'},{id:'reviews',label:'Reviews'}]};if(module==='verification'&&tab==='work')setTab('overview');const content=module==='operations'?<PropertyPipelineWorkspace profile={profile}/>:module==='support'?<CommunicationsWorkspace profile={profile} scope={scope} forcedView="inbox" hideViewTabs/>:module==='field_officer'?<StaffInspectionWorkspaceV2 profile={profile}/>:tab==='reviews'?<StaffVerificationQueueV2/>:<StaffSecurityOverviewV2 onOpenReviews={()=>setTab('reviews')} onOpenCases={()=>setTab('reviews')}/>;return <><Toaster position="top-center" richColors/><WorkspaceFrameV2 label={`WEHOUSE STAFF · ${scope.lga||'BRANCH'}`} title={config.title} items={config.items} active={tab} setActive={setTab} onAccount={onNavigate?()=>onNavigate('profile'):undefined} onLogout={onLogout}>{content}</WorkspaceFrameV2></>}
