...
export default function StaffWorkspaceRepair({ profile, onLogout, onNavigate }: Props) {
  ...
  if (!profile.assigned_state || !profile.assigned_lga) {
    return <State title="Branch assignment required" text="An Admin or Creator must assign this Staff account to a State and LGA before operational work can begin." />;
  }
  if (assigned.length !== 1) {
    return (
      <State
        title="Staff assignment needs attention"
        text={assigned.length ? 'This Staff account has conflicting module assignments. Keep one active operational module.' : 'No Staff module is assigned to this account.'}
      />
    );
  }
...
function Workspace({ module, profile, onLogout, onNavigate }: { module: Module; profile: Profile; onLogout: () => void; onNavigate?: (page: string) => void }) {
  ...
  const scope = { state: profile.assigned_state || '', lga: profile.assigned_lga || '' };

  const config = module === 'operations'
    ? { title:'Property Operations', description:'Manage property preparation and live housing activity for your assigned branch.', items:[{id:'pipeline',label:'Pipeline'},{id:'housing',label:'Live Housing'}] }
    : module === 'support'
      ? { title:'Support', description:'Handle real support conversations for your assigned branch.', items:[{id:'work',label:'Inbox'}] }
      : module === 'field_officer'
        ? { title:'Field Operations', description:'Carry out and record assigned property inspections.', items:[{id:'work',label:'Inspections'}] }
        : module === 'finance'
          ? { title:'Finance', description:'Review payments, payouts and branch financial records.', items:[{id:'overview',label:'Overview'},{id:'payments',label:'Payments'},{id:'payouts',label:'Payouts'},{id:'ledger',label:'Ledger & Audit'}] }
          : { title:'Worker Verification', description:'Review Local Service Worker identity freshness and real professional work evidence before activation.', items:[{id:'reviews',label:'Worker Reviews'}] };

  let content: React.ReactNode;
  if (module === 'operations') content = tab === 'housing' ? <HousingOperationsWorkspace /> : <PropertyPipelineWorkspace profile={profile} />;
  else if (module === 'support') content = <CommunicationsWorkspace profile={profile} scope={scope} forcedView="inbox" hideViewTabs />;
  else if (module === 'field_officer') content = <StaffInspectionWorkspaceV2 profile={profile} />;
  else if (module === 'finance') content = tab === 'overview' ? <StaffFinanceSummary open={setTab as any} /> : <StaffFinanceRecords view={tab as 'payments'|'payouts'|'ledger'} />;
  else content = <StaffVerificationQueueV2 />;
