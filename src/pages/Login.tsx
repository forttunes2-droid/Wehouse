...
import { signUpWithEmail, signInWithEmail, signInWithGoogle, signInWithPhone, verifyPhoneOtp, resetPassword, runDiagnostics } from '@/lib/supabase';
...
type Mode = 'choose' | 'choose_role' | 'signin' | 'signup' | 'forgot' | 'phone' | 'phone_verify';
...
export default function Login({ onLoginSuccess, serverError }: LoginProps) {
  ...
  async function handleSendOtp(e: React.FormEvent) {
    ...
    if (formattedPhone.length < 13) {
      setError('Phone number too short. Use format: 08012345678');
      return;
    }

    setWorking(true);
    const { error: err } = await signInWithPhone(formattedPhone);
    if (err) {
      setError(friendlyError(err.message));
      setWorking(false);
    } else {
      setPhone(formattedPhone); // Store formatted
      setMode('phone_verify');
      setInfo(`OTP sent to ${formattedPhone}`);
      setWorking(false);
    }
  }
  ...
  async function handleVerifyOtp(e: React.FormEvent) {
    ...
    if (err) {
      ...
      onLoginSuccess(data.session.user.id, data.session.user.phone || phone);
    } else {
      ...
      <div className="w-full max-w-[360px]">
        ...
        {mode === 'choose' && (
          <div className="space-y-3">
            ...
            {/* Phone */}
            <button onClick={() => { setMode('phone'); setError(''); setInfo(''); }} disabled={working} className="w-full h-12 rounded-xl bg-[#1A1A24] text-white font-medium text-sm border border-[#232330] hover:border-emerald-500/50 hover:bg-[#1E1E2C] transition-all flex items-center justify-center gap-2 disabled:opacity-50">
        ...
        {mode === 'phone' && (
          <form onSubmit={handleSendOtp} className="space-y-4">
            <div>
              <label className="text-[11px] text-[#8B8DA0] mb-1.5 block font-medium">Phone Number</label>
              <Input
                type="tel"
                value={phone}
                ...
                required
                className="h-12 rounded-xl bg-[#1A1A24] border-[#232330] text-white placeholder:text-[#5C5E72] focus:border-emerald-500 focus:ring-emerald-500/20"
              />
              <p className="text-[9px] text-[#5C5E72] mt-1.5">Enter your Nigerian phone number starting with 0</p>
            </div>
            <button type="submit" disabled={working} className="w-full h-12 rounded-xl bg-emerald-500 text-white font-medium text-sm hover:bg-emerald-600 transition-colors disabled:opacity-50">
        ...
        {mode === 'phone_verify' && (
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            <div>
              <label className="text-[11px] text-[#8B8DA0] mb-1.5 block font-medium">Enter OTP Code</label>
              <Input
                type="text"
                value={otp}
                ...
                required
                maxLength={6}
                className="h-12 rounded-xl bg-[#1A1A24] border-[#232330] text-white placeholder:text-[#5C5E72] focus:border-emerald-500 focus:ring-emerald-500/20 text-center text-lg tracking-[0.5em]"
              />
              <p className="text-[9px] text-[#5C5E72] mt-1.5">Code sent to {phone}</p>
            </div>
            <button type="submit" disabled={working} className="w-full h-12 rounded-xl bg-emerald-500 text-white font-medium text-sm hover:bg-emerald-600 transition-colors disabled:opacity-50">
              {working ? 'Verifying...' : 'Verify & Sign In'}
            </button>
            <div className="flex gap-2">
              <button type="button" onClick={() => { setMode('phone'); setOtp(''); setError(''); }} className="flex-1 text-center text-xs text-[#3B82F6] hover:text-[#3B82F6]/80 transition-colors">
                Resend OTP
              </button>
              <button type="button" onClick={() => { setMode('choose'); setOtp(''); setError(''); setInfo(''); }} className="flex-1 text-center text-xs text-[#5C5E72] hover:text-[#8B8DA0] transition-colors">
                Cancel
              </button>
            </div>
          </form>
