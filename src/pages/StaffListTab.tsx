...
<div className="mt-2 flex gap-2 overflow-x-auto">{creator&&<Picker label={role==='all'?'All roles':role==='admin'?'Admins':'Operations'} value={role} options={[['all','All roles'],['admin','Admins'],['staff','Operations members']]} onChange={value=>setRole(value as RoleFilter)}/>} {creator&&<Picker label={state||'All states'} value={state} options={[['','All states'],...NIGERIA_STATES.map(x=>[x.state,x.state] as [string,string])]} onChange={value=>{setState(value);setLga('')}}/>} {creator&&<Picker label={lga||'All LGAs'} value={lga} disabled={!state} options={[['','All LGAs'],...(stateData?.cities||[]).map(x=>[x,x] as [string,string])]} onChange={setLga}/>}</div>
</section>
...
<p className="text-[9px] text-violet-300">{person.role==='admin'?'Admin':'Operations member'}</p>
<p className="mt-1 text-[8px] text-[#666D7E]">{person.role==='staff'?(MODULES[assigned[person.user_id]]||'No work area assigned'):'Branch administration'}</p>
</div>
</button>)}</div>}
...
<p className="mt-1 text-[9px] text-[#666D7E]">Each Operations member works in one area.</p>
<div className="mt-3"><WeHouseSelect value={module} disabled={saving} onChange={value=>void saveModule(person,value)} options={[{value:'',label:'No work area assigned'},...Object.entries(MODULES).map(([value,label])=>({value,label}))]} title="Choose work area" ariaLabel="Choose work area" className="h-11 w-full"/></div>
</section>}</div>
</aside>
</div>}
