    ...
    <div className="space-y-5">
      ...
      {tab === 'trends' ? (
        <div className="space-y-4">
          ...
          <div className="grid gap-2 sm:grid-cols-2"><Money label="Verified payment volume" value={summary.verified_payment_volume} /><Money label="Commission recorded" value={summary.commission_earned} /></div>
...
function Metric({label,value}:{label:string;value:any}){return <div className="rounded-2xl border border-white/[.06] bg-[#10131B] p-4"><p className="text-xl font-bold">{Number(value||0).toLocaleString()}</p><p className="mt-1 text-[9px] text-[#717787]">{label}</p></div>}
function Money({label,value}:{label:string;value:any}){return <div className="rounded-2xl border border-emerald-500/10 bg-emerald-500/[.03] p-4"><p className="text-xl font-bold text-emerald-300">₦{Number(value||0).toLocaleString('en-NG')}</p><p className="mt-1 text-[9px] text-[#717787]">{label}</p></div>}
