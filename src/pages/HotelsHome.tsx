...
import { followPropertySearch, getMySavedSearches, removeSavedSearch, savedSearchKey, type SavedSearch } from "@/lib/supabase/saved-searches";
...
export default function HotelsHome({ onNavigate }: Props) {
  ...
  useEffect(() => {
    let live = true;
    void (async () => {
      const [{ hotels: rows }, followed, saved] = await Promise.all([getHotels(), getMySavedSearches(), getMySavedHotelIds()]);
      if (!live) return;
  ...
  const priceScale = useMemo(() => {
    const prices = hotels.flatMap((hotel) => (hotel.hotel_rooms || []).map((room) => Number(room.price_per_night || 0))).filter((price) => Number.isFinite(price) && price > 0);
    if (!prices.length) return { floor: HOTEL_PRICE_FLOOR, ceiling: HOTEL_PRICE_CEILING };
    const observedMin = Math.min(...prices), observedMax = Math.max(...prices);
    return {
      floor: Math.min(HOTEL_PRICE_FLOOR, Math.max(HOTEL_PRICE_STEP, Math.floor(observedMin / HOTEL_PRICE_STEP) * HOTEL_PRICE_STEP)),
      ceiling: Math.max(HOTEL_PRICE_CEILING, Math.ceil(observedMax / HOTEL_PRICE_STEP) * HOTEL_PRICE_STEP),
    };
  }, [hotels]);

  const filtered = useMemo(() => hotels
    .map((hotel) => { const c = coords(hotel); return { hotel, distance: userLocation && c ? distanceBetweenKm(userLocation, c) : null }; })
    .filter(({ hotel, distance }) => {
      ...
      if (minPrice !== "" || maxPrice !== "") {
        ...
        if (!prices.some((price) => (minPrice === "" || price >= minPrice) && (maxPrice === "" || price <= maxPrice))) return false;
      }
      if (radius && (distance == null || distance > radius)) return false;
      return true;
    })
    .sort((a, b) => userLocation ? (a.distance ?? Infinity) - (b.distance ?? Infinity) : Number(Boolean(b.hotel.featured)) - Number(Boolean(a.hotel.featured))),
  ...
  const currentSearchCriteria = useMemo(() => ({
    query: query.trim(), state, city,
    min_price: minPrice === "" ? null : minPrice,
    max_price: maxPrice === "" ? null : maxPrice,
    amenities, radius_km: radius === "" ? null : radius,
  ...
  function clearFilters() { setQuery(""); setState(""); setCity(""); setAmenities([]); setMinPrice(""); setMaxPrice(""); setRadius(""); }
  function clearStructuredFilters() { setState(""); setCity(""); setAmenities([]); setMinPrice(""); setMaxPrice(""); setRadius(""); }
  ...
  async function toggleHotelSave(hotelId: number) {
    ...
    setSavedHotelIds((current) => { const next = new Set(current); if (saved) next.delete(hotelId); else next.add(hotelId); return next; });
  ...
  return (
    <DiscoveryShell active="hotels" onNavigate={onNavigate}>
      <main className="mx-auto max-w-7xl space-y-4 px-4 py-5 sm:px-6 lg:px-8">
        <DiscoveryToolbar value={query} onChange={setQuery} placeholder="Search hotel name" onFilters={() => setFiltersOpen(true)} filterCount={filterCount} locationDetail={locationError || undefined} locationLabel={userLocation ? "Using current location" : "Use my location"} locationActive={Boolean(userLocation)} locationBusy={locating} onLocation={requestLocation} onClearLocation={clearLocation} />
        <div className="flex items-center justify-between gap-3">
          <div><p className="text-[11px] font-semibold">{loading ? "Loading hotels…" : `${filtered.length} ${filtered.length === 1 ? "hotel" : "hotels"}`}</p><p className="mt-1 text-[9px] text-[#666D7E]">{city ? `${city}, ${state}` : state || "All locations"}</p></div>
          <div className="flex items-center gap-3">
            {Boolean(query || filterCount) && <button type="button" disabled={savingSearch} onClick={() => void toggleFollowSearch()} className={`rounded-full border px-3 py-2 text-[9px] font-semibold disabled:opacity-40 ${followedSearch?.notifications_enabled ? "border-emerald-500/25 text-emerald-300" : "border-violet-500/20 text-violet-300"}`}>{savingSearch ? "Updating…" : followedSearch?.notifications_enabled ? "Following" : followedSearch ? "Resume alerts" : "Follow search"}</button>}
        ...
        {loading ? <div className="grid min-h-56 place-items-center"><div className="h-7 w-7 animate-spin rounded-full border-2 border-violet-500 border-t-transparent" /></div> : filtered.length === 0 ? <DiscoveryEmpty title={priceActive ? "No hotels match this nightly price range" : "No hotels match these filters"} text="Change the selected filters to see other hotels." /> : (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map(({ hotel, distance }) => <HotelCard key={hotel.hotel_id} hotel={hotel} distance={distance} saved={savedHotelIds.has(Number(hotel.hotel_id))} saving={savingHotelId === Number(hotel.hotel_id)} onToggleSave={() => void toggleHotelSave(Number(hotel.hotel_id))} onOpen={() => onNavigate("hotel_detail", String(hotel.hotel_id))} />)}
          </div>
        )}
      </main>
      ...
      {filtersOpen && <DiscoveryFilterSheet title="Find a hotel" onClose={() => setFiltersOpen(false)} onClear={clearStructuredFilters}>
        ...
        <DiscoveryPriceRangeSlider label="Nightly price" floor={priceScale.floor} ceiling={priceScale.ceiling} step={HOTEL_PRICE_STEP} minValue={minPrice} maxValue={maxPrice} onMinChange={setMinPrice} onMaxChange={setMaxPrice} />
        ...
        {userLocation && <SearchableSelect label="Distance from your current location" value={radius === "" ? "" : String(radius)} onChange={(value) => setRadius(value ? Number(value) : "")} options={[{ value: "", label: "Any distance" },{ value: "2", label: "Within 2 km" },{ value: "5", label: "Within 5 km" },{ value: "10", label: "Within 10 km" },{ value: "20", label: "Within 20 km" }]} placeholder="Any distance" searchPlaceholder="Search distance" />}
      </DiscoveryFilterSheet>}
    </DiscoveryShell>
  );
}

function HotelCard({ hotel, distance, saved, saving, onOpen, onToggleSave }: { hotel: HotelRow; distance: number | null; saved: boolean; saving: boolean; onOpen: () => void; onToggleSave: () => void }) {
  ...
  return (
    <article className="group border-b border-white/[.07] pb-5">
      <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-[#171B24]">
        ...
        <button type="button" disabled={saving} onClick={onToggleSave} aria-label={saved ? `Remove ${hotel.name} from Saved` : `Save ${hotel.name}`} aria-pressed={saved} className="absolute right-3 top-3 z-10 grid h-10 w-10 place-items-center rounded-full bg-black/55 text-white backdrop-blur disabled:opacity-50"><Heart filled={saved} /></button>
      </div>
      <button type="button" onClick={onOpen} className="block w-full px-1 pt-3 text-left">
        <div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="line-clamp-2 text-[15px] font-bold">{hotel.name}</h2><p className="mt-1 truncate text-[9px] text-[#6F7585]">{[hotel.area, hotel.city, hotel.state].filter(Boolean).join(", ")}</p></div>{Number(hotel.rating || 0) > 0 && <span className="shrink-0 text-[9px] font-semibold text-amber-300">★ {Number(hotel.rating).toFixed(1)}</span>}</div>
        {minPrice > 0 && <p className="mt-3 text-sm font-bold">₦{minPrice.toLocaleString()} <span className="text-[8px] font-medium text-[#747A89]">/ night</span></p>}
