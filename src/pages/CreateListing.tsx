...
export default function CreateListing({ profile, onBack, onSuccess }: CreateListingProps) {
  ...
  function addImages(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    setImages(current => {
      ...
      if (accepted.length > remaining) toast.error(`Maximum ${MAX_IMAGES} images`);
  ...
  function validateForReview() {
    if (!form.title.trim() || Number(form.price) <= 0 || !location.state || !location.city) { toast.error('Title, valid price, State and LGA are required'); return false; }
    if (!images.length) { toast.error('Add at least one clear property image'); return false; }
    return true;
  }
  ...
  async function submit() {
    if (!validateForReview() || !reviewing) return;
  ...
  const allMedia=[...images.map(item=>({...item,kind:'image' as const})),...videos.map(item=>({...item,kind:'video' as const}))];
  if(reviewing){const selected=allMedia[activeMedia]||allMedia[0];return <div className="min-h-[100dvh] bg-[#090A0F] pb-28 text-white"><Toaster position="top-center" richColors/><header className="sticky top-0 z-20 flex items-center gap-3 border-b border-white/[.06] bg-[#090A0F]/95 px-4 py-3 backdrop-blur-xl"><button type="button" onClick={()=>setReviewing(false)} disabled={saving} className="grid h-10 w-10 place-items-center rounded-full bg-white/[.05] disabled:opacity-40">←</button><div className="min-w-0"><p className="text-[9px] font-semibold uppercase tracking-[.16em] text-violet-300">FINAL REVIEW</p><h1 className="truncate text-base font-bold">Preview before {profile.role==='creator'?'publishing':'submitting'}</h1></div></header><main className="mx-auto max-w-4xl"><section className="bg-black">{selected?.kind==='video'?<video src={selected.preview} controls playsInline className="aspect-[4/3] max-h-[68vh] w-full object-contain sm:aspect-video"/>:<img src={selected?.preview} alt="Selected listing preview" className="aspect-[4/3] max-h-[68vh] w-full object-cover sm:aspect-video"/>}</section>{allMedia.length>1&&<div className="flex gap-2 overflow-x-auto border-b border-white/[.06] px-4 py-3">{allMedia.map((item,index)=><button type="button" key={item.preview} onClick={()=>setActiveMedia(index)} className={`relative h-16 w-20 shrink-0 overflow-hidden rounded-lg border ${index===activeMedia?'border-violet-400':'border-white/[.08]'}`}>{item.kind==='video'?<video src={item.preview} muted playsInline className="h-full w-full object-cover"/>:<img src={item.preview} alt={`Media ${index+1}`} className="h-full w-full object-cover"/>}{item.kind==='video'&&<span className="absolute inset-0 grid place-items-center">▶</span>}</button>)}</div>}<section className="space-y-5 px-4 py-5"><div><div className="flex flex-wrap items-center gap-2 text-[9px] text-[#858B9C]"><span>{form.property_type==='hotel'?'Hotel':form.sub_type==='short_let'?'Short Stay':'Long Stay'}</span><span>·</span><span>{images.length} photo{images.length===1?'':'s'}</span>{videos.length>0&&<><span>·</span><span>{videos.length} video{videos.length===1?'':'s'}</span></>}</div><h2 className="mt-2 text-2xl font-bold tracking-tight">{form.title.trim()}</h2><p className="mt-2 text-lg font-bold text-violet-300">₦{Number(form.price).toLocaleString('en-NG')} <span className="text-[10px] font-normal text-[#747A8B]">{form.sub_type==='short_let'?'/ night':'/ year'}</span></p><p className="mt-2 text-[11px] text-[#83899A]">{form.address.trim()||location.area||'Address not supplied'} · {location.city}, {location.state}</p></div><div className="grid grid-cols-2 gap-x-6 gap-y-3 border-y border-white/[.06] py-4 text-xs"><ReviewFact label="Bedrooms" value={form.bedrooms}/><ReviewFact label="Bathrooms" value={form.bathrooms}/><ReviewFact label="Owner" value={assignedPartnerId?(partners.find(p=>p.user_id===assignedPartnerId)?.full_name||partners.find(p=>p.user_id===assignedPartnerId)?.username||'Property Partner'):'WeHouse / internal'}/><ReviewFact label="Contact" value={form.contact_phone||'Not supplied'}/></div>{form.description.trim()&&<div><p className="text-[9px] uppercase tracking-[.15em] text-[#63697A]">Description</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-[#B1B5C1]">{form.description.trim()}</p></div>}{form.amenities.length>0&&<div className="flex flex-wrap gap-2">{form.amenities.map(item=><span key={item} className="rounded-full border border-white/[.08] px-3 py-1.5 text-[10px] text-[#ADB1BF]">{item}</span>)}</div>}<div className="rounded-xl border border-amber-500/15 bg-amber-500/[.05] px-3 py-2.5 text-[10px] leading-5 text-amber-100/80">Check every detail and open every photo or video. {profile.role==='creator'?'Confirming publishes this listing immediately.':'Confirming submits this listing into its approval workflow.'}</div><div className="grid grid-cols-2 gap-3"><button type="button" onClick={()=>setReviewing(false)} disabled={saving} className="h-12 rounded-xl border border-white/[.1] text-sm font-semibold disabled:opacity-40">Edit listing</button><button type="button" onClick={()=>void submit()} disabled={saving} className="h-12 rounded-xl bg-violet-500 text-sm font-bold disabled:opacity-40">{saving?'Uploading…':profile.role==='creator'?'Confirm & publish':'Confirm & submit'}</button></div></section></main></div>}
  ...
  <form onSubmit={openReview} className="mx-auto max-w-3xl space-y-5 px-4 py-6">
   ...
   <Section title="Media"><div className="grid grid-cols-2 gap-2"><button type="button" onClick={()=>imageInput.current?.click()} className="h-11 rounded-xl border border-white/[.08] bg-white/[.035] text-xs font-semibold">Add images · {images.length}/{MAX_IMAGES}</button><button type="button" onClick={()=>videoInput.current?.click()} className="h-11 rounded-xl border border-white/[.08] bg-white/[.035] text-xs font-semibold">Add videos · {videos.length}/{MAX_VIDEOS}</button></div><input ref={imageInput} hidden multiple type="file" accept="image/jpeg,image/png,image/webp" onChange={addImages}/><input ref={videoInput} hidden multiple type="file" accept="video/mp4,video/quicktime,video/webm" onChange={addVideos}/><div className="grid grid-cols-3 gap-2">{images.map((item,index)=><button type="button" key={item.preview} onClick={()=>removeImage(index)} className="relative aspect-square overflow-hidden rounded-xl"><img src={item.preview} className="h-full w-full object-cover" alt={`Property preview ${index+1}`}/><span className="absolute right-1 top-1 rounded-full bg-black/70 px-2 text-white">×</span></button>)}</div>{videos.length>0&&<div className="grid gap-2 sm:grid-cols-2">{videos.map((item,index)=><div key={item.preview} className="relative overflow-hidden rounded-xl bg-black"><video src={item.preview} controls playsInline className="aspect-video w-full object-contain"/><button type="button" onClick={()=>removeVideo(index)} className="absolute right-2 top-2 rounded-full bg-black/70 px-2 py-1 text-xs">×</button></div>)}</div>}<p className="text-[9px] leading-4 text-[#6D7282]">Up to {MAX_IMAGES} high-resolution photos and {MAX_VIDEOS} videos. Photos are resized only when necessary and compressed into storage-efficient, 4K-class JPEGs on submit so listings stay sharp without storing oversized raw camera files.</p></Section>
   <button disabled={saving} className="h-12 w-full rounded-2xl bg-blue-500 font-bold text-white disabled:opacity-50">Review listing</button>
...
function ReviewFact({label,value}:{label:string;value:string}){return <div><p className="text-[9px] uppercase tracking-wide text-[#606677]">{label}</p><p className="mt-1 break-words font-semibold text-[#D9DCE4]">{value}</p></div>}
