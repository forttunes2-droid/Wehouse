...
export default function MyReservations({
  ...
  const [accommodationProtection, setAccommodationProtection] =
    useState<AccommodationProtection | null>(null);
  const [arrivalIssue, setArrivalIssue] = useState<{
  ...
  useEffect(() => {
    const subject =
      activeHousing?.stay_type === "short_let" && activeHousing?.id
        ? { type: "short_let" as const, id: String(activeHousing.id) }
        : activeHotel?.booking_id
          ? { type: "hotel" as const, id: String(activeHotel.booking_id) }
          : null;
    let cancelled = false;
    if (!subject) {
      setAccommodationProtection(null);
      return;
    }
    setAccommodationProtection(null);
    void getMyAccommodationProtection(subject.type, subject.id).then(
      ({ protection }) => {
        if (!cancelled) setAccommodationProtection(protection);
      },
    );
    return () => {
      cancelled = true;
    ...
    activeHousing?.id,
    activeHousing?.updated_at,
    activeHotel?.booking_id,
    activeHotel?.updated_at,
  ]);
  ...
  async function submitArrivalIssue() {
    if (!arrivalIssue) return;
    const reason = arrivalReason.trim();
    if (reason.length < 10)
      return toast.error(
        "Explain the arrival problem in at least 10 characters",
      );
    setArrivalBusy(true);
    const { caseId, error } = await reportMyAccommodationArrivalIssue(
      arrivalIssue.subjectType,
      arrivalIssue.subjectId,
      reason,
    );
    setArrivalBusy(false);
    if (error || !caseId)
      return toast.error(
        error?.message || "The arrival issue could not be opened",
      );
    setAccommodationProtection((current) =>
      current
        ? {
            ...current,
            can_report_arrival_issue: false,
            arrival_issue_case_id: caseId,
            protection_state: "disputed",
          }
        : current,
    );
    setArrivalIssue(null);
    setArrivalReason("");
    toast.success(
      "Arrival issue opened. The accommodation payment is frozen for WeHouse review.",
    );
    await load(true);
  }
      ...
      <>
        <PropertyBookingDetail
          ...
          onArrivalIssue={() => {
            setArrivalReason("");
            setArrivalIssue({
              subjectType: "short_let",
              subjectId: String(activeHousing.id),
              label: activeHousing.listing_title || "Short Let",
            });
          }}
        ...
        />
        <ArrivalIssueDialog
          open={arrivalIssue !== null}
          label={arrivalIssue?.label || "stay"}
          reason={arrivalReason}
          busy={arrivalBusy}
          onReason={setArrivalReason}
          onClose={() => {
            if (!arrivalBusy) {
              setArrivalIssue(null);
              setArrivalReason("");
          ...
          onSubmit={() => void submitArrivalIssue()}
        />
      </>
    );
      ...
      <>
        <HotelBookingDetail
          ...
          onArrivalIssue={() => {
            ...
            setArrivalIssue({
              subjectType: "hotel",
              subjectId: String(activeHotel.booking_id),
              label:
                activeHotel.hotels?.name ||
