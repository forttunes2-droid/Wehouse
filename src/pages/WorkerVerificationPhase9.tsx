...
export default function WorkerVerificationPhase9({ profile, onBack }: Props) {
  ...
  async function refresh() {
    ...
    setFee(Number(Array.isArray(raw) ? raw[0]?.value : raw?.value ?? raw ?? 0));
    setLoading(false);
  }
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!ref) return;
      const result = await verifyPaymentWithRetry(ref, { purpose: 'worker_verification' });
      if (dead) return;
      if (result.success) {
        try { localStorage.removeItem(REF_KEY); } catch {}
        toast.success('Onboarding payment confirmed');
        await refresh();
      }
    })();
    return () => { dead = true; };
  ...
  const complete = a.identity_passed && paid && a.test_passed && a.evidence_saved;
  const reviewing = a.worker_status === 'profile_under_review' || (a.submitted && a.worker_status !== 'verified');
  const approvedProfile = a.worker_status === 'verified' && profile.worker_verified === true;
  const identityExpired = approvedProfile && !a.identity_current;
  const live = Boolean(a.live);
  const repeatDays = Number(a.identity_recheck_days || 14);
  ...
  async function pay() {
    if (fee <= 0) return toast.error('Onboarding fee is not configured');
    setBusy(true);
    try {
      const boot = await supabase.rpc('create_worker_verification_payment');
      if (boot.error || !boot.data?.success) throw new Error(boot.data?.error || boot.error?.message || 'Payment could not start');
      const reference = String(boot.data.reference || '');
      const init = await supabase.functions.invoke('worker-verification-payment-init', { body: { reference } });
      if (init.error) throw init.error;
      if (init.data?.already_paid) { await refresh(); setBusy(false); return; }
      if (!init.data?.authorization_url) throw new Error(init.data?.error || 'Paystack could not start');
      try { localStorage.setItem(REF_KEY, reference); } catch {}
  ...
  async function chooseVideo(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      toast.success('Skill video added');
  ...
  async function saveEvidence() {
    if (!videoPath) return toast.error('Add a skill demonstration video');
    setBusy(true);
    const result = await supabase.rpc('save_my_worker_professional_evidence', { p_certificate_path: certificatePath || null, p_video_path: videoPath });
    setBusy(false);
  ...
  if (loading) return <Shell><div className="grid min-h-[60dvh] place-items-center"><Spinner /></div></Shell>;

  return <Shell>
    <Toaster position="top-center" richColors theme="dark" />
    <header className="border-b border-white/[.06] px-4 py-4">
      <div className="mx-auto flex max-w-2xl items-center gap-3">
        <button onClick={onBack} className="grid h-10 w-10 place-items-center rounded-xl border border-white/[.07]">←</button>
        <div><p className="text-[9px] font-bold tracking-[.18em] text-violet-300">WEHOUSE · WORKER</p><h1 className="mt-1 text-lg font-bold">Identity & work verification</h1></div>
      </div>
    </header>
    <main className="mx-auto max-w-2xl space-y-3 px-4 py-4">
      <section className="rounded-2xl border border-blue-500/15 bg-blue-500/[.045] p-4">
        <p className="text-[8px] font-bold uppercase tracking-[.16em] text-blue-300">ACCOUNT PROTECTION</p>
        <h2 className="mt-1 text-sm font-semibold">Identity check repeats every {repeatDays} days</h2>
        <p className="mt-2 text-[10px] leading-5 text-[#8490A3]">WeHouse asks you to repeat the quick live face check every two weeks to confirm that the approved Worker is still the person using this account. It uses the same private selfie comparison and head-turn liveness challenge. You do not repeat payment, the readiness test or your skill review.</p>
      </section>

      {identityExpired ? <>
        <Card eyebrow="IDENTITY CHECK DUE" title="Confirm it is still you" text="Your professional profile and history are safe. Complete the quick live check to return to public discovery and new Worker activity.">
          <Status text="New bookings and public Worker activity are paused until this check passes" />
        </Card>
        <WorkerIdentityCheck profile={profile} status="expired" onSaved={refresh} />
      </> : live ? <Card eyebrow="IDENTITY CURRENT" title="Your Worker account is protected" text={a.identity_due_at ? `Next identity check: ${formatDate(a.identity_due_at)}${a.identity_days_remaining != null ? ` · ${a.identity_days_remaining} day${a.identity_days_remaining === 1 ? '' : 's'} remaining` : ''}.` : `You will be asked to repeat the check every ${repeatDays} days.`}>
        <Status text="Your services are public and you can accept new work" good />
        <Button label="Back to dashboard" onClick={onBack} />
      </Card> : <>
        <Progress profile={a.profile_complete} verification={complete} review={approvedProfile} />
        {a.profile_complete && !reviewing && !approvedProfile && <WorkerVerificationChecklist identityPassed={a.identity_passed} paymentConfirmed={paid} readinessPassed={a.test_passed} skillVideoSaved={a.evidence_saved} />}
        ...
        : !paid ? <Card eyebrow="2 · WORK VERIFICATION" title="Complete onboarding payment" text="One-time Worker onboarding fee. The recurring identity check never charges you again."><Status text="Private face check complete" good /><div className="flex items-center justify-between rounded-xl border border-white/[.07] bg-black/10 px-4 py-3"><span className="text-[10px] text-[#717888]">Onboarding fee</span><strong>₦{fee.toLocaleString()}</strong></div><Button label={busy ? 'Opening Paystack…' : 'Continue to Paystack'} onClick={() => void pay()} disabled={busy || fee <= 0} /></Card>
...
function formatDate(value: string) { return new Date(value).toLocaleDateString([], { day: 'numeric', month: 'short', year: 'numeric' }); }
...
function Spinner() { return <div className="h-7 w-7 animate-spin rounded-full border-2 border-violet-500 border-t-transparent" />; }
...
function Status({ text, good = false }: { text: string; good?: boolean }) { return <div className={`rounded-xl border px-3 py-2.5 text-[9px] ${good ? 'border-emerald-500/15 bg-emerald-500/[.05] text-emerald-300' : 'border-amber-500/15 bg-amber-500/[.05] text-amber-200'}`}>{text}</div>; }
...
function Progress({ profile, verification, review }: { profile: boolean; verification: boolean; review: boolean }) { const items = [['Work profile', profile], ['Work verification', verification], ['WeHouse review', review]] as const; return <section className="grid grid-cols-3 gap-2 rounded-2xl border border-white/[.06] bg-[#0F131A] p-3">{items.map(([label, done], index) => <div key={label} className={`rounded-xl border px-2 py-3 text-center ${done ? 'border-emerald-500/15 bg-emerald-500/[.05]' : 'border-white/[.05]'}`}><span className={`mx-auto grid h-7 w-7 place-items-center rounded-full text-[9px] font-bold ${done ? 'bg-emerald-500 text-[#04120A]' : 'bg-white/[.05] text-[#656C7B]'}`}>{done ? '✓' : index + 1}</span><p className={`mt-2 text-[8px] ${done ? 'text-emerald-300' : 'text-[#676E7E]'}`}>{label}</p></div>)}</section>; }
