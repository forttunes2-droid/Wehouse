...
function People({ onView }: { onView: (profile: Profile) => void }) {
  ...
  async function openCanonical(person:Profile){setOpening(person.user_id);const{data,error}=await supabase.rpc('admin_get_my_branch_profiles',{p_role:person.role});setOpening(null);if(error)return toast.error('The current profile could not be loaded');const current=(Array.isArray(data)?data:[]).find((row:any)=>row.user_id===person.user_id);if(!current)return toast.error('This profile is no longer available');setRows(rows=>rows.map(row=>row.user_id===current.user_id?current:row));onView(current as Profile)}
  ...
  return <Section title="People" note="Regular Users and Property Partners. Team members and Workers have dedicated workspaces."><div className="flex gap-2"><Chip active={role==='user'} onClick={()=>setRole('user')}>Users</Chip><Chip active={role==='property_partner'} onClick={()=>setRole('property_partner')}>Property Partners</Chip></div><input value={search} onChange={(event)=>setSearch(event.target.value)} placeholder="Search accounts" className="h-11 w-full rounded-xl border border-white/[.08] bg-[#141720] px-3 text-xs outline-none focus:border-violet-500/40"/>{loading?<Loading/>:shown.length===0?<Empty text="No matching accounts."/>:<div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{shown.slice(0,120).map((person)=><button key={person.user_id} disabled={opening===person.user_id} onClick={()=>void openCanonical(person)} className="flex items-center gap-3 rounded-2xl border border-white/[.06] bg-[#10131B] p-4 text-left disabled:opacity-50"><span className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-full bg-violet-500/15 text-xs font-bold">{person.avatar_url?<img src={person.avatar_url} alt="" className="h-full w-full object-cover"/>:String(person.full_name||person.username||'W')[0].toUpperCase()}</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{person.full_name||person.username||'WeHouse account'}</span><span className="mt-1 block truncate text-[9px] text-[#666D7E]">{person.email}</span><span className="mt-2 block text-[8px] capitalize text-[#565D6E]">{String(person.role||'user').replace(/_/g,' ')} · {[person.local_government||person.city,person.state].filter(Boolean).join(', ')||'Location not set'}</span></span><span className="text-[#62697A]">›</span></button>)}</div>}</Section>;
}
...
type PlatformSection='identity'|'access'|'workers'|'properties'|'legal';
const PLATFORM_SECTIONS:Array<{id:PlatformSection;label:string;note:string}>=[
  {id:'identity',label:'Identity & contact',note:'WeHouse name and public support details.'},
  {id:'access',label:'Access & registration',note:'Maintenance mode and new-account access.'},
  {id:'workers',label:'Worker marketplace',note:'Onboarding, trust, occupations and services.'},
  {id:'properties',label:'Property marketplace',note:'Property choices used in submission and discovery.'},
  {id:'legal',label:'Legal documents',note:'Published Privacy Policy and Terms & Conditions.'},
];
function PlatformControl({profile}:{profile:Profile}){
  const[section,setSection]=useState<PlatformSection|null>(null);
  if(!section)return <div className="space-y-4"><div><h2 className="text-lg font-bold">Platform settings</h2><p className="mt-1 max-w-2xl text-[10px] leading-5 text-[#707687]">Choose one control area. Settings are grouped by what they change across WeHouse.</p></div><div className="overflow-hidden border-y border-white/[.07]">{PLATFORM_SECTIONS.map((item,index)=><div key={item.id}>{index>0&&<div className="ml-12 h-px bg-white/[.055]"/>}<button type="button" onClick={()=>setSection(item.id)} className="flex min-h-[68px] w-full items-center gap-3 py-3 text-left"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-violet-500/10 text-xs font-bold text-violet-300">{index+1}</span><span className="min-w-0 flex-1"><strong className="block text-sm">{item.label}</strong><span className="mt-1 block text-[9px] leading-4 text-[#6C7283]">{item.note}</span></span><span className="text-[#646B7C]">›</span></button></div>)}</div></div>;
  const current=PLATFORM_SECTIONS.find(item=>item.id===section)!;
  return <div className="space-y-5"><header className="flex items-center gap-3 border-b border-white/[.07] pb-3"><button type="button" onClick={()=>setSection(null)} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white/[.04] text-lg" aria-label="Back to Platform settings">‹</button><div><p className="text-[8px] font-bold uppercase tracking-[.16em] text-violet-300">Platform settings</p><h2 className="mt-0.5 text-lg font-bold">{current.label}</h2><p className="mt-0.5 text-[9px] text-[#707687]">{current.note}</p></div></header>{section==='identity'&&<CreatorSettingsTabV2 profile={profile} groups={['identity']} title="Identity & contact" description="Canonical public details used across WeHouse."/>}{section==='access'&&<CreatorSettingsTabV2 profile={profile} groups={['access']} title="Access & registration" description="Platform-wide availability and account creation controls."/>}{section==='workers'&&<div className="space-y-5"><CreatorSettingsTabV2 profile={profile} groups={['worker_verification','worker_trust']} title="Worker marketplace rules" description="Worker onboarding and earned marketplace trust."/><section className="border-t border-white/[.07] pt-5"><h3 className="mb-1 text-sm font-semibold">Occupations and services</h3><p className="mb-4 text-[9px] leading-5 text-[#686F80]">Occupation describes who the Worker is; services describe the work customers can request.</p><ServiceCategoryManager profile={profile}/></section></div>}{section==='properties'&&<section><h3 className="mb-1 text-sm font-semibold">Property types</h3><p className="mb-4 text-[9px] leading-5 text-[#686F80]">One canonical list shared by Property Partner submission and public discovery.</p><PropertyTypeManager profile={profile}/></section>}{section==='legal'&&<CreatorSettingsTabV2 profile={profile} groups={['legal']} title="Published legal documents" description="The current documents shown when people create and use an account."/>}</div>;
}
