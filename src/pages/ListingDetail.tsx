...
export default function ListingDetail({ listingId, onNavigate, profile, isSaved, onToggleSave }: Props) {
  ...
  const visibleAddress = hasOwnActiveReservation
  ...
  return <div className="min-h-[100dvh] overflow-x-hidden bg-[#090A0F] pb-12 text-white">
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <main className="px-4 py-5 sm:px-6 lg:px-8"><div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="min-w-0 space-y-5">
          <section><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h1 className="break-words text-2xl font-bold">{listing.title}</h1><p className="mt-1 text-xs text-[#777B8B]">{visibleAddress || 'Location unavailable'}{!hasOwnActiveReservation ? ' · exact address after reservation' : ''}</p></div><p className="shrink-0 text-xl font-bold text-violet-400">₦{Number(listing.price || 0).toLocaleString()}<span className="ml-1 text-[10px] font-normal text-[#686C7D]">/year</span></p></div></section>
    ...
    {locationOpen&&Number.isFinite(Number((listing as any).gps_latitude))&&Number.isFinite(Number((listing as any).gps_longitude))&&<div className="fixed inset-0 z-[90] flex items-end justify-center bg-black/75 p-3 backdrop-blur-sm sm:items-center" onClick={()=>setLocationOpen(false)}><section className="w-full max-w-xl rounded-[26px] border border-white/[.09] bg-[#11141C] p-4" onClick={event=>event.stopPropagation()}><div className="mb-3 flex items-start justify-between gap-3"><div><h2 className="text-sm font-semibold">{hasOwnActiveReservation?'Property destination':'Property area'}</h2><p className="mt-1 text-[9px] leading-4 text-[#74798A]">{hasOwnActiveReservation?visibleAddress:'The exact entrance remains private until reservation.'}</p></div><button type="button" onClick={()=>setLocationOpen(false)} className="grid h-10 w-10 place-items-center text-xl text-[#7E8494]" aria-label="Close map">×</button></div><LocationMap latitude={Number((listing as any).gps_latitude)} longitude={Number((listing as any).gps_longitude)} label={hasOwnActiveReservation?'Property destination':'Approximate property area'} height={320} approximate={!hasOwnActiveReservation} showDirections={hasOwnActiveReservation}/></section></div>}
