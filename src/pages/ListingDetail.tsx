...
import { directionsUrl, distanceBetweenKm, useDiscoveryLocation } from '@/hooks/useDiscoveryLocation';
...
export default function ListingDetail({ listingId, onNavigate, profile, isSaved, onToggleSave }: Props) {
  ...
  const destination = Number.isFinite(Number(listing.gps_latitude)) && Number.isFinite(Number(listing.gps_longitude)) ? { lat: Number(listing.gps_latitude), lng: Number(listing.gps_longitude) } : null;
  const distance = location && destination ? distanceBetweenKm(location, destination) : null;
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <main className="px-4 py-5 sm:px-6 lg:px-8"><div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="min-w-0 space-y-5">
          ...
          {destination&&<section className="border-y border-white/[.07] py-4"><div className="flex items-center justify-between gap-4"><div><h2 className="text-sm font-semibold">Apartment location</h2><p className="mt-1 text-[10px] leading-5 text-[#707586]">{visibleAddress||'Location unavailable'}{distance!==null?` · about ${distance<1?`${Math.max(1,Math.round(distance*1000))} m`:`${distance.toFixed(distance<10?1:0)} km`} away`:''}</p></div><a href={directionsUrl(destination.lat,destination.lng)} target="_blank" rel="noreferrer" className="shrink-0 rounded-xl border border-white/[.09] px-4 py-2.5 text-[10px] font-semibold text-violet-300">Directions</a></div></section>}
