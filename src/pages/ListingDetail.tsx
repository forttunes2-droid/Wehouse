...
export default function ListingDetail({ listingId, onNavigate, profile, isSaved, onToggleSave }: Props) {
  ...
  const images = listing.images?.length ? listing.images : listing.videos?.length ? [] : ['https://placehold.co/900x650/171922/666A7A?text=No+Image'];
  ...
  const visibleAddress = [listing.address, listing.city, listing.state].filter(Boolean).join(', ');

  return <div className="min-h-[100dvh] overflow-x-hidden bg-[#090A0F] pb-12 text-white">
    <Toaster position="top-center" richColors />
    <div className="mx-auto max-w-6xl">
      <PropertyMediaCarousel images={images} videos={listing.videos || []} title={listing.title}>
      ...
      <main className="px-4 py-5 sm:px-6 lg:px-8"><div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="min-w-0 space-y-5">
          <section><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h1 className="break-words text-2xl font-bold">{listing.title}</h1><p className="mt-1 text-xs text-[#777B8B]">{visibleAddress || 'Location unavailable'}</p></div><p className="shrink-0 text-xl font-bold text-violet-400">₦{Number(listing.price || 0).toLocaleString()}<span className="ml-1 text-[10px] font-normal text-[#686C7D]">{listing.sub_type === 'short_let' ? '/night' : '/year'}</span></p></div></section>
