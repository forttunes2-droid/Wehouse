...
export default function ListingDetail({ listingId, onNavigate, profile, isSaved, onToggleSave }: Props) {
  ...
  async function saveRentPlan() {
    ...
    setBusy(true);
    const { reservation: updated, error } = await updateReservationPlan(reservation.id, plan.durationYears);
    setBusy(false);
    if (error || !updated) return toast.error(error?.message || 'Could not save the rent plan');
  ...
  async function openShare(){
    ...
    const accepted=(conversations||[]).filter((chat:any)=>chat.conversation_type==='roommate'&&['active','accepted'].includes(String(chat.status)));
  ...
  async function requestInspection() {
    ...
    setBusy(true);
    const { inspection: request, error } = await createInspectionRequest(reservation.id, listingId, profile.user_id, `Inspection requested for ${listing?.title || 'apartment'}`);
    setBusy(false);
    if (error) return toast.error(error.message);
    setInspection(request || null);
    await load();
  ...
  if (!listing) return <div className="grid min-h-[70dvh] place-items-center bg-[#090A0F] px-4 text-center text-white"><div><p className="text-sm font-semibold">Apartment unavailable</p><button onClick={onNavigate} className="mt-4 text-xs text-violet-400">← Go back</button></div></div>;
  ...
  const canSeeExactAddress = Boolean(reservation && ['reserved', 'inspection_pending', 'ready_for_move_in', 'occupied', 'completed'].includes(String(reservation.status)));
  const canStartReservation = status === 'available' && !hasOwnActiveReservation;
  const visibleAddress = canSeeExactAddress
  ...
  return <div className="min-h-[100dvh] overflow-x-hidden bg-[#090A0F] pb-12 text-white">
    ...
    <div className="mx-auto max-w-6xl">
      <PropertyMediaCarousel images={images} title={listing.title}>
        ...
        {!hasOwnActiveReservation && <button onClick={onToggleSave} aria-label={isSaved?'Remove from saved apartments':'Save apartment'} aria-pressed={isSaved} className="absolute right-4 top-4 grid h-10 w-10 place-items-center rounded-full bg-black/50 backdrop-blur"><Heart filled={isSaved}/></button>}
      ...
      <main className="px-4 py-5 sm:px-6 lg:px-8"><div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="min-w-0 space-y-5">
          <section><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h1 className="break-words text-2xl font-bold">{listing.title}</h1><p className="mt-1 text-xs text-[#777B8B]">{visibleAddress || 'Location unavailable'}{!canSeeExactAddress ? ' · exact address after reservation confirmation' : ''}</p></div><p className="shrink-0 text-xl font-bold text-violet-400">₦{Number(listing.price || 0).toLocaleString()}<span className="ml-1 text-[10px] font-normal text-[#686C7D]">{listing.sub_type === 'short_let' ? '/night' : '/year'}</span></p></div></section>
          ...
          {listing.description && <section className="rounded-2xl border border-white/[.06] bg-[#11141C] p-4 sm:p-5"><h2 className="text-sm font-semibold">About this apartment</h2><p className="mt-2 whitespace-pre-wrap text-xs leading-6 text-[#9599A8]">{listing.description}</p></section>}
          {Number.isFinite(Number((listing as any).gps_latitude))&&Number.isFinite(Number((listing as any).gps_longitude))&&<section className="border-y border-white/[.07] py-4"><div className="flex items-center justify-between gap-4"><div><h2 className="text-sm font-semibold">Apartment location</h2><p className="mt-1 text-[10px] leading-5 text-[#707586]">{visibleAddress||'Location unavailable'} · {canSeeExactAddress?'exact destination available':'approximate area until reservation confirmation'}</p></div><button type="button" onClick={()=>setLocationOpen(true)} className="shrink-0 rounded-xl border border-white/[.09] px-4 py-2.5 text-[10px] font-semibold text-violet-300">View map</button></div></section>}
          {listing.videos?.length > 0 && <section><h2 className="mb-3 text-sm font-semibold">Apartment videos</h2><div className="grid gap-3 sm:grid-cols-2">{listing.videos.map((url, index) => <video key={url} src={url} controls playsInline preload="metadata" className="aspect-video w-full rounded-2xl bg-black object-contain" aria-label={`Apartment video ${index + 1}`} />)}</div></section>}
          {!hasOwnActiveReservation && !sharedGroup && <section className="border-y border-white/[.07] py-4"><h2 className="text-sm font-semibold">Questions about this apartment?</h2><p className="mt-1 text-[11px] leading-relaxed text-[#7C8191]">Message WeHouse here. Your conversation stays connected to this apartment.</p><button onClick={() => support('property')} className="mt-3 text-xs font-semibold text-violet-300">Message WeHouse →</button></section>}
        </div>

        <aside className="space-y-3 lg:sticky lg:top-20 lg:self-start">
          {sharedGroup ? <SharedHomeCard group={sharedGroup} profile={profile} busy={busy} onAccept={answerShare} onPay={payShare}/> : hasOwnActiveReservation ? <ReservationPanel reservation={reservation} inspection={inspection} busy={busy} fee={reservationFee} onResume={() => void resumeCheckout()} onInspect={() => void requestInspection()} onRentOptions={() => setShowPlan(true)} onRentPay={() => void payContractRent()} onSupport={() => support(reservation?.status === 'payment_pending' || reservation?.rent_payment_status === 'payment_pending' ? 'payment' : inspection ? 'inspection' : 'reservation')} /> : canStartReservation ? <section className="border-y border-white/[.08] py-5"><div className="flex items-end justify-between gap-4"><div><p className="text-[9px] font-bold uppercase tracking-[.15em] text-violet-300">Reservation</p><h2 className="mt-1 text-lg font-bold">Reserve this apartment</h2></div><p className="text-right"><span className="block text-sm font-bold">₦{reservationFee.toLocaleString()}</span><span className="text-[8px] text-[#747889]">reservation fee</span></p></div><p className="mt-3 text-[11px] leading-relaxed text-[#777B8B]">Pay the reservation fee to submit your request. WeHouse will show each next step in your booking.</p><button disabled={busy} onClick={() => void openCheckout()} className="mt-4 h-12 w-full rounded-full bg-violet-500 text-sm font-semibold disabled:opacity-50">Reserve apartment</button><button onClick={() => setReservationOptionsOpen(true)} className="mt-3 h-10 w-full text-xs font-semibold text-violet-300">Reserve with a roommate</button></section> : <section className="rounded-3xl border border-amber-500/15 bg-amber-500/[.04] p-5"><h2 className="text-sm font-semibold">{state.label}</h2><p className="mt-2 text-[11px] text-[#8A8E9D]">This apartment is not open for a new reservation right now.</p></section>}
        </aside>
      </div></main>
    </div>

    {showPlan && <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/70 sm:items-center sm:p-4" onClick={() => !busy && setShowPlan(false)}><section className="max-h-[92dvh] w-full max-w-lg overflow-y-auto rounded-t-3xl border border-white/[.08] bg-[#11141C] p-5 text-white sm:rounded-3xl" onClick={event => event.stopPropagation()}><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-wide text-emerald-300">Rent plan</p><h2 className="mt-1 text-lg font-bold">Choose how you want to pay rent</h2><p className="mt-1 text-[10px] text-[#74798A]">Review the amounts and schedule before you confirm.</p></div><button disabled={busy} onClick={() => setShowPlan(false)} className="text-[#777B8B]">×</button></div><div className="mt-5"><RentalPlanSelector annualRent={listing.price || 0} subType={listing.sub_type || 'long_stay'} securityDepositAmount={listing.security_deposit_amount} onSelectPlan={setPlan} /></div><button onClick={() => void saveRentPlan()} disabled={!plan || busy} className="mt-5 h-12 w-full rounded-xl bg-emerald-500 text-sm font-semibold text-[#03100B] disabled:opacity-40">{busy ? 'Saving…' : 'Save rent plan'}</button><button onClick={()=>setShowPlan(false)} disabled={busy} className="mt-2 h-11 w-full text-xs text-violet-300">Keep current plan</button></section></div>}
    {reservationOptionsOpen&&<div className="fixed inset-0 z-[79] flex items-end justify-center bg-black/75 backdrop-blur-sm sm:items-center sm:p-4" onClick={()=>setReservationOptionsOpen(false)}><section className="w-full max-w-lg rounded-t-[30px] border border-white/[.08] bg-[#11141C] px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-5 text-white sm:rounded-[30px]" onClick={event=>event.stopPropagation()}><div className="flex items-start justify-between gap-3"><div><p className="text-[9px] font-bold uppercase tracking-[.16em] text-violet-300">Shared reservation</p><h2 className="mt-1 text-lg font-bold">Reserve with a roommate</h2></div><button onClick={()=>setReservationOptionsOpen(false)} className="grid h-10 w-10 place-items-center text-[#777B8B]" aria-label="Close">×</button></div><button disabled={busy} onClick={()=>{setReservationOptionsOpen(false);void openShare()}} className="mt-5 flex min-h-16 w-full items-center justify-between border-y border-white/[.07] py-3 text-left disabled:opacity-40"><span><span className="block text-sm font-semibold">Choose a connected roommate</span><span className="mt-1 block text-[9px] text-[#74798A]">Both people accept and pay their own reservation share</span></span><span className="text-violet-300">›</span></button></section></div>}
    {locationOpen&&Number.isFinite(Number((listing as any).gps_latitude))&&Number.isFinite(Number((listing as any).gps_longitude))&&<div className="fixed inset-0 z-[90] flex items-end justify-center bg-black/75 p-3 backdrop-blur-sm sm:items-center" onClick={()=>setLocationOpen(false)}><section className="w-full max-w-xl rounded-[26px] border border-white/[.09] bg-[#11141C] p-4" onClick={event=>event.stopPropagation()}><div className="mb-3 flex items-start justify-between gap-3"><div><h2 className="text-sm font-semibold">{canSeeExactAddress?'Apartment destination':'Apartment area'}</h2><p className="mt-1 text-[9px] leading-4 text-[#74798A]">{canSeeExactAddress?visibleAddress:'The exact entrance remains private until the reservation is confirmed.'}</p></div><button type="button" onClick={()=>setLocationOpen(false)} className="grid h-10 w-10 place-items-center text-xl text-[#7E8494]" aria-label="Close map">×</button></div><LocationMap latitude={Number((listing as any).gps_latitude)} longitude={Number((listing as any).gps_longitude)} label={canSeeExactAddress?'Apartment destination':'Approximate apartment area'} height={320} approximate={!canSeeExactAddress} showDirections={canSeeExactAddress}/></section></div>}
    {shareOpen&&<div className="fixed inset-0 z-[85] flex items-end justify-center bg-black/75 sm:items-center sm:p-4" onClick={()=>setShareOpen(false)}><section className="max-h-[80dvh] w-full max-w-lg overflow-y-auto rounded-t-3xl border border-white/[.08] bg-[#11141C] p-5 sm:rounded-3xl" onClick={event=>event.stopPropagation()}><div className="flex justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase text-violet-300">Shared apartment</p><h2 className="mt-1 text-lg font-bold">Choose your roommate</h2><p className="mt-1 text-[10px] text-[#74798A]">Only people with an accepted roommate conversation appear here.</p></div><button onClick={()=>setShareOpen(false)}>×</button></div><div className="mt-4 divide-y divide-white/[.06]">{roommateChats.length?roommateChats.map((chat:any)=>{const peer=chat.participant_a===profile.user_id?chat.participant_b:chat.participant_a;return <button key={chat.id} disabled={busy} onClick={()=>void shareWith(chat.id)} className="flex min-h-14 w-full items-center justify-between py-3 text-left"><span><span className="block text-sm font-semibold">{chat.other_user?.full_name||chat.other_user?.username||'Roommate'}</span><span className="mt-1 block text-[9px] text-[#6D7283]">Invite to split this apartment equally</span></span><span className="text-violet-300">›</span><span className="sr-only">{peer}</span></button>}):<p className="py-8 text-center text-[10px] text-[#74798A]">Connect with a roommate first, then return here to share an apartment.</p>}</div></section></div>}
  </div>;
}
...
function ReservationPanel({ reservation, inspection, busy, fee, onResume, onInspect, onRentOptions, onRentPay, onSupport }: { reservation: any; inspection: any; busy: boolean; fee: number; onResume: () => void; onInspect: () => void; onRentOptions: () => void; onRentPay: () => void; onSupport: () => void }) {
  ...
  if (status === 'ready_for_move_in') {
    ...
    return <section className="rounded-3xl border border-emerald-500/15 bg-[radial-gradient(circle_at_top_right,rgba(16,185,129,.12),transparent_45%),#11141C] p-5"><p className="text-[10px] font-semibold uppercase tracking-wide text-emerald-300">Payment complete · apartment ready</p><h2 className="mt-2 text-xl font-bold">Your apartment is ready for handover</h2><p className="mt-2 text-[11px] leading-relaxed text-[#8A918F]">Show this booking code to WeHouse Operations at the apartment. They will match your identity, payment and apartment before handing over access and activating the tenancy.</p>{reservation.booking_code && <Row label="Booking code" value={String(reservation.booking_code)} />}{reservation.rent_paid_at && <Row label="Rent verified" value={new Date(reservation.rent_paid_at).toLocaleString()} />}<button onClick={onSupport} className="mt-4 h-11 w-full rounded-xl border border-white/[.08] text-xs font-semibold">Message WeHouse</button></section>;
  }
  return <section className="rounded-3xl border border-emerald-500/15 bg-[#11141C] p-5"><p className="text-[10px] font-semibold uppercase tracking-wide text-emerald-300">Reservation paid</p><h2 className="mt-2 text-lg font-bold">Apartment held for you</h2>{reservation.hold_expires_at && <p className="mt-2 text-[10px] text-amber-300">Current hold · until {new Date(reservation.hold_expires_at).toLocaleString()}</p>}<div className="mt-3 rounded-xl bg-white/[.035] p-3"><Row label="Reservation fee" value={`₦${Number(reservation.amount || fee).toLocaleString()}`} /><Row label="Rent plan" value={`${reservation.rental_plan_years || 1} year${Number(reservation.rental_plan_years || 1) === 1 ? '' : 's'}`} /></div>{inspection ? <div className="mt-3 rounded-xl border border-violet-500/10 bg-violet-500/[.05] p-3 text-[10px] text-violet-300">Inspection · {String(inspection.status || 'pending').replace(/_/g, ' ')}</div> : <button disabled={busy} onClick={onInspect} className="mt-4 h-11 w-full rounded-xl bg-violet-500 text-xs font-semibold disabled:opacity-50">{busy ? 'Requesting…' : 'Request apartment inspection'}</button>}<button onClick={onRentOptions} className="mt-2 h-10 w-full text-xs font-semibold text-emerald-300">Rent options</button><button onClick={onSupport} className="h-10 w-full text-xs text-violet-300">Message WeHouse</button></section>;
}
