...
export default function Saved({ profile, onNavigate, savedIds, onToggleSave }: SavedProps) {
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] pb-20">
      {/* Header */}
      <header className="bg-[#0F1724] text-white px-5 pt-4 pb-5">
        <h1 className="text-lg font-semibold">Saved Homes</h1>
      ...
      <div className="px-5 pt-4">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm text-[#8B8680]">No saved homes yet</p>
            <button
              onClick={() => onNavigate('home')}
              className="mt-2 text-xs text-[#C8A45A] font-medium"
            >
              Browse homes
            </button>
          </div>
