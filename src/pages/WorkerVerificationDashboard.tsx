...
type WorkerTab = 'pending' | 'verified' | 'suspended' | 'all';
...
export default function WorkerVerificationDashboard({ profile }: WorkerVerificationDashboardProps) {
  ...
  const counts = {
    ...
    verified: workers.filter(w => w.worker_status === 'verified').length,
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F] pb-6">
      ...
      <div className="grid grid-cols-3 gap-3 px-5 py-4">
        {(['pending', 'verified', 'suspended'] as const).map(tab => (
          <button
            key={tab}
...
// ─── WORKER CARD ───────────────────────────────────
function WorkerCard({ worker, onUpdateStatus }: { worker: Profile; onUpdateStatus: (id: string, status: 'verified' | 'suspended' | 'rejected') => void }) {
  const [expanded, setExpanded] = useState(false);

  const statusColor = worker.worker_status === 'verified' ? 'text-emerald-400 bg-emerald-500/10' :
        ...
        <div className="px-4 pb-4 space-y-3 border-t border-white/[0.04] pt-3">
          ...
          <div className="flex gap-2">
            {worker.worker_status !== 'verified' && (
              <button
                onClick={() => onUpdateStatus(worker.user_id, 'verified')}
