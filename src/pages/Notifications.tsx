...
type DeviceLogin = { sessionId:string; device:string; os:string; browser:string; location:string; loginTime:string };
...
export default function Notifications({ profile, onNavigate, embedded = false, onUnreadChange }: Props) {
  ...
  async function open(row: Activity) {
    ...
    if(row.type==='new_device_login'){
      ...
      const{data:session,error:sessionError}=await supabase.from('user_sessions').select('id,device,os,browser,login_time,trust_status').eq('id',sessionId).maybeSingle();
      if(sessionError)return toast.error('Device record could not be opened');
      if(!session||session.trust_status!=='pending'||getStoredSessionId()===sessionId){onNavigate('devices');return}
      setDeviceLogin({sessionId,device:String(session.device||row.destination_params?.device||'Device'),os:String(session.os||row.destination_params?.os||'Unknown'),browser:String(session.browser||row.destination_params?.browser||'Unknown'),location:String(row.destination_params?.location||'Location unavailable'),loginTime:String(session.login_time||row.destination_params?.login_time||row.created_at)});return;
    }
  ...
  const content = <main className={embedded ? "py-1" : "mx-auto max-w-4xl px-4 py-5"}>{loading ? <ActivityLoading /> : error && rows.length === 0 ? <ErrorState text={error} retry={() => void load()} /> : rows.length === 0 ? <Empty /> : <div className="space-y-7">
    {unread > 0 && <div className="flex items-center justify-between border-b border-white/[.07] pb-3"><div><p className="text-sm font-semibold">{unread} need{unread===1?'s':''} your attention</p><p className="mt-1 text-[9px] text-[#697081]">Open an update to go to its property, booking, job or security record.</p></div><button onClick={() => void markAll()} className="shrink-0 rounded-full border border-white/[.08] px-3 py-2 text-[9px] font-semibold text-violet-300">Mark read</button></div>}
  ...
  const deviceConfirmation=deviceLogin&&<div className="fixed inset-0 z-[110] grid place-items-end bg-black/75 p-0 backdrop-blur-md sm:place-items-center sm:p-4" role="dialog" aria-modal="true" aria-label="Review new device login"><section className="w-full rounded-t-[28px] border border-white/[.08] bg-[#11141C] p-5 text-white shadow-2xl sm:max-w-md sm:rounded-[28px]"><div className="flex items-start justify-between gap-4"><div><p className="text-[9px] font-bold uppercase tracking-[.18em] text-violet-300">DEVICE VERIFICATION</p><h2 className="mt-2 text-xl font-bold">Approve this device?</h2><p className="mt-2 text-[10px] leading-5 text-[#858B9A]">A device is waiting to enter your WeHouse account.</p></div><button onClick={()=>setDeviceLogin(null)} disabled={deviceBusy} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white/[.05] text-lg" aria-label="Close">×</button></div><div className="mt-5 space-y-3 rounded-2xl border border-white/[.06] bg-black/15 p-4"><DeviceDetail label="Device" value={deviceLogin.device}/><DeviceDetail label="System" value={`${deviceLogin.os} · ${deviceLogin.browser}`}/><DeviceDetail label="Location" value={deviceLogin.location}/><DeviceDetail label="Time" value={new Date(deviceLogin.loginTime).toLocaleString()}/></div><div className="mt-4 grid grid-cols-2 gap-3"><button onClick={()=>void answerDeviceLogin(false)} disabled={deviceBusy} className="h-12 rounded-2xl border border-red-500/20 bg-red-500/[.05] text-xs font-semibold text-red-300 disabled:opacity-40">Block</button><button onClick={()=>void answerDeviceLogin(true)} disabled={deviceBusy} className="h-12 rounded-2xl bg-violet-500 text-xs font-semibold disabled:opacity-40">{deviceBusy?'Saving…':'Approve'}</button></div></section></div>;
  if (embedded) return <><Toaster position="top-center" richColors />{content}{confirmation}{deviceConfirmation}</>;
  return <div className="min-h-[100dvh] bg-[#090B10] pb-28 text-white"><Toaster position="top-center" richColors /><header className="sticky top-0 z-40 border-b border-white/[.06] bg-[#090B10]/95 px-4 py-4 backdrop-blur-xl"><div className="mx-auto max-w-4xl"><p className="text-[9px] font-bold uppercase tracking-[.24em] text-violet-400">WEHOUSE</p><h1 className="mt-1 text-xl font-bold">Activity</h1><p className="mt-1 text-[10px] text-[#747A8B]">Important job, payment, security and official updates linked to their source.</p></div></header>{content}{confirmation}{deviceConfirmation}</div>;
}
...
function activityAction(row: Activity, expanded: boolean) {
  ...
  if (row.type === 'new_device_login') {const decision=String(row.destination_params?.decision||'pending');return /verified|confirmed|blocked|rejected/i.test(row.title)||decision!=='pending'?'Review devices':'Review sign-in'}
