...
type DeviceLogin = { sessionId:string; device:string; os:string; browser:string; location:string; loginTime:string; currentDevice:boolean };
...
export default function Notifications({ profile, onNavigate, embedded = false, onUnreadChange }: Props) {
  ...
  async function open(row: Activity) {
    ...
    if(row.type==='new_device_login'){
      ...
      setDeviceLogin({sessionId,device:String(row.destination_params?.device||'Device'),os:String(row.destination_params?.os||'Unknown'),browser:String(row.destination_params?.browser||'Unknown'),location:String(row.destination_params?.location||'Location unavailable'),loginTime:String(row.destination_params?.login_time||row.created_at),currentDevice:getStoredSessionId()===sessionId});return;
    }
  ...
  const deviceConfirmation=deviceLogin&&<div className="fixed inset-0 z-[110] grid place-items-end bg-black/75 p-0 backdrop-blur-md sm:place-items-center sm:p-4" role="dialog" aria-modal="true" aria-label="Review new device login"><section className="w-full overflow-hidden rounded-t-[30px] border border-white/[.08] bg-gradient-to-b from-[#151925] to-[#0F1219] text-white shadow-2xl sm:max-w-md sm:rounded-[30px]"><div className="border-b border-white/[.06] p-5"><div className="flex items-start justify-between gap-4"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-violet-500/12 text-xl text-violet-300">⌾</div><button onClick={()=>setDeviceLogin(null)} disabled={deviceBusy} className="grid h-10 w-10 place-items-center rounded-full bg-white/[.05] text-lg" aria-label="Close">×</button></div><p className="mt-5 text-[9px] font-bold uppercase tracking-[.2em] text-violet-300">NEW SIGN-IN</p><h2 className="mt-2 text-xl font-bold">Was this you?</h2><p className="mt-2 text-[10px] leading-5 text-[#858B9A]">A new device used your WeHouse account. Check the details before allowing it.</p></div><div className="space-y-3 p-5"><DeviceDetail label="Device" value={deviceLogin.device}/><DeviceDetail label="System" value={`${deviceLogin.os} · ${deviceLogin.browser}`}/><DeviceDetail label="Approximate location" value={deviceLogin.location}/><DeviceDetail label="Time" value={new Date(deviceLogin.loginTime).toLocaleString()}/>{deviceLogin.currentDevice?<p className="mt-4 rounded-2xl bg-amber-500/[.07] p-4 text-[10px] leading-5 text-amber-200">This device cannot approve itself. Confirm with Google here, or review it from another trusted device.</p>:<div className="grid grid-cols-1 gap-3 pt-3 sm:grid-cols-2"><button onClick={()=>void answerDeviceLogin(true)} disabled={deviceBusy} className="h-12 rounded-2xl bg-violet-500 text-xs font-semibold disabled:opacity-40">{deviceBusy?'Saving…':'Yes, it’s me'}</button><button onClick={()=>void answerDeviceLogin(false)} disabled={deviceBusy} className="h-12 rounded-2xl border border-red-500/20 bg-red-500/[.06] text-xs font-semibold text-red-300 disabled:opacity-40">No, secure account</button></div>}</div></section></div>;
