...
export default function Search({onNavigate,savedIds,onToggleSave}:SearchProps){
 ...
 const filterCount=[bedrooms,filterState,filterCity,propertyType,radius].filter(Boolean).length+(priceMin!==''||priceMax!==''?1:0);
 ...
 const locationSummary=filterCity?`${filterCity}, ${filterState}`:filterState?filterState:'State, LGA, price and property';
 return <DiscoveryShell active="homes" title="Homes" description="Find available homes with one clean set of location, price and property filters." onNavigate={onNavigate}>
  ...
  {showFilters&&<DiscoveryFilterSheet title="Filter homes" onClose={()=>setShowFilters(false)} onClear={clearFilters}>
   ...
   {priceBounds?<DiscoveryPriceRangeSlider label="Home price range" minimum={priceBounds.minimum} maximum={priceBounds.maximum} minValue={priceMin} maxValue={priceMax} onMinChange={setPriceMin} onMaxChange={setPriceMax}/>:<section className="rounded-2xl border border-white/[.07] bg-[#151922] p-3.5"><p className="text-[10px] font-medium text-[#9CA2B1]">Home price range</p><p className="mt-1 text-[9px] leading-relaxed text-[#62697A]">The slider will activate as soon as at least two available homes have real prices. No fake price range is shown.</p></section>}
