...
export default function Search({onNavigate,savedIds,onToggleSave}:SearchProps){
 ...
 const loadProperties=useCallback(async(quiet=false)=>{if(!quiet&&!propertyCache)setLoading(true);setLoadError('');const{homes,error}=await getDiscoverableHomes();if(error){setLoadError('Apartments could not be loaded. Check your connection and try again.')}else{propertyCache=homes||[];setListings(propertyCache)}setLoading(false)},[]);
 useEffect(()=>{let live=true;void getDiscoverableHomes().then(({homes,error})=>{if(!live)return;if(error)setLoadError('Apartments could not be loaded. Check your connection and try again.');else{propertyCache=homes||[];setListings(propertyCache)}setLoading(false)});return()=>{live=false}},[]);
 ...
 return <DiscoveryShell active="homes" title="Find an apartment" description="Apartments, hotels, compatible roommates and trusted WeHouse Services—together in one place." onNavigate={onNavigate}>
  <main className="mx-auto max-w-7xl space-y-4 px-4 py-5 sm:px-6 lg:px-8">
   ...
   {mappedCount>0&&<div className="grid grid-cols-3 overflow-hidden rounded-2xl border border-white/[.07] bg-[#11141C] p-1"><button type="button" onClick={()=>setView('list')} className={`min-h-10 rounded-xl text-[9px] font-semibold ${view==='list'&&!userLocation?'bg-violet-500 text-white':'text-[#7B8191]'}`}>List</button><button type="button" onClick={()=>setView('map')} className={`min-h-10 rounded-xl text-[9px] font-semibold ${view==='map'?'bg-violet-500 text-white':'text-[#7B8191]'}`}>Map</button><button type="button" disabled={locating} onClick={()=>{setView('list');locateUser()}} className={`min-h-10 rounded-xl text-[9px] font-semibold disabled:opacity-50 ${view==='list'&&userLocation?'bg-violet-500 text-white':'text-[#7B8191]'}`}>{locating?'Locating…':'Near me'}</button></div>}
   {locationError&&<p className="px-1 text-[9px] text-amber-300" role="alert">{locationError}</p>}
   ...
   {loadError&&!listings.length?<section className="border-y border-red-500/15 px-5 py-12 text-center"><p className="text-sm font-semibold">Apartments could not be loaded</p><p className="mt-2 text-[10px] text-[#777D8D]">{loadError}</p><button type="button" onClick={()=>void loadProperties()} className="mt-4 rounded-xl bg-violet-500 px-4 py-3 text-xs font-semibold">Try again</button></section>:loading&&!listings.length?<div className="grid min-h-56 place-items-center"><div className="h-7 w-7 animate-spin rounded-full border-2 border-violet-500 border-t-transparent"/></div>:filtered.length===0?<DiscoveryEmpty title={emptyTitle} text="Change the selected filters to see other apartments."/>:view==='map'?<PropertyMapExplorer items={filtered} userLocation={userLocation} radius={radius} onOpen={listing=>onNavigate('detail',listing.id)}/>:<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{filtered.map(({listing,distance})=><ListingCard key={listing.id} listing={listing} distanceKm={distance} onClick={()=>onNavigate('detail',listing.id)} isSaved={savedIds.has(listing.id)} onToggleSave={event=>{event.stopPropagation();onToggleSave(listing.id)}}/>)}</div>}
  </main>
  ...
  {showFilters&&<DiscoveryFilterSheet title="Find apartments" onClose={()=>setShowFilters(false)} onClear={clearFilters} resultLabel={`Show ${filtered.length} ${filtered.length===1?'apartment':'apartments'}`}>
   ...
   {mappedCount>0&&userLocation&&<section><p className="mb-2 text-[10px] font-medium text-[#7B8190]">Distance from your current location</p><div className="grid grid-cols-5 gap-1.5">{([['','Any'],['2','2 km'],['5','5 km'],['10','10 km'],['20','20 km']] as const).map(([value,label])=><button key={value||'any'} type="button" onClick={()=>setRadius(value?Number(value):'')} aria-pressed={String(radius)===value} className={`h-10 rounded-lg text-[8px] font-semibold ${String(radius)===value?'bg-violet-500 text-white':'border border-white/[.07] text-[#8A90A0]'}`}>{label}</button>)}</div></section>}
