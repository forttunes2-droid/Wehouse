 ...
 return <DiscoveryShell active="homes" title="Find a place that fits" description="Homes, short stays, compatible roommates and trusted local help—together in one place." onNavigate={onNavigate}>
  <main className="mx-auto max-w-7xl space-y-4 px-4 py-5 sm:px-6 lg:px-8">
   <section className="grid grid-cols-2 border-b border-white/[.07]">
    ...
    <button type="button" onClick={()=>chooseStay('short_let')} className={`border-b-2 px-1 py-3 text-left transition ${stayType==='short_let'?'border-violet-400 text-white':'border-transparent text-[#777E8E]'}`}><span className="block text-[12px] font-bold">Short Stay</span><span className="mt-0.5 block text-[8px] text-[#666D7D]">Nightly · furnished</span></button>
   </section>
