...
export default function Search({onNavigate,savedIds,onToggleSave}:SearchProps){
 ...
 const modeLabel=stayType==='short_let'?'Short Stay':'Long Stay';
 ...
 return <DiscoveryShell active="homes" title="Homes" description="Choose Long Stay or Short Stay, then search by location, price and the details that matter." onNavigate={onNavigate}>
  <main className="mx-auto max-w-7xl space-y-4 px-4 py-5 sm:px-6 lg:px-8">
   <section className="grid grid-cols-2 gap-2 rounded-2xl border border-white/[.07] bg-[#11141C] p-1.5">
    <button type="button" onClick={()=>chooseStay('long_stay')} className={`rounded-xl px-4 py-3 text-left transition ${stayType==='long_stay'?'bg-violet-500 text-white':'text-[#858A99]'}`}><span className="block text-[11px] font-bold">Long Stay</span><span className={`mt-0.5 block text-[8px] ${stayType==='long_stay'?'text-white/70':'text-[#5E6474]'}`}>Annual rent</span></button>
    <button type="button" onClick={()=>chooseStay('short_let')} className={`rounded-xl px-4 py-3 text-left transition ${stayType==='short_let'?'bg-violet-500 text-white':'text-[#858A99]'}`}><span className="block text-[11px] font-bold">Short Stay</span><span className={`mt-0.5 block text-[8px] ${stayType==='short_let'?'text-white/70':'text-[#5E6474]'}`}>Nightly · furnished</span></button>
   </section>
  ...
  {showFilters&&<DiscoveryFilterSheet title={`Filter ${modeLabel}`} onClose={()=>setShowFilters(false)} onClear={clearFilters}>
   <div className="grid grid-cols-2 gap-2 rounded-2xl border border-white/[.07] bg-[#151922] p-1.5"><button type="button" onClick={()=>chooseStay('long_stay')} className={`rounded-xl px-3 py-2.5 text-[10px] font-semibold ${stayType==='long_stay'?'bg-violet-500 text-white':'text-[#7E8494]'}`}>Long Stay</button><button type="button" onClick={()=>chooseStay('short_let')} className={`rounded-xl px-3 py-2.5 text-[10px] font-semibold ${stayType==='short_let'?'bg-violet-500 text-white':'text-[#7E8494]'}`}>Short Stay</button></div>
