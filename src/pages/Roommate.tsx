...
export default function RoommateWorkspace({profile,onGoToChat,onEditProfile}:Props){
 ...
 async function save(){
  ...
  if(p.search_status==='active'){
   ...
   if(!result.error)setMatches(result.matches||[]);
  }
  setBusy(false);
 }
 ...
 async function stop(){const{prefs:p,error}=await stopRoommateSearch();if(error)return toast.error(error.message);setPrefs(p);setMatches([])}
  ...
  <main className="mx-auto max-w-6xl space-y-4 px-4 py-5 sm:px-6 lg:px-8">
   ...
   {(!prefs||editing)?<RoommatePreferencesPanel form={form} setForm={setForm} profileSchool={profile.school} busy={busy} onSave={save} onCancel={prefs?()=>setEditing(false):undefined}/>:<>
    <section className="rounded-2xl border border-white/[.07] bg-[#11141C] p-4">
     ...
     <div className="mt-4 flex flex-wrap gap-2"><button onClick={()=>setEditing(true)} className="rounded-xl border border-white/[.08] px-4 py-2.5 text-xs">Edit preferences</button>{matchingActive?<><button onClick={()=>void refresh()} disabled={busy} className="rounded-xl bg-violet-500 px-4 py-2.5 text-xs disabled:opacity-40">{busy?'Refreshing…':'Refresh matches'}</button><button onClick={()=>void stop()} className="rounded-xl border border-white/[.08] px-4 py-2.5 text-xs">Pause</button></>:<button onClick={()=>void start()} disabled={!canMatch||busy} className="rounded-xl bg-violet-500 px-4 py-2.5 text-xs disabled:opacity-40">Resume matching</button>}</div>
    </section>
    {matchingActive?<Matches rows={matches} onChat={onGoToChat} onInterest={interest}/>:<section className="rounded-2xl border border-dashed border-white/[.08] p-8 text-center"><p className="text-sm font-semibold">Matching is paused</p><p className="mt-1 text-[10px] text-[#686D7E]">Your preferences are saved. Resume when you want to appear in roommate discovery again.</p></section>}
   </>}
  </main>
 </DiscoveryShell>
}
