...
import { getRemainingRoommateSearches, trackRoommateSearch } from '@/lib/aiChat';
...
export default function Roommate({ profile }: RoommateProps) {
  ...
  // ─── Load remaining roommate searches ────────────
  useEffect(() => {
    async function loadRemaining() {
      const remaining = await getRemainingRoommateSearches(profile.user_id);
  ...
  const handleStartSearch = async () => {
    // Check if user has remaining searches or is premium
    const remaining = await getRemainingRoommateSearches(profile.user_id);
    if (remaining <= 0) {
    ...
    await trackRoommateSearch(profile.user_id);
    ...
    <div className="min-h-screen bg-transparent pb-20">
      ...
      {view === 'preview' && (
        <PreviewView
          ...
          onDismissUpsell={() => setShowPremiumUpsell(false)}
        />
      )}
      {view === 'edit' && (
        <EditView
  ...
  return (
    <>
      ...
      <div className="max-w-lg mx-auto px-5 space-y-4">
        ...
        {showPremiumUpsell && (
          <div className="bg-gradient-to-r from-amber-500/10 to-amber-600/5 border border-amber-500/20 rounded-2xl p-4">
            <div className="flex items-center gap-2 mb-2">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#F59E0B" strokeWidth="2"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>
              <span className="text-sm font-semibold text-white">Upgrade to Premium</span>
            </div>
            <p className="text-xs text-[#7A7A8C] mb-3">
              You've used all 3 free roommate searches. Upgrade for unlimited searches.
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => onDismissUpsell()}
                className="flex-1 h-9 rounded-xl bg-amber-500 text-white text-xs font-semibold"
              >
                Go Premium
              </button>
              <button
                onClick={() => onDismissUpsell()}
                className="h-9 px-3 rounded-xl text-xs text-[#7A7A8C] hover:text-white"
              >
                Later
              </button>
            </div>
        ...
        {!isSearchActive && (
          <div className="space-y-2">
            ...
            {!profile.is_premium && remainingSearches < 9999 && (
              <div className="flex items-center justify-between px-1">
                <span className="text-[10px] text-[#7A7A8C]">
                  {remainingSearches > 0 ? `${remainingSearches} free search${remainingSearches !== 1 ? 'es' : ''} remaining` : 'No free searches left'}
                </span>
                {!profile.is_premium && remainingSearches <= 1 && (
                  <span className="text-[10px] text-amber-400 font-medium">Premium for unlimited</span>
                )}
              </div>
            )}
            <button
              ...
              disabled={loading || (!profile.is_premium && remainingSearches <= 0 && !hasExpired)}
            ...
            >
              ...
              {hasExpired && hasMatches ? 'View Saved Matches' : remainingSearches <= 0 && !profile.is_premium ? 'Upgrade for Premium' : 'Find Roommate'}
            </button>
          </div>
        )}
