...
export default function RoommateWorkspace({profile,onGoToChat,onEditProfile}:Props){
 const[prefs,setPrefs]=useState<RoommatePreferences|null>(null),[matches,setMatches]=useState<any[]>([]),[hasMore,setHasMore]=useState(false),[form,setForm]=useState<Form>({...EMPTY,school_name:profile.school||''}),[editing,setEditing]=useState(false),[loading,setLoading]=useState(true),[busy,setBusy]=useState(false),[loadingMore,setLoadingMore]=useState(false),[interestBusy,setInterestBusy]=useState<string|null>(null);
 ...
 async function interest(match:any,status:'accepted'|'viewed'){if(interestBusy)return;setInterestBusy(match.id);const{conversationId,error}=await updateMatchStatus(match.id,status);if(error){setInterestBusy(null);return toast.error(error.message)}setMatches(current=>current.filter(row=>row.id!==match.id));setInterestBusy(null);if(status==='viewed'){toast.success('Moved down for now. Refresh matches to see this profile again.');return}if(conversationId){toast.success('It is a match — opening your chat');onGoToChat?.(conversationId)}else toast.success('Interest sent. Chat opens when they are interested too.')}
