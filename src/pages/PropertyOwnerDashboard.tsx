...
type PartnerTab = 'overview' | 'properties' | 'wallet' | 'earnings' | 'messages' | 'support';
...
const TABS: Array<{ key: PartnerTab; label: string; description: string }> = [
  ...
  { key: 'properties', label: 'My Properties', description: 'Request listings and view properties published by WeHouse' },
...
export default function PropertyOwnerDashboard({ profile }: Props) {
  ...
  return <div className="min-h-[100dvh] bg-[#09090D] text-white">
    ...
    <header className="sticky top-0 z-30 border-b border-white/[0.06] bg-[#09090D]/90 backdrop-blur-xl">
      <div className="px-4 py-4 lg:px-8"><h1 className="text-base font-bold lg:text-lg">{current.label}</h1><p className="mt-0.5 text-[10px] text-[#686A7D] lg:text-[11px]">{current.description}</p></div>
...
function OverviewTab({ profile, onTab }: { profile: Profile; onTab: (tab: PartnerTab) => void }) {
  ...
  return <div className="space-y-6">
    <section className="overflow-hidden rounded-3xl border border-violet-500/15 bg-gradient-to-br from-violet-500/[0.12] via-[#151520] to-[#101018] p-5 lg:p-7"><div className="grid gap-6 lg:grid-cols-[1.4fr_1fr] lg:items-end"><div><span className="rounded-full border border-violet-400/20 bg-violet-400/10 px-3 py-1 text-[10px] font-semibold text-violet-300">PROPERTY PARTNER</span><h2 className="mt-4 max-w-2xl text-2xl font-bold leading-tight lg:text-3xl">Your properties and verified earnings in one place.</h2><p className="mt-2 max-w-xl text-xs leading-relaxed text-[#9395A8]">Request a listing, follow WeHouse inspection progress, view published properties and track eligible earnings.</p></div><div className="flex flex-wrap gap-2 lg:justify-end"><button onClick={() => onTab('properties')} className="rounded-xl bg-violet-500 px-4 py-3 text-xs font-semibold hover:bg-violet-400">Submit property</button><button onClick={() => onTab('wallet')} className="rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-3 text-xs font-semibold text-[#C4C5D0]">Open wallet</button></div></div></section>
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-4"><Metric label="Published properties" value={data.properties}/><Metric label="Available" value={data.available}/><Metric label="Pending requests" value={data.pendingRequests}/><Metric label="Inspections" value={data.inspections}/></section>
