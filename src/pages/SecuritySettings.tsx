...
export default function SecuritySettings({profile,onBack,embedded=false,focus='all'}:Props){
  ...
  async function savePassword(){if(!currentPassword||!newPassword||!confirmPassword)return toast.error('Complete all password fields');if(newPassword!==confirmPassword)return toast.error('New passwords do not match');if(newPassword.length<15)return toast.error('Password must be at least 15 characters');setChanging(true);const {error}=await changePassword(currentPassword,newPassword,profile.email);setChanging(false);if(error)return toast.error(error.message);await logPasswordChange(profile.user_id,profile.auth_id);setCurrentPassword('');setNewPassword('');setConfirmPassword('');setShowPassword(false);toast.success('Password changed')}
  ...
  const content=<>
    ...
    {(focus==='all'||focus==='password')&&<section className="rounded-2xl border border-white/[.06] bg-[#11141C] p-4 sm:p-5"><div className="flex items-center justify-between gap-3"><div><h2 className="text-sm font-semibold">Password</h2><p className="mt-1 text-[10px] leading-5 text-[#6F7585]">Your password works with either your username or email. Google sign-in remains available separately.</p></div><button onClick={()=>setShowPassword(v=>!v)} className="rounded-xl border border-white/[.08] bg-white/[.02] px-3 py-2 text-[10px] font-semibold text-[#AEB3C1]">{showPassword?'Cancel':'Change password'}</button></div>{showPassword&&<div className="mt-4 space-y-3"><Field label="Current password" value={currentPassword} onChange={setCurrentPassword}/><Field label="New password" value={newPassword} onChange={setNewPassword}/><Field label="Confirm new password" value={confirmPassword} onChange={setConfirmPassword}/><button onClick={()=>void savePassword()} disabled={changing} className="w-full rounded-xl bg-violet-500 px-4 py-3 text-xs font-semibold disabled:opacity-50">{changing?'Changing…':'Save new password'}</button><div className="flex items-center gap-3 py-1"><span className="h-px flex-1 bg-white/[.06]"/><span className="text-[8px] uppercase tracking-[.14em] text-[#5F6574]">or</span><span className="h-px flex-1 bg-white/[.06]"/></div><button type="button" onClick={()=>void startPasswordRecovery()} disabled={changing} className="w-full rounded-xl border border-white/[.09] px-4 py-3 text-xs font-semibold text-[#D1D4DC] disabled:opacity-50">Forgot it or only use Google?</button><p className="text-center text-[9px] leading-4 text-[#626879]">This signs out this device and asks for the Google identity already linked to the account. No reset email is sent.</p></div>}</section>}
