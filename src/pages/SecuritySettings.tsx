...
type DeviceSession={id:string;device:string|null;browser:string|null;os:string|null;ip_address:string|null;is_active:boolean;is_current:boolean;login_time:string;last_seen:string|null;logout_time:string|null;trust_status:'pending'|'trusted'|'rejected'};
export default function SecuritySettings({profile,onBack,embedded=false,focus='all'}:Props){
  ...
  const load=useCallback(async()=>{setLoading(true);const [{data:{user}},{data,error}]=await Promise.all([supabase.auth.getUser(),supabase.from('user_sessions').select('id,device,browser,os,ip_address,is_active,is_current,login_time,last_seen,logout_time,trust_status').eq('user_id',profile.user_id).neq('trust_status','pending').order('login_time',{ascending:false}).limit(30)]);setEmailVerified(Boolean(user?.email_confirmed_at));if(error)toast.error('Devices could not be loaded');setSessions((data||[]) as DeviceSession[]);setLoading(false)},[profile.user_id]);
  ...
  const content=<>
    ...
    {(focus==='all'||focus==='sessions')&&<section className="rounded-2xl border border-white/[.06] bg-[#11141C] p-4 sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-sm font-semibold">Devices</h2><p className="mt-1 text-[10px] text-[#6F7585]">Devices that have signed in to your account.</p></div><button onClick={()=>void logoutAll()} className="rounded-xl border border-red-500/15 bg-red-500/[.05] px-3 py-2 text-[10px] font-semibold text-red-300">Log out everywhere</button></div><div className="mt-4 divide-y divide-white/[.05]">{loading?<p className="py-5 text-center text-[10px] text-[#696F7F]">Loading devices…</p>:sessions.length===0?<p className="py-5 text-center text-[10px] text-[#696F7F]">No device records yet.</p>:sessions.map(session=>{const current=session.id===currentSessionId;return <DeviceRow key={session.id} session={session} current={current} busy={sessionBusy===session.id} onSignOut={signOutDevice}/>})}</div></section>}
...
function DeviceRow({session,current,busy,onSignOut}:{session:DeviceSession;current:boolean;busy:boolean;onSignOut:(id:string)=>Promise<void>}){const state=current?'This device':session.is_active&&session.trust_status==='trusted'?'Active':'Signed out';const tone=current||session.trust_status==='trusted'&&session.is_active?'text-emerald-300 bg-emerald-500/[.08]':'text-[#858B9A] bg-white/[.05]';return <div className="py-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-xs font-semibold text-[#E4E6EC]">{session.device||'Device'} · {session.browser||'Browser'}</p><p className="mt-1 text-[9px] text-[#6E7586]">{session.os||'System unavailable'} · {new Date(session.login_time).toLocaleString()}</p></div><span className={`shrink-0 rounded-full px-2 py-1 text-[8px] font-semibold ${tone}`}>{state}</span></div>{!current&&session.trust_status==='trusted'&&session.is_active&&<button type="button" disabled={busy} onClick={()=>void onSignOut(session.id)} className="mt-3 h-9 rounded-xl border border-white/[.08] px-3 text-[9px] font-semibold text-[#A8ADBA] disabled:opacity-40">{busy?'Signing out…':'Sign out this device'}</button>}</div>}
