...
export default function SecuritySettings({profile,onBack,embedded=false,focus='all'}:Props){
  ...
  async function resetWithGoogle(){setChanging(true);await supabase.auth.signOut({scope:'local'}).catch(()=>{});window.location.assign('/?auth=recovery')}
