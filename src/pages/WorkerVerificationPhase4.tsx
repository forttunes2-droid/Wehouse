...
type Activation = {
  ...
  identity_status: string; identity_passed: boolean; evidence_saved: boolean; submitted: boolean; rejection_reason: string | null;
};
const EMPTY: Activation = { worker_status:'pending',live:false,profile_complete:false,payment_confirmed:false,gold_badge:false,identity_status:'not_started',identity_passed:false,evidence_saved:false,submitted:false,rejection_reason:null };
...
export default function WorkerVerificationPhase4({profile,onBack}:Props){
  ...
  useEffect(()=>{let dead=false;void(async()=>{let ref='';try{ref=localStorage.getItem(REF_KEY)||''}catch{}if(!ref)return;const result=await verifyPaymentWithRetry(ref,{purpose:'worker_verification'});if(dead)return;if(result.success){try{localStorage.removeItem(REF_KEY)}catch{}toast.success('Verification payment confirmed');await refresh()}})();return()=>{dead=true}},[]);

  const paid=Boolean(a.payment_confirmed??a.gold_badge), complete=a.identity_passed&&paid&&a.evidence_saved;
  ...
  return <Shell><Toaster position="top-center" richColors theme="dark"/><header className="border-b border-white/[.06] px-4 py-4"><div className="mx-auto flex max-w-2xl items-center gap-3"><button onClick={onBack} className="grid h-10 w-10 place-items-center rounded-xl border border-white/[.07]">←</button><div><p className="text-[9px] font-bold tracking-[.18em] text-violet-300">WEHOUSE · LOCAL SERVICE WORKER</p><h1 className="mt-1 text-lg font-bold">Professional verification</h1></div></div></header><main className="mx-auto max-w-2xl space-y-3 px-4 py-4">
    <Progress profile={a.profile_complete} verification={complete} review={live}/>
    {a.profile_complete&&!reviewing&&!live&&<WorkerVerificationChecklist identityPassed={a.identity_passed} paymentConfirmed={paid} skillVideoSaved={a.evidence_saved}/>} 
    {a.rejection_reason&&<Card eyebrow="REVIEW FEEDBACK" title="Changes needed" text={a.rejection_reason}/>} 
    {live?<Card eyebrow="LIVE" title="Your services are public" text="Your Local Service professional profile is available to customers."><Button label="Back to dashboard" onClick={onBack}/></Card>
    :reviewing?<Card eyebrow="3 · WEHOUSE REVIEW" title="Review in progress" text="WeHouse is reviewing your real professional evidence."><Status text="Your profile stays private until approval"/><Button label="Back to dashboard" onClick={onBack} secondary/></Card>
    :!a.profile_complete?<Card eyebrow="1 · PROFESSIONAL PROFILE" title="Complete your work profile" text="Add your service, experience, price and work location."><Button label="Complete professional profile" onClick={openProfile}/></Card>
    :!a.identity_passed?<WorkerIdentityCheck profile={profile} status={a.identity_status} onSaved={refresh}/>
    :!paid?<Card eyebrow="2 · WORKER VERIFICATION" title="Complete verification payment" text="One-time Worker verification fee."><Status text="Private face check complete" good/><div className="flex items-center justify-between rounded-xl border border-white/[.07] bg-black/10 px-4 py-3"><span className="text-[10px] text-[#717888]">Verification fee</span><strong>₦{fee.toLocaleString()}</strong></div><Button label={busy?'Opening Paystack…':'Continue to Paystack'} onClick={()=>void pay()} disabled={busy||fee<=0}/></Card>
    :!a.evidence_saved?<Card eyebrow="2 · WORKER VERIFICATION" title="Show your real work" text="Upload one short skill or completed-work video for private WeHouse review."><Status text="Identity and verification payment complete" good/><Upload label={certificatePath?'Certificate added':'Certificate · optional'} done={!!certificatePath} onClick={()=>certificateInput.current?.click()}/><input ref={certificateInput} type="file" accept="image/jpeg,image/png,image/webp,application/pdf" className="hidden" onChange={chooseCertificate}/><Upload label={videoPath?'Skill/work video added':'Skill/work demonstration · required'} done={!!videoPath} onClick={()=>videoInput.current?.click()}/><input ref={videoInput} type="file" accept="video/mp4,video/webm,video/quicktime" className="hidden" onChange={chooseVideo}/>{preview&&<video src={preview} controls playsInline className="max-h-64 w-full rounded-2xl bg-black object-contain"/>}<Button label={busy?'Saving…':'Save professional evidence'} onClick={()=>void saveEvidence()} disabled={busy||!videoPath}/></Card>
    :<Card eyebrow="3 · WEHOUSE REVIEW" title="Ready for review" text="Your identity, verification payment and professional work evidence are complete."><Button label={busy?'Submitting…':'Submit to WeHouse'} onClick={()=>void submit()} disabled={busy}/></Card>}
  </main></Shell>
}
...
function Progress({profile,verification,review}:{profile:boolean;verification:boolean;review:boolean}){const items=[['Professional profile',profile],['Verification',verification],['WeHouse review',review]] as const;return <section className="grid grid-cols-3 gap-2 rounded-2xl border border-white/[.06] bg-[#0F131A] p-3">{items.map(([label,done],i)=><div key={label} className={`rounded-xl border px-2 py-3 text-center ${done?'border-emerald-500/15 bg-emerald-500/[.05]':'border-white/[.05]'}`}><span className={`mx-auto grid h-7 w-7 place-items-center rounded-full text-[9px] font-bold ${done?'bg-emerald-500 text-[#04120A]':'bg-white/[.05] text-[#656C7B]'}`}>{done?'✓':i+1}</span><p className={`mt-2 text-[8px] ${done?'text-emerald-300':'text-[#676E7E]'}`}>{label}</p></div>)}</section>}
