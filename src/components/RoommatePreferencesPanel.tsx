...
type Props={form:Form;setForm:React.Dispatch<React.SetStateAction<Form>>;profileSchool?:string|null;busy:boolean;onSave:()=>void;onCancel?:()=>void};
 ...
 return <section className="border-y border-white/[.07] py-5">
  ...
  <div className="mt-5 space-y-5">
   ...
   <div className="grid gap-4 sm:grid-cols-2">
    <RoommateChoice title="Cleanliness" value={form.cleanliness} options={[['neat','Very neat'],['moderate','Balanced'],['relaxed','Relaxed']]} onChange={value=>set('cleanliness',value)}/>
    <RoommateChoice title="Home atmosphere" value={form.noise_level} options={[['quiet','Quiet'],['moderate','Balanced'],['loud','Social']]} onChange={value=>set('noise_level',value)}/>
    <RoommateChoice title="Visitors" value={form.visitors} options={[['rarely','Rarely'],['sometimes','Sometimes'],['often','Often']]} onChange={value=>set('visitors',value)}/>
    <RoommateChoice title="Stay length" value={form.stay_duration} options={[['3_months','3 months'],['6_months','6 months'],['1_year','1 year'],['1_year+','1+ years']]} onChange={value=>set('stay_duration',value)}/>
   </div>
   <div className="flex gap-2"><button onClick={onSave} disabled={busy} className="min-h-11 flex-1 rounded-xl bg-violet-500 px-4 text-xs font-semibold text-white disabled:opacity-40">{busy?'Saving…':'Save preferences'}</button>{onCancel&&<button onClick={onCancel} className="min-h-11 rounded-xl border border-white/[.08] px-4 text-xs">Cancel</button>}</div>
  </div>
 </section>
}
