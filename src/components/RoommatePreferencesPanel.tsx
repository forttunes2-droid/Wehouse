...
export default function RoommatePreferencesPanel({ form, setForm, profileSchool, busy, onSave, onCancel }: Props) {
  ...
  return <section className="mx-auto max-w-xl py-4">
    <header className="mb-6"><h2 className="text-lg font-semibold">Roommate preferences</h2><p className="mt-1 text-sm text-[#A7ADBA]">{["housing", "living", "review"].indexOf(step) + 1} of 3</p></header>
    <h3 ref={heading} tabIndex={-1} className="mb-5 scroll-mt-5 text-xl font-semibold outline-none">{titles[step]}</h3>
    ...
    {step === "living" && <>
      ...
      <section className="border-t border-white/10 pt-5"><h4 className="text-base font-semibold">Daily habits <span className="text-sm font-normal text-[#A7ADBA]">· optional</span></h4><div className="mt-4 grid grid-cols-2 gap-4">{optionalFields.map(field => <label key={field.key} className="block text-sm">{field.label}<select aria-label={field.label} className={input} value={String(form[field.key])} onChange={e => set(field.key, e.target.value)}><option value="">Not set</option>{field.options.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>)}</div></section>
    </>}
    {step === "review" && <>
