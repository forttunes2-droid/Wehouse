...
export default function PartnerHotelOperations({
  ...
  function messageWeHouse(booking?: Booking) {
    window.dispatchEvent(
      new CustomEvent("openSupportChat", {
        detail: {
          category: "hotel_operations",
          subject: booking
            ? `${hotel.name} · ${booking.booking_code || `Booking ${booking.booking_id}`}`
            : `${hotel.name} operations`,
          contextType: "hotel_operations",
          contextId: String(booking?.booking_id || hotel.hotel_id),
          contextSnapshot: {
            source_type: "hotel_operations",
            source_id: String(booking?.booking_id || hotel.hotel_id),
            hotel_id: hotel.hotel_id,
            hotel_name: hotel.name,
            hotel_status: hotel.status,
            booking_id: booking?.booking_id || null,
            booking_code: booking?.booking_code || null,
            guest_name: booking?.guest_name || booking?.profiles?.username || null,
            room_name: booking?.hotel_rooms?.room_type || null,
            rate_plan_name: booking?.rate_plan_name || null,
          ...
          {canHandleGuests ? (
            <section id="reservations" className="scroll-mt-20">
              ...
              <div className="mt-4 divide-y divide-white/[.06] border-y border-white/[.06]">
                {filteredBookings.map((row) => <ReservationRow key={row.booking_id} hotelName={hotel.name} row={row} busy={busy} chat={hotelChats.find((item) => Number(item.booking_id) === Number(row.booking_id))} onChat={setActiveChat} onWeHouse={accessRole === "owner" ? () => messageWeHouse(row) : undefined} transition={transition} />)}
...
function ReservationRow({ hotelName, row, busy, chat, onChat, onWeHouse, transition }: { hotelName: string; row: Booking; busy: boolean; chat?: HotelConversation; onChat: (chat: ActiveHotelChat) => void; onWeHouse?: () => void; transition: (row: Booking, status: "checked_in" | "checked_out") => Promise<void> }) {
