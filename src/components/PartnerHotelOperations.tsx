...
type HotelLinkInvite = { invitation_id:string; delivery:"direct"|"link"; role_key:string; permission_profile:string; status:string; expires_at:string };
function HotelTeam({ hotelId, grantableCapabilities }: { hotelId: number; grantableCapabilities: HotelCapability[] }) {
  const [rows, setRows] = useState<HotelTeamRow[]>([]); const [identifier, setIdentifier] = useState(""); const [role, setRole] = useState<"manager" | "front_desk">("front_desk"); const [saving, setSaving] = useState(false); const [removing, setRemoving] = useState<string | null>(null); const [linkInvites,setLinkInvites]=useState<HotelLinkInvite[]>([]);
  const load = useCallback(async () => {
    const [team,invites]=await Promise.all([
      supabase.rpc("get_my_hotel_team", { p_hotel_id: hotelId }),
      supabase.rpc("get_my_resource_invitations",{p_resource_type:"hotel",p_resource_id:String(hotelId)}),
    ]);
    if (team.error) return toast.error(team.error.message);
    setRows(Array.isArray(team.data) ? team.data : []);
    setLinkInvites(Array.isArray(invites.data)?invites.data.filter((row:any)=>row.delivery==="link"&&row.status==="pending"):[]);
  }, [hotelId]);
  ...
  async function revokeLink(invitationId:string){
    if(saving)return;
    setSaving(true);
