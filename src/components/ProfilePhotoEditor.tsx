...
export default function ProfilePhotoEditor({
  ...
  const input = useRef<HTMLInputElement>(null),
    ...
    [stage, setStage] = useState<"idle" | "preparing" | "uploading" | "saving">(
      "idle",
    );
  ...
  function choose(selected?: File) {
    if (!selected) return;
    if (!TYPES.has(selected.type))
      return toast.error("Choose a JPG, PNG or WebP image");
    if (selected.size > 10 * 1024 * 1024)
      return toast.error("Profile photo must be under 10MB");
    if (preview) URL.revokeObjectURL(preview);
    setFile(selected);
    ...
    <>
      ...
      <div className="flex items-center gap-4">
        ...
        >
          ...
          {!avatar ? (
            <span className="absolute inset-x-0 bottom-0 bg-black/65 py-1 text-[7px] font-semibold">
              ADD
            </span>
          ) : null}
        </button>
          ...
          <div className="mt-3 flex gap-2">
            ...
            {avatar && onRemove && (
              <button
                ...
                disabled={disabled || busy}
                className="rounded-full px-3 py-2 text-[10px] font-semibold text-red-300 disabled:opacity-40"
              >
                Remove
              </button>
            )}
          ...
          >
            <header className="flex h-14 items-center justify-between border-b border-white/[.08] px-4">
              <button
                onClick={close}
                disabled={busy}
                className="grid h-10 w-10 place-items-center text-xl disabled:opacity-40"
            ...
            <footer className="p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
              <button
                type="button"
                onClick={() => input.current?.click()}
                disabled={busy}
