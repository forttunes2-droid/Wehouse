...
export default function WorkerVerificationChecklist({ identityPassed, skillVideoSaved, identityRequired = false }: Props) {
  const items = [
    ...
    { label: 'Worker profile', done: true },
  ...
  return <section className="rounded-2xl border border-white/[.06] bg-[#0D1118] p-3">
    <div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-[8px] font-bold uppercase tracking-[.16em] text-[#686F7F]">WORKER REVIEW</p><p className="mt-1 text-[10px] text-[#8A90A0]">Free onboarding, identity check and real work evidence</p></div><span className="text-[9px] font-semibold text-violet-300">{items.filter(item=>item.done).length}/{items.length}</span></div>
