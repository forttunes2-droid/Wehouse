...
export default function StaffFinanceRecords({
  view,
  ...
  const load = useCallback(async () => {
    ...
    setRows(
      view === "payments"
        ...
        : view === "payouts"
          ? [
              ...(result.withdrawals || []).map((row: any) => ({
                ...row,
                _kind: "Withdrawal",
              })),
              ...(result.refunds || []).map((row: any) => ({
                ...row,
                _kind: "Refund",
              })),
            ]
          : [
              ...(result.commissions || []).map((row: any) => ({
                ...row,
                _kind: "Commission",
              })),
              ...(result.payment_protection || []).map((row: any) => ({
                ...row,
                _kind: "Payment Protection",
              })),
              ...(result.audit || []).map((row: any) => ({
                ...row,
                _kind: "Audit",
              })),
            ],
  ...
  async function executePayoutAction(
    ...
    const { data, error } = await supabase.functions.invoke("payout-withdrawal", {
      body: {
        ...
        reason: reason?.trim(),
        creator_elevation_id: creatorElevationId || null,
      },
    });
    setActing(null);
  ...
  function payoutAction(
    ...
    void executePayoutAction(row, action);
  }
              ...
              >
                  ...
                  <div className="mt-3 flex gap-2 border-t border-white/[.05] pt-3">
                    {pending && (
                      <>
                        <button
                          ...
                          className="min-h-10 flex-1 rounded-xl border border-rose-500/20 text-[9px] font-semibold text-rose-300 disabled:opacity-40"
                        >
                          Reject
                        </button>
                        <button
                          type="button"
                          disabled={acting === row.id}
                          onClick={() => payoutAction(row, "approve")}
                          className="min-h-10 flex-1 rounded-xl bg-violet-500 text-[9px] font-semibold disabled:opacity-40"
                        >
                          {acting === row.id
                            ? "Contacting Paystack…"
                            : "Review and send"}
                    ...
                    {processing && (
                      <button
                        type="button"
                        disabled={acting === row.id}
                        onClick={() => payoutAction(row, "reconcile")}
                        className="min-h-10 w-full rounded-xl border border-violet-500/25 bg-violet-500/[.06] text-[9px] font-semibold text-violet-200 disabled:opacity-40"
                      >
                        {acting === row.id
                          ? "Checking Paystack…"
                ...
                {withdrawal && pending && rejecting === row.id && (
                  <div className="mt-3 rounded-xl border border-rose-500/15 bg-rose-500/[.04] p-3">
                    <label
                      className="text-[9px] font-semibold text-rose-200"
                    ...
                    <div className="mt-2 flex gap-2">
                      ...
                      <button
                        type="button"
                        disabled={acting === row.id || !rejectionReason.trim()}
                        onClick={() => payoutAction(row, "reject")}
                        className="min-h-9 flex-1 rounded-lg bg-rose-500 text-[9px] font-semibold text-white disabled:opacity-40"
                      >
                        {acting === row.id ? "Rejecting…" : "Confirm rejection"}
                      </button>
                    </div>
                  </div>
                )}
...
function money(row: any) {
  const value =
    row.commission_amount ?? row.amount_total ?? row.amount ?? row.amount_payee;
  return value == null
    ? ""
...
function bankSummary(row: any) {
  ...
  return row.snapshot_bank_name
    ? `${row.snapshot_bank_name} · •••• ${number.slice(-4)} · ${row.snapshot_bank_account_name || "Verified account"}`
    : "";
}
