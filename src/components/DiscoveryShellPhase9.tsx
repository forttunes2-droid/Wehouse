...
export function DiscoveryToolbar({ value = '', onChange, placeholder = 'Search', showSearch = true, toolbarLabel, onFilters, filterCount = 0, locationLabel, locationDetail, locationActive = false, locationBusy = false, onLocation, onClearLocation, children }: ToolbarProps) {
  return <section className="rounded-3xl border border-white/[.065] bg-[#10131A] p-3 shadow-[0_18px_50px_rgba(0,0,0,.16)] sm:p-4">
    <div className="flex items-center gap-2">
      {showSearch ? <label className="relative min-w-0 flex-1"><SearchIcon /><input value={value} onChange={(event) => onChange?.(event.target.value)} placeholder={placeholder} className="h-12 w-full rounded-2xl border border-white/[.075] bg-[#171A22] pl-10 pr-4 text-xs text-white outline-none placeholder:text-[#5E6473] focus:border-violet-500/40" /></label> : <div className="min-w-0 flex-1"><p className="text-[9px] font-bold uppercase tracking-[.14em] text-[#777E8E]">{toolbarLabel || 'Choose location'}</p><p className="mt-1 text-[10px] text-[#5E6575]">Choose once. Results update immediately.</p></div>}
...
export function DiscoveryFilterSheet({ title = 'Filters', onClose, onClear, children }: { title?: string; onClose: () => void; onClear?: () => void; children: ReactNode; resultLabel?: string; resultDisabled?: boolean }) {
  return <>
    ...
    <section className="fixed inset-x-2 bottom-2 z-[89] flex max-h-[88dvh] flex-col overflow-hidden rounded-[28px] border border-white/[.09] bg-[#0F1218] shadow-2xl sm:inset-x-auto sm:bottom-auto sm:left-1/2 sm:top-1/2 sm:w-[min(92vw,39rem)] sm:-translate-x-1/2 sm:-translate-y-1/2">
      <div className="shrink-0 border-b border-white/[.055] px-4 pb-3 pt-4 sm:px-5">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1"><p className="text-[8px] font-bold uppercase tracking-[.19em] text-violet-400">FILTERS</p><h2 className="mt-1 text-lg font-bold">{title}</h2><p className="mt-1 text-[9px] text-[#666D7D]">Changes apply immediately.</p></div>
          <div className="flex shrink-0 items-center gap-2">{onClear&&<button type="button" onClick={onClear} className="h-10 rounded-xl px-3 text-[9px] font-semibold text-violet-300 hover:bg-white/[.04]">Reset</button>}<button type="button" aria-label="Close" onClick={onClose} className="grid h-10 w-10 place-items-center rounded-full border border-white/[.07] bg-white/[.035] text-lg text-[#A5AAB8]">×</button></div>
        </div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 pb-[max(1rem,env(safe-area-inset-bottom))] sm:px-5"><div className="space-y-4 [&_.grid-cols-2]:grid-cols-1 sm:[&_.grid-cols-2]:grid-cols-2">{children}</div></div>
    </section>
  </>;
}
