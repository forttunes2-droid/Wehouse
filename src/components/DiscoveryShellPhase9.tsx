...
export function DiscoveryToolbar({ value, onChange, placeholder, onFilters, filterCount = 0, locationLabel, locationDetail, locationActive = false, locationBusy = false, onLocation, onClearLocation, children }: ToolbarProps) {
  return <section className="rounded-3xl border border-white/[.065] bg-[#10131A] p-3 shadow-[0_18px_50px_rgba(0,0,0,.16)] sm:p-4">
    <div className="flex gap-2">
      <label className="relative min-w-0 flex-1"><SearchIcon /><input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="h-12 w-full rounded-2xl border border-white/[.075] bg-[#171A22] pl-10 pr-4 text-xs text-white outline-none placeholder:text-[#5E6473] focus:border-violet-500/40" /></label>
      {onFilters && <button type="button" onClick={onFilters} className={`relative flex h-12 shrink-0 items-center gap-2 rounded-2xl border px-3.5 text-[10px] font-semibold ${filterCount ? 'border-violet-500/25 bg-violet-500/[.09] text-violet-200' : 'border-white/[.075] bg-[#171A22] text-[#C4C8D2]'}`}><FilterIcon /><span className="hidden xs:inline">Filters</span>{filterCount > 0 && <span className="grid h-5 min-w-5 place-items-center rounded-full bg-violet-500 px-1 text-[8px] font-bold text-white">{filterCount}</span>}</button>}
    </div>
    {(onLocation || children) && <div className="mt-2.5 flex flex-wrap items-center gap-2">
      {onLocation && locationLabel && <button type="button" onClick={onLocation} disabled={locationBusy} className={`flex min-h-9 items-center gap-2 rounded-full border px-3 text-[9px] font-semibold transition disabled:opacity-50 ${locationActive ? 'border-violet-500/25 bg-violet-500/10 text-violet-200' : 'border-white/[.07] bg-black/10 text-[#8C92A1]'}`}><LocationIcon />{locationBusy ? 'Finding location…' : locationLabel}</button>}
      {locationActive && onClearLocation && <button type="button" onClick={onClearLocation} className="rounded-full px-2 py-2 text-[9px] font-semibold text-[#737A8A] hover:text-white">Clear location</button>}
      {children}
    </div>}
  ...
  return <>
    ...
    <section className="fixed inset-x-2 bottom-2 z-[89] flex max-h-[88dvh] flex-col overflow-hidden rounded-[28px] border border-white/[.09] bg-[#0F1218] shadow-2xl sm:inset-x-auto sm:bottom-auto sm:left-1/2 sm:top-1/2 sm:w-[min(92vw,39rem)] sm:-translate-x-1/2 sm:-translate-y-1/2">
      <div className="shrink-0 border-b border-white/[.055] px-4 pb-3 pt-4 sm:px-5">
        ...
        <div className="flex items-center justify-between gap-3"><div><p className="text-[8px] font-bold uppercase tracking-[.19em] text-violet-400">REFINE RESULTS</p><h2 className="mt-1 text-lg font-bold">{title}</h2><p className="mt-1 text-[9px] text-[#666D7D]">Choose only what matters. You can reset everything at any time.</p></div><button type="button" onClick={onClose} className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-white/[.07] bg-white/[.035] text-lg text-[#A5AAB8]">×</button></div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-5">
        <div className="space-y-4 [&_.grid-cols-2]:grid-cols-1 sm:[&_.grid-cols-2]:grid-cols-2">{children}</div>
      </div>
      <div className="grid shrink-0 grid-cols-[.8fr_1.4fr] gap-2 border-t border-white/[.055] bg-[#0D1016] p-3 sm:p-4">
        {onClear ? <button type="button" onClick={onClear} className="h-12 rounded-2xl border border-white/[.08] text-[10px] font-semibold text-[#9CA2B1]">Reset</button> : <span />}
        <button type="button" onClick={onClose} disabled={resultDisabled} className="h-12 rounded-2xl bg-violet-500 px-3 text-[10px] font-semibold text-white disabled:cursor-not-allowed disabled:bg-white/[.06] disabled:text-[#5F6676]">{resultDisabled ? 'No matches' : (resultLabel || 'Show results')}</button>
      </div>
    </section>
  </>;
}
...
function FilterIcon(){return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M4 6h16M7 12h10M10 18h4"/></svg>}
