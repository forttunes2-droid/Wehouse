...
export default function PropertyAccessRecorder({ code, expiresAt, recordedFile, onRecorded }: Props) {
  ...
  return <section className="rounded-2xl border border-violet-500/20 bg-violet-500/[.045] p-4 md:col-span-2">
    ...
    {!recordedFile && <>
      {cameraReady && <div className="relative mt-3 overflow-hidden rounded-2xl bg-black"><video ref={videoRef} muted playsInline className="aspect-video w-full object-cover"/>{recording && <span className="absolute left-3 top-3 rounded-full bg-red-500 px-2.5 py-1 text-[9px] font-bold">● LIVE · {seconds}s</span>}</div>}
