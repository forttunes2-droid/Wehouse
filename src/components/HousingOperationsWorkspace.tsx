...
export default function HousingOperationsWorkspace() {
  ...
  async function checkInHotel() {
    ...
    const { error } = await supabase.rpc('confirm_hotel_check_in_by_code', { p_booking_code: verifiedBooking.code });
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-2xl border border-violet-500/10 bg-violet-500/[.03] p-4">
      ...
      {verifiedBooking && <div className={`mt-3 rounded-xl border p-3 ${verifiedBooking.valid?'border-emerald-500/15 bg-emerald-500/[.04]':'border-amber-500/20 bg-amber-500/[.04]'}`}><div className="flex items-center justify-between gap-3"><div><p className={`text-xs font-bold tracking-wide ${verifiedBooking.valid?'text-emerald-300':'text-amber-300'}`}>{verifiedBooking.code}</p><p className="mt-1 text-[10px] font-semibold">{verifiedBooking.property_name}</p></div><span className={`rounded-full border px-2 py-1 text-[8px] capitalize ${verifiedBooking.valid?'border-emerald-500/20 text-emerald-300':'border-amber-500/20 text-amber-300'}`}>{verifiedBooking.valid?'Valid':'Not actionable'}</span></div><div className="mt-3 grid grid-cols-2 gap-2"><Info label="Customer" value={verifiedBooking.customer_name || '—'} /><Info label="Phone" value={verifiedBooking.customer_phone || '—'} /><Info label="Booking status" value={String(verifiedBooking.status || '—').replace(/_/g,' ')} /><Info label="Payment" value={String(verifiedBooking.payment_status || '—').replace(/_/g,' ')} /></div>{verifiedBooking.kind === 'housing' && verifiedBooking.can_handover && verifiedHousingRow && <button type="button" onClick={() => { setVerifiedMoveInCode(String(verifiedBooking.code)); setSelected(verifiedHousingRow); }} className="mt-3 h-11 w-full rounded-xl bg-emerald-500 text-[10px] font-semibold text-[#03100B]">Continue to verified property handover</button>}{verifiedBooking.kind === 'housing' && verifiedBooking.valid && !verifiedBooking.can_handover && <p className="mt-3 rounded-xl bg-amber-500/[.06] p-3 text-[9px] leading-5 text-amber-200">The booking exists, but handover is blocked until its reservation, inspection and rent state are ready.</p>}{verifiedBooking.kind === 'hotel' && <><p className="mt-3 text-[9px] text-[#8A909F]">Stay: {verifiedBooking.check_in ? new Date(verifiedBooking.check_in).toLocaleDateString() : '—'} → {verifiedBooking.check_out ? new Date(verifiedBooking.check_out).toLocaleDateString() : '—'} · {verifiedBooking.guest_count || 1} guest(s)</p>{verifiedBooking.can_check_in?<button type="button" disabled={checkingInHotel} onClick={()=>void checkInHotel()} className="mt-3 h-11 w-full rounded-xl bg-emerald-500 text-[10px] font-semibold text-[#03100B] disabled:opacity-40">{checkingInHotel?'Checking in…':'Confirm hotel check-in'}</button>:<p className="mt-3 rounded-xl bg-amber-500/[.06] p-3 text-[9px] leading-5 text-amber-200">This code cannot check in a guest in its current payment, date or booking state.</p>}</>}</div>}
    </section>
...
function HousingCase({ row, bookingCode, back }: { row: any; bookingCode: string | null; back: () => void }) {
  ...
  return <div className="space-y-4">
    ...
    {canMoveIn && !bookingCode && <section className="rounded-2xl border border-amber-500/15 bg-amber-500/[.035] p-4"><h4 className="text-sm font-semibold text-amber-300">Booking code required</h4><p className="mt-1 text-[10px] leading-5 text-[#85808A]">Open Find a booking in the Operations Inbox and verify the code shown by the customer. A property cannot become Occupied from this record alone.</p></section>}
