...
<button onClick={openSupport} className="text-[9px] font-semibold text-violet-300">Need WeHouse?</button>
   ...
   {messages.map(msg=>{const mine=msg.sender_id===profile.user_id;return <div key={msg.id} className={`flex ${mine?'justify-end':'justify-start'}`}>
...
<BookingAttachment key={`${msg.id}-${i}`} url={url}/>)}{msg.content&&<MessageContent content={msg.content}/>}<p className={`mt-1 text-[8px] ${mine?'text-violet-100/70':'text-[#5C6070]'}`}>{new Date(msg.created_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}{mine&&msg.is_read?' · Seen':''}</p>
</div>
...
function ConversationIdentitySheet({booking,isWorker,name,avatar,presence,onClose}:{booking:any;isWorker:boolean;name:string;avatar?:string|null;presence:string;onClose:()=>void}){const viewingWorker=!isWorker;const details=viewingWorker?[['Service',booking?.service_type||booking?.worker_occupation||'Local service'],['Verification',booking?.worker_verified===false?'Not verified':'WeHouse Worker'],['Booking',booking?.booking_code?'#'+booking.booking_code:'Active conversation']]:[['Connection','Service customer'],['Booking',booking?.booking_code?'#'+booking.booking_code:'Active conversation'],['Privacy','Booking information only']];return <div className="fixed inset-0 z-[75] flex items-end bg-black/75 backdrop-blur-sm sm:items-center sm:justify-center" onClick={onClose}><section className="w-full rounded-t-[32px] bg-[#11151D] px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-4 shadow-2xl sm:max-w-sm sm:rounded-[32px]" onClick={e=>e.stopPropagation()}><div className="mx-auto mb-5 h-1 w-10 rounded-full bg-white/15"/><ChatAvatar name={name} src={avatar}/><h2 className="mt-3 text-center text-xl font-bold">{name}</h2><p className="mt-1 text-center text-[10px] text-emerald-300">{presence||'WeHouse conversation'}</p><div className="mt-6 divide-y divide-white/[.06] border-y border-white/[.06]">{details.map(([label,value])=><div key={label} className="flex items-center justify-between gap-5 py-4"><span className="text-[10px] text-[#707788]">{label}</span><span className="text-right text-xs font-semibold text-[#E4E6EC]">{value}</span></div>)}</div><p className="mt-5 text-center text-[9px] leading-5 text-[#646B7B]">{viewingWorker?'Professional details shown here are tied to this Worker booking.':'Only information needed for this Worker booking is shared.'}</p><button onClick={onClose} className="mt-4 h-12 w-full rounded-2xl bg-violet-500 text-xs font-semibold">Back to conversation</button></section></div>}
