  ...
  return <>
    <section className="relative overflow-hidden bg-[#11141C] sm:m-4 sm:rounded-3xl">
      ...
      >
        ...
        >
          <ListingMediaImage reference={image} alt={`${title} · photo ${index + 1}`} loading={index === 0 ? 'eager' : 'lazy'} fetchPriority={index === 0 ? 'high' : 'auto'} draggable={false} className="aspect-[4/3] w-full select-none object-cover sm:aspect-[16/9]" />
        </button>)}
      </div>
    ...
    {fullscreen && <div className="fixed inset-0 z-[100200] flex h-[100dvh] flex-col bg-black" role="dialog" aria-modal="true" aria-label="Apartment photo viewer">
      <header className="flex min-h-14 shrink-0 items-center justify-between px-4 pb-2 pt-[max(.5rem,env(safe-area-inset-top))]"><span className="text-xs text-white/70">{activeIndex + 1} of {images.length}</span><button type="button" onClick={() => setFullscreen(false)} className="grid h-11 w-11 place-items-center rounded-full bg-white/10 text-xl" aria-label="Close photo viewer">×</button></header>
      ...
      {images.length > 1 && <div className="flex shrink-0 gap-2 overflow-x-auto px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-4 scrollbar-hide">{images.map((image, index) => <button key={`${image}-thumb-${index}`} type="button" onClick={() => moveFullscreenTo(index)} className={`h-16 w-20 shrink-0 overflow-hidden rounded-xl border-2 ${index === activeIndex ? 'border-violet-400' : 'border-transparent opacity-60'}`}><ListingMediaImage reference={image} alt="" className="h-full w-full object-cover" /></button>)}</div>}
      {images.length === 1 ? <div className="h-[env(safe-area-inset-bottom)] shrink-0"/> : null}
    </div>}
  </>;
}
