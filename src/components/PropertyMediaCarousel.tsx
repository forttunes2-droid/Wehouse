...
export default function PropertyMediaCarousel({ images, videos = [], title, children }: Props) {
  ...
  return <>
    ...
    {fullscreen && <div className="fixed inset-0 z-[100200] flex h-[100dvh] flex-col bg-black" role="dialog" aria-modal="true" aria-label={`${title} WeHouse media viewer`}>
      ...
      <div ref={fullscreenRailRef} onScroll={(event) => { const rail=event.currentTarget; if(rail.clientWidth)setActiveIndex(Math.min(items.length-1,Math.max(0,Math.round(rail.scrollLeft/rail.clientWidth)))) }} className="flex min-h-0 flex-1 snap-x snap-mandatory overflow-x-auto overscroll-x-contain scrollbar-hide">{items.map((item,index)=><div key={`${item.reference}-full-${index}`} className="relative flex h-full w-full min-w-full flex-none shrink-0 snap-center items-center justify-center overflow-hidden">{item.kind === 'video' ? <ResolvedVideo reference={item.reference} active={index === activeIndex}/> : <><ListingMediaImage reference={item.reference} alt="" aria-hidden="true" className="absolute inset-0 h-full w-full scale-110 object-cover opacity-25 blur-2xl"/><ListingMediaImage reference={item.reference} alt={`${title} · photo ${index + 1}`} className="relative block max-h-full w-full object-contain" /></>}</div>)}</div>
