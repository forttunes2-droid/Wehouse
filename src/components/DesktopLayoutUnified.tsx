        ...
        <aside className={`fixed left-0 top-0 z-40 hidden h-screen flex-col border-r border-white/[.05] bg-[#080A0F] transition-all duration-300 lg:flex ${collapsed ? 'w-[72px]' : 'w-[240px]'}`}>
          <div className="flex h-16 items-center border-b border-white/[.05] px-4">
            <button onClick={() => onNavigate('home')} className="flex min-w-0 items-center" aria-label="WeHouse home">
              <span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-violet-500 text-white shadow-lg shadow-violet-500/10"><HomeIcon /></span>
              {!collapsed && <span className="ml-3 truncate text-sm font-bold tracking-tight">WeHouse</span>}
            </button>
            <button onClick={() => setCollapsed(value => !value)} className="ml-auto grid h-8 w-8 place-items-center rounded-xl border border-white/[.06] bg-white/[.025] text-[#777D8F] hover:text-white" aria-label={collapsed ? 'Expand navigation' : 'Collapse navigation'}>{collapsed ? '›' : '‹'}</button>
          </div>

          <nav className="flex-1 space-y-1 overflow-y-auto px-2 py-3">
            {navItems.map(item => {
              const active = userDesktopItemActive(item, activePage);
              return <button key={item.id} onClick={() => onNavigate(item.id)} title={collapsed ? item.label : undefined} aria-current={active ? 'page' : undefined} className={`relative flex h-11 w-full items-center gap-3 rounded-xl px-3 transition ${active ? 'border border-violet-500/20 bg-violet-500/10 text-violet-300' : 'text-[#858A9B] hover:bg-white/[.035] hover:text-white'}`}><span className="shrink-0">{item.icon(active)}</span>{!collapsed && <span className="min-w-0 flex-1 truncate text-left text-[12px] font-semibold">{item.label}</span>}{!collapsed && item.badge ? <span className="grid h-5 min-w-5 place-items-center rounded-full bg-red-500 px-1 text-[9px] font-bold text-white">{item.badge > 9 ? '9+' : item.badge}</span> : null}</button>;
            })}
          </nav>

          <div className="border-t border-white/[.05] p-3"><div className={`flex items-center gap-3 ${collapsed ? 'justify-center' : ''}`}><div className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-xl bg-[#151923] text-xs font-bold text-white">{userAvatar ? <img src={userAvatar} alt="" className="h-full w-full object-cover" /> : initials}</div>{!collapsed && <div className="min-w-0"><p className="truncate text-xs font-semibold">{userName || 'WeHouse user'}</p><p className="mt-0.5 truncate text-[9px] capitalize text-[#666C7E]">{role.replace(/_/g, ' ')}</p></div>}</div></div>
        </aside>
      )}
