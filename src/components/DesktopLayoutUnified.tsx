...
const ROOT_PAGES = new Set<NavPage>([
  'home', 'explore', 'search', 'saved', 'messages', 'profile',
...
export default function DesktopLayoutUnified({
  ...
  function back() {
    ...
    else onNavigate(operational ? roleRoot(role) : 'home');
  }

  return (
    <div className="min-h-[100dvh] min-w-0 bg-[#080A0F] text-white">
      {!operational && (
        <aside className={`fixed left-0 top-0 z-40 hidden h-screen flex-col border-r border-white/[.05] bg-[#080A0F] transition-all duration-300 lg:flex ${collapsed ? 'w-[72px]' : 'w-[240px]'}`}>
          <div className="flex h-16 items-center border-b border-white/[.05] px-4">
            ...
            <button
              ...
              className="ml-auto grid h-8 w-8 place-items-center rounded-xl border border-white/[.06] bg-white/[.025] text-[#777D8F] hover:text-white"
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {!collapsed && item.badge ? <span className="grid h-5 min-w-5 place-items-center rounded-full bg-red-500 px-1 text-[9px] font-bold text-white">{item.badge > 9 ? '9+' : item.badge}</span> : null}
                </button>
              );
            })}
          </nav>
          ...
          <div className="border-t border-white/[.05] p-3">
            <div className={`flex items-center gap-3 ${collapsed ? 'justify-center' : ''}`}>
              <div className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-xl bg-[#151923] text-xs font-bold text-white">
                {userAvatar ? <img src={userAvatar} alt="" className="h-full w-full object-cover" /> : initials}
              </div>
              {!collapsed && <div className="min-w-0"><p className="truncate text-xs font-semibold">{userName || 'WeHouse user'}</p><p className="mt-0.5 truncate text-[9px] capitalize text-[#666C7E]">{role.replace(/_/g, ' ')}</p></div>}
            </div>
          </div>
        </aside>
      )}
        ...
        {showBack && (
          <div className="sticky top-0 z-[55] border-b border-white/[.05] bg-[#080A0F]/95 px-4 py-2 backdrop-blur-xl sm:px-5 lg:px-8">
            <div className="mx-auto flex min-h-10 max-w-7xl items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 text-[10px] text-[#676D7E]"><span className="font-bold text-[#AAB0BE]">WeHouse</span><span>/</span><span className="capitalize">{activePage.replace(/_/g, ' ')}</span></div>
            </div>
          </div>
        )}

        {!showBack && !operational && (
          <header className="sticky top-0 z-30 hidden h-14 items-center justify-between border-b border-white/[.05] bg-[#080A0F]/92 px-6 backdrop-blur-xl lg:flex">
            <div className="text-[12px] font-semibold text-[#AAB0BE]">WeHouse</div>
            <div className="text-[10px] capitalize text-[#666C7E]">{activePage.replace(/_/g, ' ')}</div>
          </header>
        )}
...
function roleRoot(role: string): NavPage {
...
function HomeIcon() {
  return <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 11.5 12 4l9 7.5"/><path d="M5.5 10.5V20h13v-9.5"/><path d="M9.5 20v-5h5v5"/></svg>;
}
