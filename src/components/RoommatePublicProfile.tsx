      ...
      <main className="mx-auto max-w-xl px-5 pb-12 pt-6">
            ...
            {highlights.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {highlights.map((item) => (
                  <span
                    key={item}
                    className="rounded-full border border-violet-500/15 bg-violet-500/[.06] px-3 py-1.5 text-[9px] font-semibold text-violet-200"
                  >
                    {item}
                  </span>
                ))}
        ...
        <section className="divide-y divide-white/[.06] border-b border-white/[.06]">
          {person.preferredArea && (
            <Detail label="Preferred area" value={person.preferredArea} />
          )}{" "}
...
function Detail({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-6 py-4">
      ...
      <strong className="max-w-[68%] text-right text-[11px] font-semibold text-[#D7DAE3]">
        {value}
      </strong>
    </div>
  );
