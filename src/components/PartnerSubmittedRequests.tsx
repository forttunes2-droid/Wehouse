...
function AccessChallenge({profile,request,onUpdated}:{profile:Profile;request:RequestRow;onUpdated:()=>Promise<void>}){const[busy,setBusy]=useState(false);const verified=request.access_evidence_status==='verified';const submitted=request.access_evidence_status==='submitted';const passed=submitted||verified;const expired=!!request.access_challenge_expires_at&&new Date(request.access_challenge_expires_at).getTime()<=Date.now();async function upload(file:File|null){if(!file)return;if(file.size>100*1024*1024)return toast.error('Access video must be under 100MB');if(!['video/mp4','video/webm','video/quicktime'].includes(file.type))return toast.error('Use an MP4, WebM or MOV video');setBusy(true);const extension=file.name.split('.').pop()?.toLowerCase()||'mp4';const path=`${profile.user_id}/${request.id}/${crypto.randomUUID()}.${extension}`;const result=await supabase.storage.from('property-access-private').upload(path,file,{contentType:file.type,upsert:false});if(result.error){setBusy(false);return toast.error(result.error.message||'Access video could not be uploaded')}const{error}=await supabase.rpc('submit_my_property_access_evidence',{p_request_id:request.id,p_video_path:path});if(error)await supabase.storage.from('property-access-private').remove([path]);setBusy(false);if(error)return toast.error(error.message);toast.success('Property access evidence submitted privately');await onUpdated()}async function refreshCode(){setBusy(true);const{error}=await supabase.rpc('refresh_my_property_access_challenge',{p_request_id:request.id});setBusy(false);if(error)return toast.error(error.message);toast.success('New one-use code created');await onUpdated()}return <section className={`rounded-3xl border p-5 ${passed?'border-emerald-500/15 bg-emerald-500/[.04]':'border-violet-500/20 bg-violet-500/[.05]'}`}><p className="text-[9px] font-bold uppercase tracking-[.16em] text-violet-300">PROPERTY ACCESS CHECK</p><h3 className="mt-2 text-sm font-semibold">{verified?'Access confirmed':submitted?'Recording received':'Show that you can access this property'}</h3><p className="mt-2 text-[10px] leading-5 text-[#858A9A]">{passed?'The one-use code has been consumed and is no longer valid. Your private recording is attached to this submission only.':'At the property, record one continuous video showing the one-use code at the entrance and continue inside. The recording stays private and separate from public property media.'}</p>{!passed&&request.access_challenge_code&&<div className="mt-4 rounded-2xl border border-white/[.08] bg-black/15 p-4 text-center"><p className="text-[8px] uppercase tracking-[.16em] text-[#6C7181]">One-use recording code</p><p className="mt-2 text-2xl font-black tracking-[.18em] text-white">{request.access_challenge_code}</p>{request.access_challenge_expires_at&&<p className="mt-2 text-[8px] text-[#676C7C]">{expired?'Expired':`Use before ${new Date(request.access_challenge_expires_at).toLocaleString()}`}</p>}</div>}{!passed&&(expired?<button type="button" onClick={()=>void refreshCode()} disabled={busy} className="mt-4 h-12 w-full rounded-xl bg-violet-500 text-xs font-semibold disabled:opacity-40">{busy?'Creating…':'Create a new one-use code'}</button>:<label className="mt-4 flex h-12 cursor-pointer items-center justify-center rounded-xl bg-violet-500 text-xs font-semibold"><span>{busy?'Uploading privately…':'Record or choose access video'}</span><input type="file" accept="video/mp4,video/webm,video/quicktime" capture="environment" disabled={busy} className="hidden" onChange={e=>{void upload(e.target.files?.[0]||null);e.target.value=''}}/></label>)}{passed&&<p className="mt-3 text-[10px] font-semibold text-emerald-300">{verified?'Accepted for field verification':'Waiting for WeHouse review'}</p>}</section>}
