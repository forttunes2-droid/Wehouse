...
function RequestDetail({ request, onBack, onContact }: { request: RequestRow; onBack: () => void; onContact: () => void }) {
  ...
  const progress = request.completed_at ? 3 : request.scheduled_date ? 2 : 1;
  return <div className="space-y-5">
    <button type="button" onClick={onBack} className="flex items-center gap-2 text-xs text-[#A1A3B1]">← Submitted properties</button>
    ...
    <section className="rounded-3xl border border-white/[.06] bg-[#111119] p-5"><h3 className="text-sm font-semibold">Inspection progress</h3><div className="mt-5 grid grid-cols-3 gap-2">{['Request received', 'Visit scheduled', 'Inspection complete'].map((label, index) => <div key={label} className="text-center"><div className={`mx-auto grid h-9 w-9 place-items-center rounded-full text-[10px] font-bold ${progress > index ? 'bg-violet-500 text-white' : 'bg-white/[.05] text-[#5F6272]'}`}>{progress > index ? '✓' : index + 1}</div><p className={`mt-2 text-[8px] ${progress > index ? 'text-[#CFD0D9]' : 'text-[#5F6272]'}`}>{label}</p></div>)}</div>{request.scheduled_date && <p className="mt-4 rounded-xl bg-violet-500/[.06] p-3 text-[10px] text-violet-200">Visit: {new Date(request.scheduled_date).toLocaleString()}</p>}</section>
...
function Status({ value }: { value: string }) { const good = ['approved', 'completed', 'published'].includes(value.toLowerCase()); return <span className={`shrink-0 rounded-full px-2 py-1 text-[8px] font-semibold capitalize ${good ? 'bg-emerald-500/10 text-emerald-300' : 'bg-amber-500/10 text-amber-300'}`}>{friendly(value)}</span>; }
