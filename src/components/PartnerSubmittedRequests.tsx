  ...
  return <div className="space-y-6">
    ...
    {!creating&&<section>
      ...
      {loading ? <Loader /> : visibleRequests.length === 0 ? <Empty filter={filter} /> : <div className="space-y-3">{visibleRequests.map(request => <button key={request.id} type="button" onClick={() => setSelected(request)} className="flex w-full items-center gap-3 rounded-2xl border border-white/[.06] bg-[#111119] p-3 text-left transition active:scale-[.99] sm:p-4">
        <div className="h-20 w-24 shrink-0 overflow-hidden rounded-xl bg-[#191A24]">{request.photo_urls?.[0] ? <ListingMediaImage reference={request.photo_urls[0]} alt="" className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center text-[9px] text-[#595C6D]">No photo</div>}</div>
        <div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><p className="truncate text-xs font-semibold">{request.property_address || request.property_type || 'Property'}</p><Status request={request} /></div><p className="mt-1 truncate text-[9px] text-[#696C7D]">{[request.property_city, request.property_state].filter(Boolean).join(', ')}</p><div className="mt-2 flex flex-wrap items-center gap-2 text-[8px] text-[#777A8B]"><span>{request.request_code || 'Request sent'}</span>{request.submission_batch_id && Number(batchCounts.get(request.submission_batch_id) || 0) > 1 && <span className="rounded-full bg-violet-500/10 px-2 py-1 text-violet-300">Property {request.submission_batch_position} of {batchCounts.get(request.submission_batch_id)}</span>}<span className="ml-auto text-violet-300">View details →</span></div></div>
      </button>)}</div>}
    </section>}
  </div>;
}
...
function journeyNext(stage:string) {
  ...
  if(stage==='inspection_ready')return 'Next: WeHouse assigns Field Operations member for an independent visit.';
...
function partnerState(request:RequestRow){
  const stage=request.lifecycle_stage||'access_required';
  const value=stage==='live'?'live':['changes_requested','rejected'].includes(stage)?stage:String(request.status||'pending');
  const tone=value==='live'||value==='completed'||value==='approved'?'bg-emerald-500/10 text-emerald-300':value==='rejected'?'bg-red-500/10 text-red-300':['changes_requested','pending'].includes(value)?'bg-amber-500/10 text-amber-300':'bg-violet-500/10 text-violet-300';
