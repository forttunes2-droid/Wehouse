        ...
        {mode === 'setup' && (
          <>
            ...
            <form onSubmit={handleSetup} className="px-6 pb-6 space-y-3">
              ...
              <button type="submit" disabled={isLoading} className="w-full h-11 rounded-xl bg-gradient-to-r from-purple-500 to-[#7C3AED] text-white text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-40">{isLoading ? 'Saving...' : 'Set Password'}</button>
            </form>
          </>
        )}
        ...
        {mode === 'change' && (
          <>
            ...
            <form onSubmit={handleChange} className="px-6 pb-6 space-y-3">
              <input type="password" value={oldPassword} onChange={(e) => setOldPassword(e.target.value)} placeholder="Current password" autoFocus className="w-full h-11 rounded-xl bg-[#1A1A24] border border-[#232330] text-white text-sm px-4 placeholder-[#5C5E72] focus:border-purple-500/50 focus:outline-none transition-colors" />
              <input type="password" value={password} onChange={(e) => setPasswordInput(e.target.value)} placeholder="New password (min 6 characters)" className="w-full h-11 rounded-xl bg-[#1A1A24] border border-[#232330] text-white text-sm px-4 placeholder-[#5C5E72] focus:border-purple-500/50 focus:outline-none transition-colors" />
              <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Confirm new password" className="w-full h-11 rounded-xl bg-[#1A1A24] border border-[#232330] text-white text-sm px-4 placeholder-[#5C5E72] focus:border-purple-500/50 focus:outline-none transition-colors" />
              {displayError && <p className="text-xs text-red-400 flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><path d="M12 8v4M12 16h.01" /></svg>{displayError}</p>}
              <button type="submit" disabled={isLoading} className="w-full h-11 rounded-xl bg-gradient-to-r from-purple-500 to-[#7C3AED] text-white text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-40">{isLoading ? 'Changing...' : 'Change Password'}</button>
