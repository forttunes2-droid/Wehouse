...
export default function AccountIdentityReviewQueue({ accountRole }: { accountRole: Role }) {
  ...
  async function open(row:Row){
    ...
    setSelected(row);setEvidence(null);setUrl("");setNotes("");setReason("");setOpening(true);setOpenError("");
    try {
      const {data,error}=await withTimeout(supabase.rpc("get_review_account_identity_check",{p_user_id:row.user_id}),15000,'Review could not be loaded.');
      if(error)throw error;
      const next=(data||null) as Evidence|null;
      if(current!==request.current)return;
      if(!next?.photo_path)throw new Error('The private reference is unavailable.');
      const signed=await withTimeout(supabase.storage.from("worker-identity-private").createSignedUrl(next.photo_path,300),15000,'Reference could not be loaded.');
      if(signed.error||!signed.data?.signedUrl)throw new Error('The private reference could not be opened.');
      if(current!==request.current)return;
      setEvidence(next);setUrl(signed.data.signedUrl);
    }catch{if(current===request.current)setOpenError('The private reference could not be opened. Try again before reviewing this submission.');}
  ...
  if(selected)return <section className="rounded-3xl border border-violet-500/15 bg-[#10131B] p-4 sm:p-5">
    <button type="button" onClick={()=>{request.current++;setSelected(null);setViewer(false);}} className="text-[10px] font-semibold text-violet-300">← Identity queue</button>
    ...
    {opening&&<p role="status" className="mt-4 text-xs text-[#9298A6]">Opening private reference…</p>}
    {openError&&<div role="alert" className="mt-4 text-xs text-amber-200"><p>{openError}</p><button onClick={()=>void open(selected)} className="min-h-11 text-violet-300">Try again</button></div>}
    ...
    <div className="mt-3 grid grid-cols-2 gap-2"><button type="button" disabled={saving||opening||!url||evidence?.status!=="pending_review"} onClick={()=>void decide("approved")} className="min-h-11 rounded-xl bg-violet-500 text-[10px] font-semibold disabled:opacity-35">Approve identity</button><button type="button" disabled={saving||opening||!evidence} onClick={()=>void decide("rejected")} className="min-h-11 rounded-xl bg-red-500/15 text-[10px] font-semibold text-red-300 disabled:opacity-35">Reject with reason</button></div>
