...
export default function WorkerActivationHomeV3({ profile, onProfile, onVerification }: Props) {
  ...
  const stages = [
    { label: 'Work profile', done: data.profile_complete },
    { label: 'Work verification', done: verificationDone },
    { label: 'WeHouse review', done: live },
  ];
  const complete = stages.filter((stage) => stage.done).length;

  let title = 'Set up your work profile';
  let note = 'Tell customers what you do, your experience and where you work.';
  let action = onProfile;
  let actionLabel = 'Complete work profile';

  if (live) {
    title = 'Your services are live';
    note = 'Customers can now find your professional profile.';
    action = onProfile;
    actionLabel = 'View professional profile';
  } else if (underReview) {
    title = 'Review in progress';
    note = 'WeHouse is reviewing your work evidence. No action is needed right now.';
    ...
    note = 'Your work checks are complete. Send them for final review.';
    action = onVerification;
    actionLabel = 'Submit for review';
  } else if (data.profile_complete) {
    title = 'Verify your work';
    note = 'Complete the verification payment, skill check and one short work video.';
    action = onVerification;
      ...
      <section className="rounded-3xl border border-violet-500/15 bg-gradient-to-br from-violet-500/[.09] via-[#12141C] to-[#0F1218] p-4 sm:p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="shrink-0 rounded-2xl border border-white/[.06] bg-black/10 px-3 py-2 text-center">
            <p className="text-lg font-bold">{complete}/3</p>
            <p className="text-[8px] text-[#686F7F]">done</p>
          </div>
        </div>
        ...
        <div className="mt-5 grid grid-cols-3 gap-2">
          {stages.map((stage, index) => {
            ...
            return (
              <div key={stage.label} className={`rounded-2xl border px-2 py-3 text-center ${stage.done ? 'border-emerald-500/15 bg-emerald-500/[.05]' : active ? 'border-violet-500/20 bg-violet-500/[.06]' : 'border-white/[.05] bg-black/10'}`}>
                <span className={`mx-auto grid h-7 w-7 place-items-center rounded-full text-[9px] font-bold ${stage.done ? 'bg-emerald-500 text-[#04120A]' : active ? 'bg-violet-500 text-white' : 'bg-white/[.05] text-[#656C7B]'}`}>{stage.done ? '✓' : index + 1}</span>
                <p className={`mt-2 text-[8px] font-medium leading-tight ${stage.done ? 'text-emerald-300' : active ? 'text-violet-200' : 'text-[#676E7E]'}`}>{stage.label}</p>
              </div>
            );
          })}
        </div>
