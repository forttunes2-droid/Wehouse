...
export default function StaffSecurityOverviewV2({onOpenCases}:{onOpenCases:()=>void}){
 const[data,setData]=useState<Monitor>({}),[loading,setLoading]=useState(true);
 useEffect(()=>{let live=true;void(async()=>{const{data:result}=await supabase.rpc('get_my_staff_security_monitor');if(live){setData((result||{}) as Monitor);setLoading(false)}})();return()=>{live=false}},[]);
 ...
 return <div className="space-y-5">
  <section className="rounded-3xl border border-red-500/15 bg-gradient-to-br from-red-500/[.08] via-[#15131A] to-[#0D1118] p-5 sm:p-6 lg:p-8">
   ...
   <p className="mt-2 max-w-2xl text-xs leading-relaxed text-[#8990A1]">This workspace shows session patterns, restricted accounts and financial failures recorded by WeHouse. Worker verification and Staff assignment are separate operational responsibilities.</p>
