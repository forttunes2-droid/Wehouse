...
export default function SupportEntryCard({profile,compact=false}:Props){
 ...
 const inner=<><div className="relative grid h-12 w-12 shrink-0 place-items-center rounded-full bg-gradient-to-br from-violet-500 to-fuchsia-600 font-bold text-white shadow-lg shadow-violet-500/10">S<span className="absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-[#11141C] bg-emerald-400"/></div><div className="min-w-0 flex-1"><p className="truncate text-[13px] font-semibold text-white">WeHouse Support</p><p className={`mt-1 truncate text-[11px] ${unread?'font-medium text-[#E3E5EB]':'text-[#777C8D]'}`}>{preview}</p><p className="mt-0.5 truncate text-[9px] text-[#5F6474]">Human support · files and voice messages supported</p></div><div className="shrink-0 self-start pt-0.5 text-right">{time&&<p className={`text-[8px] ${unread?'text-violet-300':'text-[#555A6B]'}`}>{formatTime(time)}</p>}{unread>0&&<span className="ml-auto mt-2 grid h-5 min-w-5 place-items-center rounded-full bg-violet-500 px-1 text-[8px] font-bold text-white">{unread>99?'99+':unread}</span>}</div></>;
