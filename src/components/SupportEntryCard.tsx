...
export default function SupportEntryCard({ compact = false }: Props) {
  const [preview, setPreview] = useState(
      "Message a real WeHouse support team member.",
    ),
    [unread, setUnread] = useState(0),
    [time, setTime] = useState<string | null>(null),
  ...
  const load = useCallback(async () => {
    ...
    setConversationId(row?.conversation_id || null);
    setPreview(
      row?.last_message || "Message a real WeHouse support team member.",
    );
    setUnread(Number(row?.unread_count || 0));
    setTime(row?.last_message_time || row?.created_at || null);
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`support-entry:${conversationId}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
    ...
    <>
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate text-[9px] text-[#5F6474]">
          Human support · text, photos and documents
        </p>
      </div>
