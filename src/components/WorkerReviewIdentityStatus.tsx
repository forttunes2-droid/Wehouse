...
export default function WorkerReviewIdentityStatus({
  workerId,
}: {
  workerId: string;
}) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void supabase
      .rpc("admin_get_worker_review_trust_status", { p_worker_id: workerId })
      .then(({ data, error }) => {
        if (!active) return;
  ...
  const ready = Boolean(
    data?.profile_ready && data?.readiness_passed && data?.evidence_saved,
  );
      ...
      {loading ? (
        ...
        <>
          <div className="mt-2 flex flex-wrap gap-2">
            <Badge good={!!data?.profile_ready}>Profile</Badge>
            <Badge good={!!data?.readiness_passed}>
              Readiness
              {data?.readiness_percent != null
...
function Badge({
  ...
  return (
    <span
      className={`rounded-full px-2 py-1 text-[8px] font-semibold ${good ? "bg-emerald-500/10 text-emerald-300" : "bg-amber-500/10 text-amber-300"}`}
    >
      {good ? "✓ " : "• "}
      {children}
    </span>
