...
type Props = {
  ...
  profile: Profile;
  onClose: () => void;
  onCreated: (conversationId: string, bookingId: string) => void;
};

export default function WorkerBookingRequestSheetV2({
  worker,
  profile,
  onClose,
  onCreated,
  ...
  const [description, setDescription] = useState(""),
    ...
    [servicesLoading, setServicesLoading] = useState(true),
    [serviceLoadFailed, setServiceLoadFailed] = useState(false);
  const min = useMemo(() => {
    const now = new Date();
  ...
  const dates = useMemo(
    () =>
      Array.from({ length: 14 }, (_, index) => {
        ...
        return {
          value: local.toISOString().slice(0, 10),
          label:
            index === 0
              ? "Today"
              : index === 1
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const { data, error } = await supabase
        ...
        .order("service_name");
      if (cancelled) return;
      const skills = Array.isArray(worker.worker_skills)
        ? worker.worker_skills.filter(
            (item): item is string =>
              typeof item === "string" && Boolean(item.trim()),
          )
        : [];
      if (error && skills.length === 0) {
        setServiceLoadFailed(true);
  ...
  }, [worker.user_id, worker.worker_occupation, worker.worker_skills]);

  async function submit() {
    if (servicesLoading)
      return toast.error("Worker services are still loading");
    if (serviceLoadFailed)
      return toast.error("Worker services could not be loaded. Try again.");
    if (!service) return toast.error("Choose a service");
    if (!description.trim() || !address.trim() || !date)
      return toast.error(
        "Service, description, address and preferred date are required",
      );
    ...
    setSubmitting(false);
    if (error || !booking)
      return toast.error(error?.message || "Booking request failed");
    toast.success("Booking request sent");
    onCreated(booking.conversation_id, booking.booking_id);
  }

  const detailsReady = Boolean(service && description.trim() && address.trim()),
          ...
          {step === 1 && (
            <section>
              ...
              <div className="mt-6 space-y-3">
                {servicesLoading ? (
                  <FieldMessage>Loading services…</FieldMessage>
                ) : serviceLoadFailed ? (
                  <FieldMessage error>
                    Could not load this Worker’s services. Close and try again.
                  </FieldMessage>
                ) : services.length === 0 ? (
                  <FieldMessage>No bookable service is available.</FieldMessage>
                ) : (
                  <SearchableSelect
                    label="Service"
                    value={service}
                    ...
                    searchPlaceholder="Search this Worker’s services…"
          ...
          {step === 3 && (
            <section>
              ...
              <div className="mt-6 divide-y divide-white/[.06] border-y border-white/[.06]">
                ...
                <Review
                  label="Preferred day"
                  value={new Date(`${date}T12:00:00`).toLocaleDateString([], {
                    weekday: "long",
                    day: "numeric",
                    month: "long",
                  })}
                />
                {message && <Review label="Note" value={message} />}
              </div>
            </section>
          )}
        </div>
      ...
      <footer className="shrink-0 border-t border-white/[.06] bg-[#0A0A0F] p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
        <div className="mx-auto flex max-w-lg gap-2">
          ...
          {step === 1 && detailsReady && (
            <button
              onClick={() => setStep(2)}
              className="h-12 flex-1 rounded-xl bg-violet-500 text-sm font-semibold"
            >
              Choose day
            </button>
          )}
          {step === 2 && scheduleReady && (
            <button
              onClick={() => setStep(3)}
              className="h-12 flex-1 rounded-xl bg-violet-500 text-sm font-semibold"
            >
          ...
          {step === 3 && (
            <button
              onClick={() => void submit()}
              disabled={submitting || !profile.user_id}
              className="h-12 flex-1 rounded-xl bg-violet-500 text-sm font-semibold disabled:opacity-40"
            >
