...
export default function WorkerBookingRequestSheetV2({worker,profile,onClose,onCreated}:Props){
 ...
 return <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/70 backdrop-blur-sm sm:items-center sm:p-4" onClick={onClose}><section onClick={e=>e.stopPropagation()} className="max-h-[92dvh] w-full max-w-md overflow-y-auto rounded-t-[28px] border border-white/[.08] bg-[#11141C] p-5 sm:rounded-[28px]"><div className="flex items-start justify-between gap-3"><div><p className="text-[9px] font-bold uppercase tracking-[.16em] text-blue-400">NEW SERVICE REQUEST</p><h2 className="mt-1 text-lg font-bold">{worker.full_name||worker.username||'Professional'}</h2><p className="mt-1 text-[10px] leading-relaxed text-[#6D7282]">Choose the exact service you need. You and the professional can agree the final date and price before payment.</p></div><button onClick={onClose} className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white/[.04] text-[#777C8B]">×</button></div><div className="mt-5 space-y-3"><label className="block rounded-2xl border border-white/[.08] bg-[#181B24] px-4 py-3"><span className="mb-1.5 block text-[9px] font-semibold uppercase tracking-wide text-[#6D7282]">Service</span>{servicesLoading?<p className="text-sm text-[#8B91A0]">Loading services…</p>:serviceLoadFailed?<p className="text-xs text-red-300">Could not load this Worker’s services. Close and try again.</p>:services.length===0?<p className="text-xs text-[#8B91A0]">No bookable service is available.</p>:<select value={service} onChange={e=>setService(e.target.value)} className="w-full bg-transparent text-sm text-white outline-none [color-scheme:dark]">{services.map(item=><option key={item} value={item} className="bg-[#181B24]">{item}</option>)}</select>}</label><Input value={description} set={setDescription} placeholder="Describe the work needed"/><Input value={address} set={setAddress} placeholder="Service address"/><label className="block rounded-2xl border border-white/[.08] bg-[#181B24] px-4 py-3"><span className="mb-1.5 block text-[9px] font-semibold uppercase tracking-wide text-[#6D7282]">Preferred date</span><input type="date" min={min} value={date} onChange={e=>setDate(e.target.value)} className="w-full bg-transparent text-sm text-white outline-none [color-scheme:dark]"/></label><textarea value={message} onChange={e=>setMessage(e.target.value)} rows={3} placeholder="Anything else the professional should know? (optional)" className="w-full resize-none rounded-2xl border border-white/[.08] bg-[#181B24] px-4 py-3 text-sm outline-none focus:border-blue-500/40"/><button onClick={()=>void submit()} disabled={submitting||!profile.user_id||servicesLoading||serviceLoadFailed||!service} className="h-12 w-full rounded-2xl bg-blue-500 text-sm font-semibold disabled:opacity-40">{submitting?'Sending…':'Send booking request'}</button></div></section></div>;
}
