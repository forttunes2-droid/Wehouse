...
export default function PropertyPipelineWorkspace({ profile }: {
    ...
    return <Case profile={profile} row={selected} back={() => { setSelected(null); void load(); }}/>; return <div className="space-y-5"><div className="flex items-start justify-between gap-3"><div><h3 className="text-base font-bold">Property pipeline</h3><p className="mt-1 text-[10px] text-[#707386]">Property Partner submission → inspection → preparation → publication. Completing an inspection never makes a property public.</p></div><button onClick={() => void load()} disabled={loading} className="shrink-0 rounded-xl border border-white/[0.08] bg-white/[0.025] px-3 py-2 text-[9px] font-semibold text-[#8D92A2] disabled:opacity-40">Refresh</button></div><nav className="grid grid-cols-3 border-y border-white/[0.07] sm:grid-cols-6" aria-label="Property pipeline stages">{visible.map(([id, label]) => <button key={id} onClick={() => setStage(id)} className={`relative min-h-12 px-1 text-center text-[9px] font-semibold leading-tight sm:text-[10px] ${stage === id ? 'text-violet-300' : 'text-[#73798A]'}`}>{label}{stage === id && <span className="absolute inset-x-2 bottom-0 h-0.5 rounded-full bg-violet-400"/>}</button>)}</nav>{loading ? <Loading /> : rows.length === 0 ? <Empty text="No properties are in this stage."/> : <div className="divide-y divide-white/[0.065] border-y border-white/[0.065]">{rows.map(r => <button key={r.id} onClick={() => setSelected(r)} className="flex min-h-20 w-full items-center gap-3 px-1 py-3 text-left active:bg-white/[0.025]"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-violet-500/[0.08] text-[10px] font-bold text-violet-300">{String(r.property_type || 'P').charAt(0).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{r.property_address || r.request_code}</p><p className="mt-1 truncate text-[9px] text-[#6D7182]">{r.property_city}, {r.property_state} · {String(r.property_type || 'property').replace(/_/g, ' ')}</p><p className="mt-1 truncate text-[8px] text-[#555B6C]">{r.owner_name || r.owner_email || 'Property Partner'} · {r.request_code}</p></div><div className="flex shrink-0 items-center gap-2"><Badge value={r.published_at ? 'published' : r.status}/><span className="text-[#5E6475]">›</span></div></button>)}</div>}</div>; }
