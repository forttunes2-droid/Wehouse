...
export default function BookingNegotiationChatPhase9({ conversationId, bookingId, profile, isWorker, onClose }: Props) {
  ...
  return <div className="flex min-h-[100dvh] flex-col bg-[#090A0F] text-white">
    <header className="sticky top-0 z-20 shrink-0 border-b border-white/[.06] bg-[#0E1118]/96 backdrop-blur-2xl">
      <div className="mx-auto flex max-w-4xl items-center gap-2.5 px-3 py-2.5 sm:px-4"><button onClick={onClose} className="grid h-10 w-10 shrink-0 place-items-center rounded-full text-[#8E93A3] hover:bg-white/[.05]" aria-label="Back">←</button><ChatAvatar name={peerName} src={peerAvatar} /><div className="min-w-0 flex-1"><div className="flex min-w-0 items-center gap-2"><p className="min-w-0 flex-1 truncate text-[14px] font-semibold">{peerName}</p>{statusInfo && <span className={`shrink-0 rounded-full px-2 py-1 text-[7px] ${statusInfo.color}`}>{statusInfo.label}</span>}</div><p className={`mt-0.5 truncate text-[9px] ${presence?.online ? 'text-emerald-300' : 'text-[#676D7D]'}`}>{presenceText || `${booking?.service_type || 'Worker booking'} · #${booking?.booking_code || ''}`}</p></div><button onClick={() => setMenuOpen((value) => !value)} className="grid h-10 w-10 shrink-0 place-items-center rounded-full text-xl text-[#8E93A3] hover:bg-white/[.05]">⋯</button></div>
      {menuOpen && <div className="absolute right-3 top-[3.65rem] z-30 w-56 overflow-hidden rounded-2xl border border-white/[.08] bg-[#171B24] p-1.5 shadow-2xl"><button onClick={openSupport} className="flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left text-[11px] text-violet-300 hover:bg-violet-500/[.06]">? <span>WeHouse Support</span></button><button onClick={() => { setMenuOpen(false); setConfirmDelete(true); }} className="flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left text-[11px] text-red-300 hover:bg-red-500/[.07]">⌫ <span>Delete from my Messages</span></button></div>}
