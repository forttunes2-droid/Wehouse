      ...
      <div className="space-y-4 py-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2 sm:w-fit">
            <button onClick={()=>setMode('roles')} className={`min-h-10 rounded-xl px-4 text-xs font-semibold ${mode==='roles'?'bg-blue-500 text-white':'border border-white/[.06] text-[#8A8D9D]'}`}>By account type</button>
            <button onClick={()=>setMode('people')} className={`min-h-10 rounded-xl px-4 text-xs font-semibold ${mode==='people'?'bg-blue-500 text-white':'border border-white/[.06] text-[#8A8D9D]'}`}>Specific people</button>
          </div>
        </div>
        ...
        {mode==='roles'?<div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="sm:col-span-2 lg:col-span-4 rounded-xl border border-blue-500/10 bg-blue-500/[.04] px-3 py-2 text-[10px] text-blue-300">{counting?'Counting recipients…':`${recipientCount} recipient${recipientCount===1?'':'s'} selected`}</div>
...
function RoleChoice({label,selected,onChange}:{label:string;selected:boolean;onChange:(value:boolean)=>void}){return <button onClick={()=>onChange(!selected)} className={`flex min-h-12 items-center gap-3 rounded-2xl border px-3 text-left text-xs ${selected?'border-blue-500/25 bg-blue-500/[.08] text-blue-100':'border-white/[.06] bg-white/[.02] text-[#B5B8C3]'}`}><span className={`grid h-5 w-5 shrink-0 place-items-center rounded-md border text-[10px] ${selected?'border-blue-500 bg-blue-500':'border-white/[.14]'}`}>{selected?'✓':''}</span>{label}</button>}
function AnnouncementCard({row,canDelete,onDelete}:{row:any;canDelete:boolean;onDelete:()=>void}){const[expanded,setExpanded]=useState(false);const body=String(row.content||'');const long=body.length>260;return <article className="rounded-2xl border border-white/[.06] bg-[#11131B] p-4 sm:p-5"><div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-blue-500/10 font-bold text-blue-300">W</div><div className="min-w-0 flex-1"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-sm font-semibold">{row.title||'WeHouse update'}</p><p className="mt-1 text-[9px] text-[#626678]">{new Date(row.created_at).toLocaleString()}</p></div>{canDelete&&<button onClick={onDelete} className="text-[10px] text-red-300">Delete</button>}</div><p className="mt-3 whitespace-pre-wrap text-[11px] leading-6 text-[#D0D2DB]">{expanded||!long?body:body.slice(0,260)+'…'}</p>{long&&<button onClick={()=>setExpanded(value=>!value)} className="mt-2 text-[10px] font-semibold text-blue-300">{expanded?'Show less':'Read more'}</button>}<div className="mt-3 flex flex-wrap gap-2 border-t border-white/[.05] pt-3 text-[9px] text-[#6C7080]"><span>{row.scope||'Platform-wide'}</span><span>·</span><span>{Number(row.recipient_count||0)} recipients</span><span>·</span><span>{Number(row.read_count||0)} read</span></div></div></div></article>}
