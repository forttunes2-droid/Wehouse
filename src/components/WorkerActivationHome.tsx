...
export default function WorkerActivationHome({ profile, onProfile, onVerification }: Props) {
  ...
  let note = 'Add your service, experience, price and work location.';
      ...
      <section className="rounded-3xl border border-violet-500/15 bg-gradient-to-br from-violet-500/[.09] via-[#12141C] to-[#0F1218] p-4 sm:p-5">
        ...
        <div className="mt-5 grid grid-cols-3 gap-2">
          {stages.map((stage, index) => {
            ...
            return <div key={stage.label} className={`rounded-2xl border px-2 py-3 text-center ${stage.done ? 'border-emerald-500/15 bg-emerald-500/[.05]' : active ? 'border-violet-500/20 bg-violet-500/[.06]' : 'border-white/[.05] bg-black/10'}`}><span className={`mx-auto grid h-7 w-7 place-items-center rounded-full text-[9px] font-bold ${stage.done ? 'bg-emerald-500 text-[#04120A]' : active ? 'bg-violet-500 text-white' : 'bg-white/[.05] text-[#656C7B]'}`}>{stage.done ? '✓' : index + 1}</span><p className={`mt-2 text-[8px] font-medium leading-tight ${stage.done ? 'text-emerald-300' : active ? 'text-violet-200' : 'text-[#676E7E]'}`}>{stage.label}</p></div>;
          })}
        </div>
