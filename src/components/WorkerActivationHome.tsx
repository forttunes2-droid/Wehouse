...
export default function WorkerActivationHome({ profile, onProfile, onVerification }: Props) {
  ...
  if (live) return <section className="rounded-3xl border border-emerald-500/15 bg-emerald-500/[.04] p-4 sm:p-5"><p className="text-[9px] font-bold uppercase tracking-[.16em] text-emerald-300">WEHOUSE SERVICE WORKER</p><h2 className="mt-2 text-xl font-bold">You’re ready for customers</h2><p className="mt-1 text-[10px] text-[#747B8B]">Your professional service profile is live on WeHouse.</p><button onClick={onProfile} className="mt-4 h-12 w-full rounded-2xl bg-violet-500 text-[11px] font-semibold text-white">View professional profile</button></section>;
  ...
  return <div className="space-y-3">
    ...
    <WorkerVerificationChecklist identityPassed={data.identity_passed} paymentConfirmed={paymentDone} paymentRequired={data.payment_required!==false} skillVideoSaved={data.evidence_saved}/>
