...
export default function WorkerActivationHome({ profile, onProfile, onVerification }: Props) {
  ...
  if (!data.profile_complete) return <section className="rounded-3xl border border-violet-500/15 bg-gradient-to-br from-violet-500/[.09] via-[#12141C] to-[#0F1218] p-4 sm:p-5"><p className="text-[9px] font-bold uppercase tracking-[.16em] text-violet-300">WEHOUSE SERVICES</p><h2 className="mt-2 text-xl font-bold">Set up your Service Worker profile</h2><p className="mt-1 text-[10px] leading-relaxed text-[#747B8B]">Add your service, experience, price and service area first. You can continue the remaining review steps afterward.</p><button onClick={onProfile} className="mt-4 h-12 w-full rounded-2xl bg-violet-500 text-[11px] font-semibold text-white">Continue Service Worker setup</button></section>;

  if (reviewed) return <section className="rounded-3xl border border-emerald-500/15 bg-emerald-500/[.04] p-4 sm:p-5"><p className="text-[9px] font-bold uppercase tracking-[.16em] text-emerald-300">WEHOUSE REVIEWED</p><h2 className="mt-2 text-xl font-bold">Your Service Worker review is complete</h2><p className="mt-1 text-[10px] text-[#747B8B]">{data.live?'Your Service Worker profile is live in WeHouse Services.':'Your review is complete. We’ll let you know when bookings open.'}</p><button onClick={onProfile} className="mt-4 h-12 w-full rounded-2xl bg-violet-500 text-[11px] font-semibold text-white">View Service Worker profile</button></section>;
