...
export default function RoommateSchoolFilter({school,sameSchool,profileSchool,onSchool,onToggle}:Props){
 return <section className="rounded-2xl border border-violet-500/15 bg-violet-500/[.04] p-4">
  ...
  <input value={school} onChange={event=>onSchool(event.target.value)} placeholder={profileSchool||'Enter your school'} className="mt-4 h-11 w-full rounded-xl border border-white/[.08] bg-[#0D0F16] px-3 text-xs text-white outline-none focus:border-violet-500/40"/>
