...
export default function RoommateSchoolFilter({school,sameSchool,profileSchool,onSchool,onToggle}:Props){
 ...
 return <section className="border-y border-white/[.07] py-4">
  <div className="flex items-start justify-between gap-4">
   <div className="min-w-0"><h3 id={id+'-title'} className="text-base font-semibold">Same-school matching <span className="text-sm font-normal text-[#AAA3B3]">(optional)</span></h3><p id={id+'-hint'} className="mt-2 text-sm leading-6 text-[#AAA3B3]">Limit new suggestions to your school. This does not remove existing connections.</p></div>
   <button type="button" role="switch" aria-labelledby={id+'-title'} aria-describedby={id+'-hint'} disabled={!school.trim()} aria-checked={sameSchool} onClick={onToggle} className="grid h-11 w-14 shrink-0 place-items-center rounded-xl disabled:opacity-35 focus-visible:outline focus-visible:outline-violet-300"><span className={`relative block h-7 w-12 rounded-full ${sameSchool?'bg-violet-500':'bg-[#292D37]'}`}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white transition ${sameSchool?'left-6':'left-1'}`}/></span></button>
  </div>
  ...
  <p className="mt-2 text-sm leading-6 text-[#AAA3B3]">{sameSchool?`New suggestions must have the same saved school as “${school.trim()}”.`:'Your school does not restrict new suggestions while this is off.'}</p>
 </section>;
}
