...
function short(v:number){if(v>=1000000)return `₦${(v/1000000).toFixed(1)}M`;if(v>=1000)return `₦${Math.round(v/1000)}k`;return `₦${v}`}
...
export default function DualRangeSlider({min,max,floor,ceiling,step,onChange}:DualRangeSliderProps){
  ...
  function commit(which:'min'|'max'){
    if(which==='min'){
      ...
      const parsed=Number(maxDraft);const wanted=Number.isFinite(parsed)&&maxDraft!==''?parsed:safeMax;const snapped=Math.round(wanted/step)*step;const next=Math.min(ceiling,Math.max(snapped,safeMin+step));onChange(safeMin,next);setMaxDraft(String(next));
    }
    setEditing(null);
  }
...
function BudgetInput({label,value,onFocus,onChange,onBlur}:{label:string;value:string;onFocus:()=>void;onChange:(v:string)=>void;onBlur:()=>void}){return <label className="block"><span className="mb-1 block text-[9px] text-[#6C7080]">{label}</span><div className="flex h-11 items-center rounded-xl border border-white/[.08] bg-[#171923] px-3 focus-within:border-blue-500/45"><span className="mr-1 text-xs text-[#777B8C]">₦</span><input inputMode="numeric" value={value} onFocus={onFocus} onChange={e=>onChange(e.target.value)} onBlur={onBlur} onKeyDown={e=>{if(e.key==='Enter')e.currentTarget.blur()}} className="min-w-0 flex-1 bg-transparent text-xs text-white outline-none"/></div></label>}
