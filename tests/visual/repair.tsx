...
function Review() {
  ...
  if (inner === 'account' || inner === 'ordinary-account') return <AccountCenter key={workspace} profile={{user_id:'layout-review',role:'user',full_name:'Example Account',worker_status:'profile_under_review'} as Profile} activeWorkspace={workspace} workspaceAccess={{identity:{user_id:'layout-review',account_kind:'consumer'},personal_workspace:true,privileged_workspaces:inner==='ordinary-account'?[]:[{role:'worker'},{role:'property_partner'},{role:'hotel'},{role:'admin'}]}} onSwitchWorkspace={setWorkspace} onGoToPrivacy={()=>{}} onGoToSaved={()=>{}} onGoToSecurity={()=>{}} onGoToProfileEdit={()=>{}} />;
  return <main style={{ padding: 24, color: 'white' }}><h1>Layout review · simulated data</h1><p>No customer, verification, or payment changes. Account records require authentication and will show a recoverable error here.</p><nav style={{display:'flex',gap:24,margin:'24px 0'}}>{['receipt','conversation','account','ordinary-account'].map(kind => <a key={kind} href={`?view=${kind}`}>{kind}</a>)}</nav></main>;
}
