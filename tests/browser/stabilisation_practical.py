  ...
  for width in [320,390,768,1440]:
   for mode in ['preferences','profile','publication-global','publication-worker','split','shared-guest']:
    ...
    try:
     ...
     if mode=='preferences':
      await expect(page.get_by_role('heading',name='Roommate preferences',exact=True)).to_be_visible()
      await expect(page.get_by_role('region',name='Roommate preferences',exact=True)).to_be_visible()
      assert await page.get_by_text('1 of 3',exact=True).count()==0
      assert await page.get_by_role('button',name='Continue',exact=True).count()==0
      assert await page.get_by_role('button',name='Save preferences',exact=True).count()==1
      ...
      # Hidden required inputs are revealed on invalid save, without a fake value.
      await page.get_by_role('button',name='Save preferences',exact=True).click()
      await expect(page.get_by_role('alert')).to_contain_text('Choose who')
      await expect(page.get_by_role('button',name='Edit who you would live with',exact=True)).to_have_attribute('aria-expanded','true')
      assert not await page.evaluate('Boolean(window.__savedForm)')
      await page.get_by_role('button',name='Anyone',exact=True).click()
      await page.get_by_role('button',name='Edit where you want to live',exact=True).click()
      ...
      await page.get_by_role('button',name='Edit your yearly rent share',exact=True).click()
      ...
      await page.get_by_role('button',name='Edit daily habits',exact=True).click()
      ...
      # A visit to another group cannot erase the answers just entered.
      await page.get_by_role('button',name='Edit your yearly rent share',exact=True).click()
      await expect(page.get_by_label('Maximum annual rent share')).to_have_value('500000')
      await page.get_by_role('button',name='Edit daily habits',exact=True).click()
      await expect(page.get_by_label('Overnight visitors',exact=True)).to_have_value('agreement')
      await page.get_by_role('button',name='Edit daily habits',exact=True).click()
      assert await page.evaluate('history.length')==before_history
      assert not await page.evaluate('Boolean(window.__savedForm)')
      await page.evaluate('scrollTo(0,0)')
      await page.screenshot(path=str(OUT/f'roommate-preferences-unified-{width}.png'),full_page=True)
