   ...
   for mode in ['preferences','profile','publication-global','publication-worker','split','shared-guest']:
    ...
    try:
     ...
     if mode=='preferences':
      ...
      await page.get_by_role('button',name='Flexible',exact=True).click()
      await page.get_by_role('button',name='Separate bedrooms in a shared home',exact=True).click()
      ...
      await page.get_by_text('Optional daily habits',exact=True).click()
      await page.get_by_role('group',name='How often do you usually have visitors?',exact=True).get_by_role('button',name='Rarely',exact=True).click()
      await page.get_by_role('group',name='Are you comfortable with overnight visitors?',exact=True).get_by_role('button',name='Only by agreement',exact=True).click()
