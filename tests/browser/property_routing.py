 ...
 async with async_playwright() as p:
  ...
  try:
   for width,height in [(390,844),(768,1024),(1440,900)]:
    ...
    page=await context.new_page(); errors=[]; calls=[]; mode='partner'; denied=False
    ...
    async def route(handler):
     ...
     if name=='get_my_legal_status': data={}
      ...
      data={'hotel':hotel,'capabilities':caps,'rooms':ROOMS,'bookings':BOOKINGS,'inventory':[],'room_units':[],'venues':[]}
