...
const out='test-results/profile-refinement-offline';fs.mkdirSync(out,{recursive:true});
const client=path.resolve('tests/browser/profileRefinementClientFixture.ts');
await build({entryPoints:['tests/browser/profile-refinement.tsx'],bundle:true,format:'iife',jsx:'automatic',outfile:out+'/fixture.js',plugins:[{name:'profile-refinement-offline-client',setup(api){api.onResolve({filter:/.*/},args=>{
