...
const out='test-results/attachment-offline';fs.mkdirSync(out,{recursive:true});
const client=path.resolve('tests/browser/attachmentClientFixture.ts');
await build({entryPoints:['tests/browser/attachments.tsx'],bundle:true,format:'iife',jsx:'automatic',outfile:out+'/fixture.js',plugins:[{name:'attachment-offline-client',setup(api){api.onResolve({filter:/.*/},args=>{
