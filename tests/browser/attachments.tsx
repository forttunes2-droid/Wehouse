import ChatAttachmentPicker from '../../src/components/ChatAttachmentPicker';
import SecureSupportAttachment from '../../src/components/SecureSupportAttachment';
import { validateChatUpload } from '../../src/lib/chatMediaPolicy';
...
const photoMessage = () => ({...message('Here are the room photos.'),attachments:[image,image+'?v=2',image+'?v=3',image+'?v=4',image+'?v=5'],attachment_types:Array(5).fill('image/png')});
...
function Fixture() {
  ...
  const selectMedia=async(selection:FileList|null)=>{setMediaError('');try{const picked=Array.from(selection||[]);for(const file of picked)await validateChatUpload(file,false);setFiles(picked);}catch(error){setMediaError(error instanceof Error?error.message:'Choose a photo or video.');}};
  ...
  const reply = (target=message(propertyShareMessage(house,'This is the place I mentioned.'))) => {setReplyTarget(target);(window as any).__replyTarget=target.id;(window as any).__replies=((window as any).__replies||0)+1;};
  const onSend = ()=>{ if (!property && !note.trim()) return; const content=property?propertyShareMessage(property,note):note;setSent(current=>[...current,content]);setQuotes(current=>[...current,replyTarget]);(window as any).__replyIds=[...((window as any).__replyIds||[]),replyTarget?.id||null];setReplyTarget(null);(window as any).__sent=[...sent,content];clearPropertyShare('qa-personal','chat-ada');setProperty(null);setNote('');};
  ...
  return <div className="mx-auto flex h-[100dvh] max-w-3xl flex-col bg-[#090B10] text-white">
    ...
    <main className="min-h-0 flex-1 space-y-5 overflow-y-auto px-3 py-5">
      ...
      {(mode==='shared'||mode==='own-shared') && <><RoommateBubble msg={message(propertyShareMessage(house,'This is the place I mentioned.')) as any} mine={mode==='own-shared'} onOpenActions={action} onTapReaction={action} onReply={()=>reply()} onOpenProperty={open} /><RoommateBubble msg={{...message('I like the location. Let us check the rooms.','followup'),sender_id:'qa-personal'} as any} mine onOpenActions={action} onTapReaction={action} onReply={()=>reply()} onOpenProperty={open} /></>}
      {mode==='gallery' && <><RoommateBubble msg={photoMessage() as any} mine={false} onOpenActions={action} onTapReaction={action} onReply={()=>reply(photoMessage())} onOpenProperty={open} /><MessageMedia items={[{url:'https://assets.wehouse.test/test-voice.wav',type:'audio/wav'},{url:'https://assets.wehouse.test/lease.pdf',type:'application/pdf'},{url:'javascript:alert(1)',type:'application/pdf'}]} /></>}
      ...
      {mode==='policy' && <><p className="text-sm text-[#AAA3B3]">Photos and videos only. Record a voice note with the microphone.</p><SecureSupportAttachment path="private/lease.pdf" type="application/pdf" /></>}
      {mode==='hotel' && <SharedPropertyCard property={{kind:'hotel',id:'7'}} onOpen={open} />}
      {sent.map((content,index)=><RoommateBubble key={index} msg={{...message(content,'sent-'+index),sender_id:'qa-personal'} as any} mine onOpenActions={action} onTapReaction={action} quoted={quotes[index]} onReply={()=>reply({...message(content,'sent-'+index),sender_id:'qa-personal'})} onOpenProperty={open} />)}
    </main>
    <footer className="shrink-0 border-t border-white/10 bg-[#10131B] p-3">
      ...
      {mode==='policy' && <ChatAttachmentPicker onFiles={selection=>void selectMedia(selection)} />}
